...
import { DeleteProductionDialog } from "@/features/productions/audiovisual/delete-production-dialog"
import { OperatorIconButton } from "@/components/operator-action"
import { PartCaptionsDialog } from "@/features/productions/audiovisual/support/part-captions-dialog"
import { MovePartPositionDialog } from "@/features/productions/audiovisual/support/move-part-position-dialog"
import { ProductionSpeechCreatorStage } from "@/features/creator/speech/production-speech-creator-host"
...
import { ProductionFloatingTransport } from "@/features/productions/audiovisual/support/production-floating-transport"
import { productionHealth } from "@/features/productions/audiovisual/support/production-health-sheet"
import { useProductionSpeechJobs } from "@/features/productions/audiovisual/support/use-production-speech-jobs"
import ProductionOverlays, { type ConfirmAction } from "@/features/productions/audiovisual/support/production-overlays"
...
import type {
  ...
  PlayerSource, Production, ProductionPart, SoundScene, StudioConfig, WorkspaceFile, WorkspaceFolder, WorkspaceProject, VisualScene, VoiceDirectory,
} from "@/types/domain"
...
import "./audiovisual-production.css"
...
export function AudiovisualProductionPage({ production, project, soundScene, visualScene, folders, files, productionFileIds, libraryFileIds, fileState, config, directory, refresh, refreshFiles }: {
  ...
  const [tool, setTool] = useState<ProductionToolKind>(null)
  ...
  const [replacingFile, setReplacingFile] = useState<ProductionPart | null>(null)
  const centerPaneRef = useRef<HTMLElement | null>(null)
  const soundSessionRef = useRef<SoundSceneSession | null>(null)
  const visualPersistence = useRef((document: VisualScene["document"], expectedRevision: number) => originsApi.updateVisualScene(production.id, expectedRevision, document))
  ...
  const actions = useProductionActions({ production: production, soundScene, player, refresh, refreshFiles, preparePlayerSource, feedbackMode: "inline" })
  ...
  visualPersistence.current = async (document, expectedRevision) => {
    const saved = await originsApi.updateVisualScene(production.id, expectedRevision, document)
    try {
  ...
  const productionUsageCounts = useMemo(() => {
  ...
  const renameProduction = useCallback(async (name: string) => {
    await originsApi.updateProduction(production.id, { name })
    await refresh()
  ...
  const openTool = useCallback((kind: Exclude<ProductionToolKind, null>) => {
    if (kind === "speech") { openNewSpeech(); return }
    if (kind === "audio") setAudioTarget({ mode: "new-track" })
    setInsertBeforePartId(null)
    setTool(kind)
  }, [openNewSpeech])
  const openFileReplacement = useCallback((part: ProductionPart) => {
    setInsertBeforePartId(null)
  ...
  const inspector = creatorOpen ? <ProductionSpeechCreatorStage
    context={{ workspace_id: production.workspace_id, folder_id: production.folder_id, production_id: production.id, production_type: "audiovisual", selection: { capability: "speech", target: "script_part" } }}
  ...
  return <>
      ...
      <div className="ws-body">
        ...
        {stage === "library" && <ProductionLibraryStage
          ...
          onPlayAudio={(file) => {
            ...
            void player.toggleSource({ key, url, title: file.name || file.title || "Audio", subtitle: "Production Library", kind: "file" })
          }}
          ...
          onUpload={async (file) => {
            if (!production.workspace_id) throw new Error("The Workspace File library is unavailable.")
            const details = {
              name: file.name.replace(/\.[^.]+$/, ""),
              category: null,
              tags: [],
            }
            return await originsApi.uploadAudiovisualProductionFile(production.id, file, details) as WorkspaceFile
          }}
        />}
    ...
    />
    <DeleteProductionDialog production={production} open={deleteProductionOpen} onOpenChange={setDeleteProductionOpen} onDeleted={() => { player.pause(); navigate(`${originsBase}/productions`) }} />
    ...
    {overlaysOpen && <ProductionOverlays
      tool={tool} production={production} nextPartNumber={sourceParts.length + 1} insertAt={creatorInsertAt} insertBeforePartId={insertBeforePartId}
