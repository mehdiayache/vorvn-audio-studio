...
const api = vi.hoisted(() => ({
  attachProductionLibraryFile: vi.fn(), detachProductionLibraryFile: vi.fn(),
...
describe("Production workflow", () => {
  ...
  it("starts the Production Library with Creator open and the five creation capabilities", async () => {
    render(<ProductionLibraryStage productionId={7} workspaceId={1} files={[]} libraryFileIds={[]} onUpload={vi.fn()} onRefresh={vi.fn()} />)
  ...
  it("adds a reusable visual to the Production without placing it on the Timeline", async () => {
    ...
    api.attachProductionLibraryFile.mockResolvedValue({ file_id: 88, duplicate: false })
    ...
    fireEvent.click(screen.getByRole("button", { name: "Add Harbour dusk to this Production" }))

    await waitFor(() => expect(api.attachProductionLibraryFile).toHaveBeenCalledWith(7, 88))
    expect(refresh).toHaveBeenCalledOnce()
    expect(screen.queryByRole("button", { name: /Add to Timeline/ })).toBeNull()
  })

  it("collects reusable audio from Workspace Files through the same Production Library", async () => {
    const refresh = vi.fn().mockResolvedValue(undefined)
    api.attachProductionLibraryFile.mockResolvedValue({ file_id: 89, duplicate: false })
    render(<ProductionLibraryStage
      productionId={7}
      workspaceId={1}
    ...
    fireEvent.click(screen.getByRole("button", { name: "Add Quiet score to this Production" }))

    await waitFor(() => expect(api.attachProductionLibraryFile).toHaveBeenCalledWith(7, 89))
    expect(refresh).toHaveBeenCalledOnce()
  })

  it("accepts visual files across the Production Library and shows the real pending object", async () => {
    ...
    render(<ProductionLibraryStage productionId={7} workspaceId={1} files={[]} libraryFileIds={[]} onUpload={onUpload} onRefresh={refresh} />)
  ...
  it("accepts audio uploads through the Production Library File entrance", async () => {
    const onUpload = vi.fn().mockResolvedValue({ id: 92, media_type: "audio", name: "Narration", filename: "narration.mp3" })
    const refresh = vi.fn().mockResolvedValue(undefined)
    api.attachProductionLibraryFile.mockResolvedValue({ file_id: 92, duplicate: false })
    render(<ProductionLibraryStage productionId={7} workspaceId={1} files={[]} libraryFileIds={[]} onUpload={onUpload} onRefresh={refresh} />)
    ...
    await waitFor(() => expect(onUpload).toHaveBeenCalledWith(file))
    await waitFor(() => expect(api.attachProductionLibraryFile).toHaveBeenCalledWith(7, 92))
    expect(refresh).toHaveBeenCalledOnce()
  })
  ...
  it("keeps a failed upload visible with an explicit retry", async () => {
    ...
    render(<ProductionLibraryStage productionId={7} workspaceId={1} files={[]} libraryFileIds={[]} onUpload={onUpload} onRefresh={vi.fn()} />)
  ...
  it("productions canonical visual metadata into editing facts without inventing missing values", () => {
  ...
  it("requires confirmation before detaching a visual from the Production Library", async () => {
    const refresh = vi.fn().mockResolvedValue(undefined)
    const confirm = vi.fn()
    const file = { id: 88, media_type: "image" as const, name: "Harbour dusk", filename: "harbour.webp", width: 1200, height: 800 }
    ...
    expect(request).toMatchObject({
      title: "Remove “Harbour dusk” from the Production Library?",
      confirmLabel: "Remove from Production",
      variant: "default",
    })

    await request.action()
    expect(api.detachProductionLibraryFile).toHaveBeenCalledWith(7, 88)
    expect(refresh).toHaveBeenCalledOnce()
  })
  ...
  it("keeps Library as a newest-first five-lane masonry without a parallel List mode", () => {
    ...
    render(<ProductionLibraryStage productionId={7} workspaceId={1} files={files} productionFileIds={files.map(({ id }) => id)} libraryFileIds={files.map(({ id }) => id)} onUpload={vi.fn()} onRefresh={vi.fn()} />)
