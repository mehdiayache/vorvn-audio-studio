...
export type ProductionToolKind = "speech" | "file" | "silence" | "audio" | "import" | null
...
export function ProductionToolDialog({ open, production, config, nextPartNumber, beforePartId, replacingFileId, initialAudioFileId, files, fileState, usedFileIds = [], directory, playingKey, playerPlaying, onClose, onAddSilence, onInsertFile, onPlaceAudio, onUploadFile, onUpdateFile, onKeepFile, onImported, onPlay, onRetryFiles }: {
  open: Exclude<ProductionToolKind, "speech">
  ...
  const description = open === "import" ? "Append authored Speech Drafts and Silence to this existing Production. Validate locally, map roles to owned Voices, then confirm once." : fileMode === "sound" ? "Audition freely. Nothing enters this Audio Track until you confirm." : `${destination} Audition freely; the Script changes only after confirmation.`
  useEffect(() => {
