...
export function VisualFileCard({ file, mode = "production-library", pending = false, addLabel = "Add", usedCount = 0, onPreview, onAdd, onAddToProduction, onAddToTimeline, onRemove }: {
  file: WorkspaceFile
  mode?: "production-library" | "workspace-library"
  pending?: boolean
  addLabel?: string
  usedCount?: number
  ...
  const actionButtons = mode === "production-library" ? <>
    ...
    {onAddToProduction && <OperatorIconButton label={`Add ${name} to this Production`} detail="Associates this Workspace File with the current Production." side="bottom" variant="secondary" busy={pending} busyLabel={`Adding ${name}…`} onClick={() => onAddToProduction(file)}><Plus /></OperatorIconButton>}
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end">
        <DropdownMenuGroup>
          ...
          {onAddToProduction && <DropdownMenuItem onSelect={() => onAddToProduction(file)}><Plus /> Add to this Production</DropdownMenuItem>}
          {onAddToTimeline && <DropdownMenuItem onSelect={() => onAddToTimeline(file)}><Plus /> Add to Timeline</DropdownMenuItem>}
        </DropdownMenuGroup>
        {onRemove && <><DropdownMenuSeparator /><DropdownMenuGroup><DropdownMenuItem onSelect={() => onRemove(file)}><X /> Remove from Production…</DropdownMenuItem></DropdownMenuGroup></>}
      </DropdownMenuContent>
    </DropdownMenu>
  </> : null
