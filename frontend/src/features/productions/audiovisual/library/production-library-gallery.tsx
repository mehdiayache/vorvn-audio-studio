...
type ProductionLibraryEntry =
  | { kind: "generation"; item: CreatorLibraryCreationItem; order: number; libraryEntry: LibraryEntry }
  | { kind: "upload"; item: ProductionLibraryUploadItem; order: number; libraryEntry: LibraryEntry }
  | { kind: "file"; file: WorkspaceFile; order: number; libraryEntry: LibraryEntry }

export function ProductionLibraryGallery({
  folders = [], files, productionFileIds = [], libraryFileIds = [], currentFolderId,
  uploads, creationItems = [], usageCounts, pendingId, onPreview, onAddToProduction,
  ...
  const queryContext = useMemo(() => ({ productionFileIds: productionIds, usedFileIds: usedIds, currentFolderId }), [currentFolderId, productionIds, usedIds])
  const failedCount = creationItems.filter(({ status }) => status === "failed" || status === "canceled").length
  const items = useMemo(() => {
    const candidates: ProductionLibraryEntry[] = [
      ...creationItems.map((item, order) => ({
        ...
        libraryEntry: {
          ...
          productionAssociated: item.productionAssociated ?? true,
          pending: true,
  ...
  function renderEntry(entry: (typeof items)[number]) {
    if (entry.kind === "generation") return <div className="production-library-generation-gallery-entry" key={`generation-${entry.item.id}`}>{entry.item.node}</div>
    if (entry.kind === "upload") return <ProductionLibraryUploadCard key={entry.item.id} item={entry.item} onRetry={onRetryUpload} onDismiss={onDismissUpload} />
  ...
  return <section className="production-library-gallery" aria-label="Production Library files">
    <header>
      <div className="production-library-primary-tools">
        <label className="production-library-search"><Search aria-hidden="true" /><Input aria-label="Search Library" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search names or tags" /></label>
        ...
        <Select value={usage} onValueChange={(next) => setUsage(next as LibraryUsageFilter)}><SelectTrigger aria-label="Production usage"><SelectValue /></SelectTrigger><SelectContent>{LIBRARY_USAGE_OPTIONS.map((item) => <SelectItem key={item.id} value={item.id}>{item.label}</SelectItem>)}</SelectContent></Select>
        <Button variant="outline" size="sm" onClick={onUpload}><Upload />Upload</Button>
      </div>
      <div className="production-library-gallery-filters">
