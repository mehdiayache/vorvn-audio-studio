...
import { visualFilePlaybackUrl, visualFilePosterUrl, visualFileUrl } from "@/features/files/file-presentation"
...
type ScopeFilter = Extract<LibraryScope, "production" | "workspace">

export const TimelineMediaBrowser = memo(function TimelineMediaBrowser({ files, productionFileIds, usedFileIds, collapsed, onCollapsedChange, selectedFileId, onPreview, onAdd }: {
  files: WorkspaceFile[]
  ...
  const visible = useMemo(() => queryLibraryFiles(files, libraryQuery, { productionFileIds: productionIds, usedFileIds: usedIds }), [files, libraryQuery, productionIds, usedIds])
  ...
  return <aside className="timeline-media-browser" aria-label="Media Browser">
    ...
    <div className="timeline-media-scope" aria-label="Media scope">
      {(["production", "workspace"] as ScopeFilter[]).map((value) => <button key={value} aria-pressed={scope === value} onClick={() => setScope(value)}>{value === "production" ? "This Production" : "Workspace"}</button>)}
    </div>
