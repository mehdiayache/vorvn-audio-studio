...
export function ProductionImportTool({ workspaceId, folderId, existing, config, directory, playingKey, playerPlaying, onPlay, onCancel, onCompleted }: {
  workspaceId: number
  ...
  async function validate(source: string) {
    ...
    try {
      const checked = await originsApi.validateProductionImport(decodeProductionImportJson(source))
  ...
  async function execute() {
    ...
    try {
      ...
      const job = await originsApi.enqueueProductionImport<ProductionImportResult>(plan)
      setJobId(job.id); setStage("running")
      const result = await originsApi.productionImportResult<ProductionImportResult>(job.id)
      toast.success(`${result.title} is ready to review`, { description: `${result.speech} Speech Drafts · ${result.silence} Pauses` })
      onCompleted?.(result)
      window.location.assign(`${originsBase}/productions/${result.production_public_id}`)
    } catch (reason) {
  ...
  return <div className="production-import-tool">
    ...
    <div className="production-import-body">
      {stage === "source" && <section className="import-stage import-source-stage">
        <header><span className="eyebrow">Production document</span><h2>Bring in the authored story</h2><p>Origins validates the structure. Voice, method and delivery are chosen here—not inherited silently from JSON.</p></header>
        ...
        <div className="production-import-source-actions">
          ...
          <Button variant="ghost" size="sm" asChild><a href={exampleHref} download="origins-audiovisual-production-v1-example.json"><Download /> Download example</a></Button>
        </div>
      ...
      {stage === "review" && validation && <section className="import-stage import-review-stage">
        ...
        <dl className="import-preflight">
          <div><dt>Production</dt><dd>{title}</dd></div><div><dt>Destination</dt><dd>{destinationChoice === "current" ? existing?.name : "Current Workspace"}</dd></div>
