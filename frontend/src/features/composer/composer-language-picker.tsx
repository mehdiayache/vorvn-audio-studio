...
export function ComposerLanguagePicker({ value, options, route, customVoice, onChange }: {
  value: string
  options: string[]
  route?: VoiceChoice
  customVoice: boolean
  ...
  const item = (language: string) => {
    ...
    return <CommandItem key={language} value={`${language} ${languageName(language)}`} onSelect={() => choose(language)}>
      <span className="composer-language-flag" aria-hidden="true">{language === "Auto" ? "A" : languageFlag(language)}</span>
      <span className="composer-picker-copy"><b>{languageName(language)}</b>{route && language !== "Auto" && <small>{status === "documented" ? "Documented for this method" : "Not documented · provider may still accept it"}</small>}</span>
      {value === language && <Check className="composer-picker-check" />}
    </CommandItem>
  }

  return <Popover open={open} onOpenChange={setOpen}>
    <PopoverTrigger asChild>
      <Button type="button" variant="outline" className="composer-context-trigger" aria-label="Output language" aria-expanded={open}>
        <span className="composer-language-flag" aria-hidden="true">{value === "Auto" ? "A" : languageFlag(value)}</span>
        <span className="composer-picker-copy"><b>{languageName(value)}</b><small>{route ? voiceLanguageStatus(route, value, customVoice) === "documented" ? "Documented" : value === "Auto" ? "Provider decides" : "Not documented" : "Choose after Voice"}</small></span>
        <ChevronsUpDown />
      </Button>
    </PopoverTrigger>
    <PopoverContent className="composer-picker-popover composer-language-popover" align="start" sideOffset={6}>
      <Command>
