...
export function ComposerPerformance() {
  ...
  return <section className="composer-section">
    ...
    {(controls.rate || controls.pitch || controls.volume || controls.seed) && <div className="composer-fine-grid composer-performance-controls">
      ...
      {controls.volume && <label><span>Volume <b>{composer.volume}%</b></span><Slider aria-label="Recording volume" value={[composer.volume]} min={0} max={100} step={1} onValueChange={([value = DEFAULT_RECORDING_VOLUME]) => composer.setVolume(value)} /></label>}
