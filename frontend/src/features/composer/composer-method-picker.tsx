...
export function ComposerMethodPicker({ routes, availableRoutes, selectedRouteId, selectedCapabilityId, language, customVoice, config, onSelect }: {
  routes: VoiceChoice[]
  ...
  return <Popover open={open} onOpenChange={setOpen}>
    <PopoverTrigger asChild>
      <Button type="button" variant="outline" className="composer-context-trigger composer-method-trigger" aria-label="Recording method" aria-expanded={open} disabled={!routes.length}>
        <span className="composer-method-mark" aria-hidden="true">●</span>
        <span className="composer-picker-copy"><b>{selected?.capability?.name || (routes.length ? "Choose recording method" : "Choose a Voice first")}</b><small>{selected ? `${providerName(selected.route.provider)} · ${modelLabel(selected.route)} · ${exactModel(selected.route)}` : "Exact route required"}</small></span>
        <ChevronDown />
      </Button>
    </PopoverTrigger>
    <PopoverContent className="composer-picker-popover composer-method-popover" align="start" sideOffset={6}>
      <Command>
          ...
          {groups.map((provider) => <CommandGroup key={provider} heading={providerName(provider)}>
            {methods.filter((method) => method.route.provider === provider).map((method) => {
              ...
              return <CommandItem key={method.id} value={`${method.capability?.name || "Recording method"} ${modelLabel(method.route)} ${method.route.modelId}`} disabled={!available} onSelect={() => { onSelect(method.route, method.capabilityId); setOpen(false) }}>
                <span className="composer-method-mark" aria-hidden="true">●</span>
                <span className="composer-picker-copy"><b>{method.capability?.name || "Recording method"}</b><small>{modelLabel(method.route)} · {documented ? `${language} documented` : `${language} not documented`}</small><code>{exactModel(method.route)}</code>{method.capability?.description && <em>{method.capability.description}</em>}</span>
                {selected?.id === method.id && <Check className="composer-picker-check" />}
              </CommandItem>
            })}
          </CommandGroup>)}
        </CommandList>
