...
export function ComposerDialogs() {
  ...
  return <>
    <Dialog open={Boolean(composer.editorialCommand)} onOpenChange={(open) => { if (!open) composer.setEditorialCommand(null) }}>
      <DialogContent><DialogHeader><DialogTitle>Update this Part before recording?</DialogTitle><DialogDescription>The edited words must become the Part’s editorial truth before its recording is created.</DialogDescription></DialogHeader><DialogFooter><Button variant="outline" onClick={() => composer.setEditorialCommand(null)}>Cancel</Button><Button onClick={() => { const next = composer.editorialCommand; composer.setEditorialCommand(null); if (next) composer.continueGeneration(next, true) }}>Update Part and record</Button></DialogFooter></DialogContent>
    </Dialog>
