...
export function ComposerDialogs() {
  ...
  return <>
    <Dialog open={Boolean(composer.editorialCommand)} onOpenChange={(open) => { if (!open) composer.setEditorialCommand(null) }}>
      <DialogContent><DialogHeader><DialogTitle>Update this Part before generating?</DialogTitle><DialogDescription>The edited words become the Part’s script. The current audio remains available until the new recording succeeds.</DialogDescription></DialogHeader><DialogFooter><Button variant="outline" onClick={() => composer.setEditorialCommand(null)}>Cancel</Button><Button onClick={() => { const next = composer.editorialCommand; composer.setEditorialCommand(null); if (next) composer.continueGeneration(next, true) }}>Update Part and generate</Button></DialogFooter></DialogContent>
    </Dialog>
