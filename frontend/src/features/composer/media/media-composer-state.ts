...
import { inputMode, ratioChoices, type MediaAttachmentKind, type MediaOperationCapability, type MediaParameterValues } from "./media-composer-config"
...
export function assignInputs(attachments: MediaComposerAttachment[], capability: MediaOperationCapability) {
  const counts = new Map<string, number>()
  const retained: MediaComposerAttachment[] = []
...
export function nestedReferenceAttachments(capability: MediaOperationCapability, values: MediaParameterValues, files: WorkspaceFile[]) {
  const byId = new Map(files.map((file) => [file.id, file]))
  return capability.parameters.flatMap((field): MediaComposerAttachment[] => {
    ...
    return groups.flatMap((group, groupIndex) => {
      ...
      const toAttachment = (fileId: number, listKey: "file_ids" | "audio_file_ids", position: number): MediaComposerAttachment | null => {
      ...
      ].filter((item): item is MediaComposerAttachment => Boolean(item))
    })
  })
}

export function removeNestedReference(values: MediaParameterValues, attachment: MediaComposerAttachment) {
...
export function generationAttachments(generation: MediaGeneration, files: WorkspaceFile[]) {
  const byId = new Map(files.map((file) => [file.id, file]))
  return generation.preset.inputs.map((input): MediaComposerAttachment => {
