...
import type { MediaParameterCapability, MediaParameterValues } from "./media-composer-config"

export type MediaShot = { prompt: string; duration: number }

export function mediaParameterIsVisible(field: MediaParameterCapability, values: MediaParameterValues) {
...
export function MediaScalarParameter({ field, value, onChange }: {
  field: MediaParameterCapability
  value: unknown
  onChange: (value: unknown) => void
}) {
  if (field.type === "boolean") return <label className={`media-parameter-toggle${field.key === "audio" ? " is-audio" : ""}`}>
...
export function MediaShotEditor({ field, value, onChange }: {
  field: MediaParameterCapability
  value: unknown
  onChange: (value: MediaShot[]) => void
}) {
  const shots = Array.isArray(value) ? value as MediaShot[] : []
  ...
  const update = (index: number, changes: Partial<MediaShot>) => onChange(shots.map((shot, current) => current === index ? { ...shot, ...changes } : shot))
  return <section className="media-shot-editor">
    ...
    {shots.length ? <div className="media-shot-list">{shots.map((shot, index) => <div className="media-shot-row" key={index}>
    ...
    </div>)}</div> : <p className="media-shot-empty">Add the shots you want Media to follow. Their durations must equal the final video duration.</p>}
  </section>
}
