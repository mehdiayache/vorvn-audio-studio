...
import { visualFileName, visualFilePosterUrl } from "@/features/projects/audiovisual/visuals/visuals-files"
import type { MediaAdvancedValues } from "./media-advanced-settings"
import type { MediaComposerAttachment } from "./media-composer-attachments"
import { MediaComposerInput } from "./media-composer-input"
import {
...
import type { VisualsCreationItem } from "@/features/projects/audiovisual/visuals/visuals-gallery"
...
function hiddenRequestsKey(context: ComposerContext) {
...
function initialHiddenRequests(context: ComposerContext) {
...
export function MediaComposer({ context, createOpen, onCreateOpenChange, uploading, uploadLabel, libraryFiles, recentFileIds = [], usageCounts, onUploadReference, onGenerationOutputReady, onPreviewGenerated, onAddGeneratedToTimeline, renderCreations }: {
  ...
  renderCreations?: (generatedOutputIds: Set<number>, generationItems: VisualsCreationItem[]) => ReactNode
}) {
  ...
  const [attachments, setAttachments] = useState<MediaComposerAttachment[]>([])
  ...
  const { generations, submitting, workingId, create, cancel, confirm, retryIngestion } = useMediaGenerations(context, setComposerError)
  ...
  useEffect(() => {
    ...
    void originsApi.mediaModels().then((capabilities) => {
      ...
      if (active) setComposerError(reason instanceof Error ? reason.message : "Media capabilities could not be loaded.")
    })
    return () => { active = false }
  }, [preferredOutputType])

  useEffect(() => {
    void originsApi.workspaceSavedVisualReferences(workspaceId).then(setSavedReferences).catch((reason) => setComposerError(reason instanceof Error ? reason.message : "Saved references could not be loaded."))
  }, [workspaceId])
  ...
  function addAttachments(incoming: MediaComposerAttachment[]) {
  ...
  function receiveFile(file: WorkspaceFile, preferredRole?: string, verified?: { state: "compatible" | "incompatible" | "unknown"; reasons: string[] }) {
    ...
    if (!kind || !capability || !catalog || !model) {
      setComposerError(`${visualFileName(file)} is not supported by Media.`)
      return
    }
    ...
    if (preferredSlot && !preferredSlot.media_types.includes(kind)) {
      setComposerError(`${visualFileName(file)} is not compatible with ${preferredSlot.label}.`)
      return
    }
    if (preferredSlot) {
      ...
      if (result.state !== "compatible") {
        setComposerError(result.reasons[0] || `${visualFileName(file)} is not compatible with ${preferredSlot.label}.`)
        return
      }
    }
    ...
    if (nestedKinds.includes(kind)) {
      ...
      if (!next) {
        setComposerError(`${visualFileName(file)} has no available compatible reference position.`)
        return
      }
  ...
  function removeReference(attachment: MediaComposerAttachment) {
  ...
  function generationCard(generation: MediaGeneration, compact = false) {
    ...
    return <MediaGenerationCard
      ...
      onAddToTimeline={outputFile && onAddGeneratedToTimeline ? () => {
        void onAddGeneratedToTimeline(outputFile).catch((reason) => setComposerError(operatorMessage(
  ...
  if (preferredOutputUnavailable || !catalog || !model || !capability) return <section className={`media-composer-shell${panelOpen ? "" : " is-create-collapsed"}`} aria-label="Create visual material">
    <aside className={`ws-left-pane media-create-panel${panelOpen ? "" : " is-collapsed"}`}>{panelOpen ? <><header className="ws-pane-header"><span><b>Create</b><small>{preferredOutputUnavailable ? `${preferredOutputType === "image" ? "Image" : "Video"} unavailable` : "Loading models…"}</small></span><OperatorTooltip label="Hide Create panel" detail="Give Creations more room while keeping Create one click away." side="bottom"><Button variant="ghost" size="icon-sm" aria-label="Hide Create panel" onClick={() => setPanelOpen(false)}><PanelLeftClose /></Button></OperatorTooltip></header><div className="media-composer-loading">{preferredOutputUnavailable ? `Connect an ${preferredOutputType}-capable model to use this Creation Action.` : "Loading Media capabilities…"}</div>{composerError && <p className="media-composer-error" role="alert">{composerError}</p>}</> : <div className="ws-collapsed-pane"><OperatorTooltip label="Show Create panel" detail="Show model, mode, inputs and generation controls." side="right"><Button className="ws-pane-expand" variant="ghost" size="icon-sm" aria-label="Show Create panel" onClick={() => setPanelOpen(true)}><PanelLeftOpen /></Button></OperatorTooltip><span className="ws-collapsed-context"><Sparkles aria-hidden="true" /></span></div>}</aside>
    <section className="ws-center-pane media-creations-workspace" aria-labelledby="media-creations-title"><header className="ws-pane-header media-creations-heading"><span><b id="media-creations-title">Creations</b><small>{context.project_id ? "Media for this Project" : "Reusable Workspace Files"}</small></span></header>{renderCreations?.(new Set(), [])}</section>
  </section>

  return <section className={`media-composer-shell${panelOpen ? "" : " is-create-collapsed"}`} aria-label="Create visual material">
    <aside className={`ws-left-pane media-create-panel${panelOpen ? "" : " is-collapsed"}`}>
      ...
      <header className="ws-pane-header media-create-heading"><span><b>Create</b><small>{model.label}</small></span><OperatorTooltip label="Hide Create panel" detail="Give Creations more room without losing this setup." side="bottom"><Button variant="ghost" size="icon-sm" aria-label="Hide Create panel" onClick={() => setPanelOpen(false)}><PanelLeftClose /></Button></OperatorTooltip></header>
      <MediaComposerInput
      ...
      />
      {composerError && <p className="media-composer-error" role="alert">{composerError}</p>}
      </>}
    </aside>
    <section className="ws-center-pane media-creations-workspace" aria-labelledby="media-creations-title">
      <header className="ws-pane-header media-creations-heading"><span><b id="media-creations-title">Creations</b><small>{generations.length} requests</small></span>{activeEstimate > 0 && <span className="media-active-estimate">Generation pending</span>}</header>
