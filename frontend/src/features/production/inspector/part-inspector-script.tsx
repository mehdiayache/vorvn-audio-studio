...
export function PartInspectorScript({ part, directory, currentPlaying, onPlay, onNewTake, onDuplicate, onDelete }: {
  ...
  return <div className="inspector-panel inspector-text-panel">
    <div className="inspector-part-summary">
      ...
      <div className="inspector-summary-copy">{silence ? <><b>Intentional silence</b><p>Editorial timing · {formatExactDurationMs(Number(part.duration_ms || 0))}</p></> : asset ? <><b>{part.title || "Venture audio"}</b><p>Linked Venture asset · {formatDuration(Number(part.duration_ms || 0) / 1000)}</p></> : <><VoiceIdentity voice={part.voice} identityId={part.voice_identity_id} directory={directory} compact /><p>{recorded ? `Take ${part.selected_take_number || "—"} selected · ${formatDuration(Number(part.duration_ms || 0) / 1000)} · ${formatMoney(part.spent ?? part.cost)}` : `Draft speech · revision ${part.revision || 1}`}</p></>}</div>
      <div className="inspector-summary-actions">
        ...
        {(recorded || draft) && <Button variant="outline" onClick={() => onNewTake(part)}><Plus /> {draft ? "Record draft" : "New Take"}</Button>}
      </div>
    </div>
    ...
    {recorded && <section className="inspector-text-block is-take-wording">
      <div className="inspector-section-heading"><div><span className="eyebrow">Selected immutable Take</span><h3>Take {part.selected_take_number || "—"} wording</h3></div><div className="inspector-heading-actions"><Badge className="inspector-used-badge">{takeWording.label} · used</Badge><CopyButton value={takeWording.value} /><Button variant="ghost" size="sm" disabled={!takeWording.value || !part.text} onClick={() => setCompareOpen(true)}><Columns2 /> Compare</Button></div></div>
      {takeWording.value ? <p className="inspector-script" dir={textDirection(takeWording.value)}>{takeWording.value}</p> : <div className="inspector-truth-empty"><CircleAlert /><span><b>Input version is unknown</b><small>This historical Take does not contain enough immutable snapshot truth to identify or reconstruct its active wording safely.</small></span></div>}
    ...
    {silence && <section className="inspector-type-view"><Clock3 /><div><span className="eyebrow">Sequence timing</span><h3>{formatExactDurationMs(Number(part.duration_ms || 0))} of silence</h3><p>Silence is an editorial Part. It has no Voice, Take, provider operation, captions, or generation spend.</p></div></section>}
