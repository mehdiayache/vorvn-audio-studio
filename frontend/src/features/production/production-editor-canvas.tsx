...
import type { HierarchyNode, MusicBed, PlayerSource, Production, RenderTask, VoiceDirectory } from "@/types/domain"

export function ProductionEditorCanvas({ production, tree, music, directory, renderTasks, duration, releaseOpen, contextPanel, selected, playingKey, playerPlaying, previewing, productionPlaying, productionLoaded, productionCurrentTime, exporting, onReleaseOpen, onContextPanel, onTool, onSelected, onPreview, onLocate, onSeekProduction, onPlay, onMusicChange, onChooseMusic, onExport, onRetryRender, onDismissRender, sequenceActions }: {
  ...
  return (
    <div className="production-page">
      ...
      {!releaseOpen && <main className="production-main">
        {tree && <ContextToolDock panel={contextPanel} onPanel={onContextPanel} nodes={tree} activeKey={production.key} parts={production.parts} directory={directory} productionPlaying={productionPlaying} onLocate={onLocate} onOpenTool={onTool} onPreview={onPreview} onRelease={() => onReleaseOpen(true)} />}
