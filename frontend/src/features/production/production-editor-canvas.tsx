  ...
  return (
    <div className={`production-page${composerOpen ? " has-studio-dock" : ""}`}>
      ...
      {!releaseOpen && <main className="production-main">
        ...
        <details className="production-preview-section" open={previewOpen} onToggle={(event) => setPreviewOpen(event.currentTarget.open)}><summary>Preview, timing &amp; music <span>{productionLoaded ? "Preview ready" : music.filename ? "Music attached" : "Narration only"}</span></summary>{previewOpen && <TimingOverview parts={production.parts} music={music} previewing={previewing} playingKey={playingKey} playing={playerPlaying} productionPlaying={productionPlaying} productionLoaded={productionLoaded} productionCurrentTime={productionCurrentTime} onPreview={onPreview} onLocate={onLocate} onSeekProduction={onSeekProduction} onPlay={onPlay} onMusicChange={onMusicChange} onChooseMusic={onChooseMusic} />}</details>
