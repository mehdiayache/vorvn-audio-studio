  ...
  return <section className="mix-export-workspace">
    ...
    <section className="mix-section mix-current" aria-label="Current mix summary">
      ...
      <dl className="mix-fact-list">
        ...
        <div><dt><Music2 /> Sound Design</dt><dd><b>{audioClips.length ? `${audioClips.length} audio clip${audioClips.length === 1 ? "" : "s"}` : "Sequence only"}</b><span>{firstAudioClip ? `${firstAudioClip.asset_name || "Audio selected"}${firstAudioClip.loop ? " · looping" : ""}` : "No parallel Audio Library clips in this Sound Scene"}</span></dd></div>
