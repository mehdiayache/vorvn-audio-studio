  ...
  return <section className="mix-export-workspace">
    ...
    <section className="mix-section mix-current" aria-label="Current mix summary">
      ...
      <dl className="mix-fact-list">
        <div><dt><Waves /> Script</dt><dd><b>{soundScene.resolved.sequence_projection.spans.length} audible Parts · {formatDuration(duration / 1000)}</b><span>{draftCount ? `${draftCount} planned Draft${draftCount === 1 ? " is" : "s are"} not part of current timing` : "Current canonical audio order"}</span></dd></div>
        ...
        <div><dt><Music2 /> Timeline</dt><dd><b>{audioClips.length ? `${audioClips.length} audio clip${audioClips.length === 1 ? "" : "s"}` : "Script only"}</b><span>{firstAudioClip ? `${firstAudioClip.asset_name || "Audio selected"}${firstAudioClip.loop ? " · looping" : ""}` : "No parallel Audio Library clips in Timeline"}</span></dd></div>
    ...
    <section className="mix-export-panel"><div><span className="eyebrow">Create immutable output</span><h3>Final MP3</h3><p>{draftCount ? `${draftCount} planned Draft${draftCount === 1 ? "" : "s"} will stay in Script and remain outside this file. You will confirm before export.` : "Export is local finishing. It does not call a speech provider, replace a recording, or add provider generation spend."}</p></div><Button disabled={!ready || exporting} onClick={onExport}><Download /> {exporting ? "Making MP3…" : draftCount ? "Export recorded audio" : "Make MP3"}</Button></section>
