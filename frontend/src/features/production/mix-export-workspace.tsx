  ...
  return <section className="mix-export-workspace">
    ...
    <section className="mix-section mix-current" aria-label="Current export summary">
      ...
      <dl className="mix-fact-list">
        ...
        <div><dt><Music2 /> Audio tracks</dt><dd><b>{audioClips.length ? `${audioClips.length} audio clip${audioClips.length === 1 ? "" : "s"}` : "Script only"}</b><span>{firstAudioClip ? `${firstAudioClip.asset_name || "Audio selected"}${firstAudioClip.loop ? " · looping" : ""}` : "No parallel audio clips"}</span></dd></div>
