...
export function VoiceProfileCard({ profile, playing = false, onComplete, onRetry, onEdit, onPreview }: {
  ...
  return <article className="voice-profile-card">
    ...
    <footer><div className="voice-profile-footer-copy"><span>{profile.references[0]?.original_name ? `Master · ${profile.references[0].original_name} · recorded in ${languageName}` : "Historical Alibaba voice · source recording unavailable"}</span>{active ? <span className="voice-profile-working"><i /> Building capabilities</span> : missing > 0 ? <Button variant="outline" onClick={onComplete}><Plus /> Add {missing} missing capabilit{missing === 1 ? "y" : "ies"}</Button> : <span className="voice-profile-complete"><CheckCircle2 /> All capabilities ready</span>}</div><div className="voice-profile-footer-actions"><Button variant="outline" size="sm" disabled={!canPreview} onClick={onPreview}>{playing ? <Pause /> : <Play />} {canPreview ? "Preview" : "No sample"}</Button><Button variant="ghost" size="sm" onClick={onEdit}><Pencil /> Edit</Button></div></footer>
  </article>
}
