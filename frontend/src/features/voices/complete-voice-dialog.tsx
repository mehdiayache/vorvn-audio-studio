...
export function CompleteVoiceDialog({ profile, onOpenChange, onQueued }: {
  ...
  return <Dialog open={Boolean(profile)} onOpenChange={(open) => { if (!open && !busy) onOpenChange(false) }}><DialogContent className="voice-complete-dialog"><DialogHeader><DialogTitle>Complete {profile?.name}</DialogTitle><DialogDescription>Create only the compatible model versions that are still missing. Existing voices will not be touched.</DialogDescription></DialogHeader>{!savedReference && <section className="voice-complete-source"><h3>Source recording needed</h3><p>This older voice has no preserved source. Add one clean recording to build its missing versions.</p><FileDropZone file={file} accept="audio/wav,audio/mpeg,audio/mp4,.wav,.mp3,.m4a" disabled={busy} onFile={(next) => { setFile(next); setError("") }} hint="WAV, MP3 or M4A · up to 10 MB" /></section>}<div className="voice-complete-routes">{missing.map((route) => <div key={route.model_id}><span><b>{route.label}</b><small>{route.role}</small></span><em>{route.estimated_creation_cost ? `up to $${route.estimated_creation_cost.toFixed(2)}` : "Free creation"}</em></div>)}</div>{error && <p className="voice-create-error">{error}</p>}<DialogFooter><Button variant="outline" disabled={busy} onClick={() => onOpenChange(false)}>Cancel</Button><Button disabled={busy || !missing.length || (!savedReference && !file)} onClick={() => void complete()}>{busy ? <><LoaderCircle className="spin" /> Preparing…</> : <><Sparkles /> Create {missing.length} version{missing.length === 1 ? "" : "s"}</>}</Button></DialogFooter></DialogContent></Dialog>
}
