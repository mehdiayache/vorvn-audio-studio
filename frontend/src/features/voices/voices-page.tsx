...
export function VoicesPage() {
  ...
  return <main className="voices-page">
    ...
    <section className="voice-profile-grid" aria-label={showArchived ? "Archived voices" : "Your cloned voices"}>{shownProfiles.map((profile) => <VoiceProfileCard key={profile.id} profile={profile} playing={player.source?.key === `voice:${profile.id}` && player.state === "playing"} onOpen={() => setOpenProfileId(profile.id)} onPreview={() => preview(profile)} />)}{!shownProfiles.length && (showArchived ? <div className="voices-empty"><Mic2 /><h2>No archived voices</h2><p>Archived identities stay out of casting while existing Projects keep their historical reference.</p></div> : <div className="voices-empty"><Sparkles /><h2>Create one voice, not one model ID</h2><p>Add a clean recording. {productIdentity.name} will build every compatible voice capability and keep them under one identity.</p><Button onClick={() => setCreating(true)}><Plus /> Create your first voice</Button></div>)}</section>
