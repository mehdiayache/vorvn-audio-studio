...
export function VoicesPage() {
  ...
  const shownProfiles = useMemo(() => (showArchived ? archivedProfiles : activeProfiles).filter((profile) => {
    const searchable = `${profile.name} ${profile.metadata.language || ""} ${profile.metadata.trait || ""} ${profile.metadata.accent || ""}`.toLocaleLowerCase()
