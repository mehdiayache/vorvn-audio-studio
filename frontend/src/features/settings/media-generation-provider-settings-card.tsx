...
export function MediaGenerationProviderSettingsCard({ settings, onUpdated }: {
  settings: SettingsSnapshot
  onUpdated: (next: SettingsSnapshot) => void
}) {
  const [baseUrl, setBaseUrl] = useState(settings.media_generation_provider.base_url)
  const [key, setKey] = useState("")
  const actions = useAsyncAction<"save" | "test">()
  const changed = useMemo(() => baseUrl.trim().replace(/\/$/, "") !== settings.media_generation_provider.base_url || Boolean(key.trim()), [baseUrl, key, settings.media_generation_provider.base_url])

  const save = () => actions.run("save", async () => {
    try {
      const next = await originsApi.updateKieSettings({ api_key: key, base_url: baseUrl })
      onUpdated(next)
      setKey("")
  ...
  const test = () => actions.run("test", async () => {
    try {
      const result = await originsApi.testKieConnection()
  ...
  return <section className="settings-card settings-wide">
    <header><Clapperboard /><div><h2>KIE media generation</h2><p>Server-only credential used by enabled Kling and Seedance model adapters.</p></div></header>
    <div className="settings-form-grid">
      <label><span>API base</span><Input value={baseUrl} onChange={(event) => setBaseUrl(event.target.value)} /></label>
      <label><span>API key</span><Input type="password" autoComplete="new-password" value={key} onChange={(event) => setKey(event.target.value)} placeholder={settings.media_generation_provider.configured ? "Configured — leave blank to keep" : "Enter KIE API key"} /></label>
    </div>
