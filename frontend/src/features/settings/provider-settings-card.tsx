...
export function ProviderSettingsCard({ settings, onUpdated }: { settings: SettingsSnapshot; onUpdated: (next: SettingsSnapshot) => void }) {
  ...
  const [workspace, setWorkspace] = useState(settings.provider.provider_workspace_id || "")
  const [key, setKey] = useState("")
  const actions = useAsyncAction<"save" | "test">()
  const hasUnsavedChanges = useMemo(() => region !== settings.provider.region || workspace.trim() !== (settings.provider.provider_workspace_id || "") || Boolean(key.trim()), [key, region, settings.provider.region, settings.provider.provider_workspace_id, workspace])
  const save = () => actions.run("save", async () => { try { const next = await originsApi.updateProviderSettings({ region, provider_workspace_id: workspace, api_key: key }); onUpdated(next); setKey(""); toast.success("Alibaba connection saved.") } catch (reason) { toast.error(reason instanceof Error ? reason.message : "Provider settings could not be saved.") } })
  const test = () => actions.run("test", async () => { try { const result = await originsApi.testAlibabaConnection(); if (result.connected) toast.success(`Alibaba accepted the saved ${result.region_label} connection.`); else toast.error(result.reason || "Alibaba connection is not ready.") } catch (reason) { toast.error(reason instanceof Error ? reason.message : "Alibaba connection test failed.") } })
