...
export function AdvancedSettingsCard({ settings, onUpdated }: { settings: SettingsSnapshot; onUpdated: (next: SettingsSnapshot) => void }) {
  ...
  const save = () => actions.run("save", async () => { try { onUpdated(await studioApi.updateSettings({ synth_flags: flags, extra_params: extra, naming })); toast.success("Advanced defaults saved.") } catch (reason) { toast.error(reason instanceof Error ? reason.message : "Advanced settings could not be saved.") } })
  const reset = () => actions.run("reset", async () => { try { const next = await studioApi.resetNaming(); onUpdated(next); setNaming(next.naming); toast.success("Naming defaults restored.") } catch (reason) { toast.error(reason instanceof Error ? reason.message : "Naming defaults could not be restored.") } })
