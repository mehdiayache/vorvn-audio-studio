...
export function PronunciationSettingsCard() {
  ...
  useEffect(() => { void originsApi.pronunciations().then(setRules).catch(() => undefined) }, [])
  const save = () => actions.run("add", async () => { try { const result = await originsApi.savePronunciation(draft); setRules(result.rules); setDraft(blank); toast.success("Pronunciation rule added.") } catch (reason) { toast.error(reason instanceof Error ? reason.message : "Rule could not be saved.") } })
  const toggle = (rule: PronunciationRule) => actions.run(`toggle:${rule.id}`, async () => { try { const result = await originsApi.savePronunciation({ ...rule, enabled: !rule.enabled }); setRules(result.rules) } catch (reason) { toast.error(reason instanceof Error ? reason.message : "The pronunciation rule could not be updated.") } })
