...
export function StorageSettingsCard({ settings, onUpdated }: { settings: SettingsSnapshot; onUpdated: (next: SettingsSnapshot) => void }) {
  ...
  const save = () => actions.run("save", async () => { try { onUpdated(await originsApi.updateStorageSettings(values)); setValues((current) => ({ ...current, access_key: "", secret_key: "" })); toast.success("Reference storage saved.") } catch (reason) { toast.error(reason instanceof Error ? reason.message : "Storage could not be saved.") } })
  const test = () => actions.run("test", async () => { try { const status = await originsApi.testStorage(); if (status.configured) toast.success("Storage is reachable."); else toast.error(String(status.reason || "Storage is not ready.")) } catch (reason) { toast.error(reason instanceof Error ? reason.message : "Storage test failed.") } })
