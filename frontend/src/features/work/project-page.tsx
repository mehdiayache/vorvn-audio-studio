...
export function ProjectPage({ data, refresh }: { data: ProjectOverview; refresh: () => void }) {
  ...
  return <main className="work-page">
    ...
    <div className="work-content project-library-view">
      ...
      <WorkSection title="Productions" description="One-off and independent work, outside a Series." count={data.standalone_productions.length} action={<Button size="sm" onClick={() => setCreating("production")}><Plus /> New Production</Button>}>
        {productions.length ? <div className="production-summary-list">{productions.map((production) => <RowWithMove production={production} onMove={setMoving} onDelete={setDeleting} key={production.id} />)}</div> : <WorkEmpty compact icon={<FileAudio2 />} title={needle ? "No matching Productions" : "No direct Productions"} description={needle ? "Try another search." : "Create an independent Production, or work inside a Series above."} />}
      </WorkSection>
    </div>
