  ...
  return <>
    ...
    <Dialog open={mode === "settings"} onOpenChange={(open) => { if (!open && !saving) setMode(null) }}>
      <DialogContent className="project-settings-dialog" onOpenAutoFocus={(event) => { event.preventDefault(); nameInput.current?.focus() }}>
        ...
        {venture && <div className="project-parent-context"><VentureMark identity={venture.icon} name={venture.name} /><span><small>Venture</small><b>{venture.name}</b><em>This Project lives inside this Venture.</em></span><a href={resourceHref("venture", venture.id)} aria-label={`Open Venture ${venture.name}`}><ArrowUpRight /></a></div>}
