...
export function VentureMedia({ ventureId, summary, refresh }: { ventureId: number; summary: VentureOverview["asset_summary"]; refresh: () => void }) {
  ...
  return <><section className="work-section venture-media"><header className="work-section-head"><div><h2>Media</h2><p>Reusable Venture-owned Intros, Outros, Music and Stingers.</p></div></header><div className="media-collection-grid">{collections.map((item) => <MediaCollection item={item} refresh={refresh} onOpen={() => setOpen(item)} key={item.collection_id} />)}</div></section><Dialog open={Boolean(open)} onOpenChange={(next) => { if (!next) setOpen(null) }}><DialogContent><DialogHeader><DialogTitle>{open?.name || "Media"}</DialogTitle><DialogDescription>Reusable files available to Productions in this Venture.</DialogDescription></DialogHeader><div className="venture-asset-list">{assets.length ? assets.map((asset) => <article key={asset.id}><Music2 /><div><b>{asset.title || asset.name || asset.filename}</b><small>{formatDuration(Number(asset.duration_ms || 0) / 1000)}</small></div>{asset.filename && <OperatorIconButton label={`Play ${asset.title || asset.filename}`} detail="Auditions this reusable Venture asset." size="icon" onClick={() => void player.toggleSource({ key: `asset:${asset.id}`, url: audioUrl(asset.filename), title: String(asset.title || asset.filename), subtitle: open?.name, kind: open?.name === "Music" ? "music" : "asset" })}><Play /></OperatorIconButton>}</article>) : <p className="work-empty compact">No files in this library yet.</p>}</div></DialogContent></Dialog></>
}
