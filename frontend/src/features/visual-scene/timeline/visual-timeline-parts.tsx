import { ChevronDown, ChevronUp, Copy, Eye, EyeOff, Film, Image as ImageIcon, Lock, MoreHorizontal, PencilLine, Plus, Scissors, Trash2, Unlock } from "lucide-react"
    ...
    <div className="visual-track-actions">
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent side="right" align="center">
          <DropdownMenuItem onSelect={onAdd}><Plus /> Add visual</DropdownMenuItem>
...
export function VisualTimelineClip({ clip, asset, selected, trackLocked, style, onSelect, onGesture }: {
  ...
  const name = asset ? visualAssetName(asset) : "Missing visual"
...
export function VisualContextToolbar({ track, clip, asset, saving, canSplit, onSplit, onLock, onDuplicate, onDelete }: {
  ...
  return <div className="visual-context-toolbar">
    <span>{asset?.media_type === "video" ? <Film /> : <ImageIcon />}<b>{asset ? visualAssetName(asset) : "Missing visual"}</b><small>{asset?.media_type === "video" ? `Source ${(clip.source_offset_ms / 1000).toFixed(1)}s · ` : ""}{(clip.start_ms / 1000).toFixed(1)}s · {(clip.duration_ms / 1000).toFixed(1)}s</small></span>
