...
import { visualFileName, visualFilePosterUrl, visualFileUrl } from "@/features/projects/audiovisual/visuals/visuals-files"
...
function trackMediaSummary(track: VisualSceneTrack, files: WorkspaceFile[]) {
  const byId = new Map(files.map((file) => [file.id, file]))
  let images = 0
  let videos = 0
  let missing = 0
  for (const clip of track.clips) {
    const type = byId.get(clip.file_id)?.media_type
...
export function VisualTrackControl({ track, files, collapsed, first, last, onVisible, onLocked, onAdd, onMove, onRename, onRemove }: {
  ...
  const media = trackMediaSummary(track, files)
  const displayName = visualTrackDisplayName(track, files)
  ...
  const structureActions = <>
      <OperatorIconButton label={`Add ${displayName.toLowerCase()} to ${displayName} track`} detail="Choose a compatible visual File and place it in this exact track at the playhead." className="visual-track-add" onClick={onAdd}><Plus /></OperatorIconButton>
      <DropdownMenu>
...
export function VisualTimelineClip({ clip, file, selected, trackLocked, style, onSelect, onGesture }: {
  ...
  const name = file ? visualFileName(file) : "Missing media"
  const isVideo = file?.media_type === "video"
  const thumbnailUrl = file ? (isVideo ? visualFilePosterUrl(file) : visualFileUrl(file)) : ""
  return <div className={cn("visual-timeline-clip", isVideo ? "is-video" : "is-image", selected && "is-selected", locked && "is-locked", !file && "is-missing")} style={style} role="button" tabIndex={0} aria-label={`${name} media clip`} onPointerDown={(event) => onGesture(event, "move")} onClick={onSelect} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); onSelect(event) } }}>
    {file ? <figure className="visual-timeline-thumbnail" style={isVideo ? undefined : { backgroundImage: `url(${JSON.stringify(thumbnailUrl)})` }}><img src={thumbnailUrl} alt="" draggable={false} /></figure> : <ImageIcon />}
...
export function VisualContextToolbar({ count = 1, track, clip, file, saving, canSplit, hasAudio = false, audioGain = 1, audioMuted = false, selectionLocked = clip.locked, onAudioVolumePreview, onAudioVolume, onSplit, onLock, onDuplicate, onDelete }: {
  count?: number
  track: VisualSceneTrack
  clip: VisualSceneClip
  file?: WorkspaceFile
  ...
  const label = count > 1 ? `${count} media clips` : file ? visualFileName(file) : "Missing media"
  const meta = count > 1 ? "Media selection" : file?.media_type === "video" ? "Video clip" : "Image clip"
  const mixActions = count === 1 && file?.media_type === "video" && hasAudio && onAudioVolume
  ...
  const objectActions = <>
    ...
    {count === 1 && file?.media_type === "video" && <OperatorIconButton label="Split video at playhead" detail={canSplit ? "Creates two non-destructive placements using the same source File." : "Place the playhead inside this video, at least 0.1 seconds from either edge."} className="selection-bar-command" disabled={saving || !canSplit || selectionLocked || track.locked} onClick={onSplit}><Scissors /></OperatorIconButton>}
    <OperatorIconButton label="Duplicate media placement" detail="Creates another Timeline placement using the same visual File." className="selection-bar-command" disabled={saving} onClick={onDuplicate}><Copy /></OperatorIconButton>
    <OperatorIconButton label="Remove media placement" detail={selectionLocked || track.locked ? "Unlock the placement or track before removing it." : "The visual File remains available."} className="selection-bar-command danger" disabled={saving || selectionLocked || track.locked} onClick={onDelete}><Trash2 /></OperatorIconButton>
  </>
  return <SelectionBar
    ariaLabel={`${label} actions`}
    icon={file?.media_type === "video" ? <Film /> : <ImageIcon />}
    label={label}
    meta={meta}
