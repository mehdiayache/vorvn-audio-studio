...
import { getVoiceIdentities, routesForIdentity, type SpeechEngine, type SpeechModel, type VoiceChoice } from "@/lib/voice-options"
...
export function BatchPage() {
  ...
  useEffect(() => {
    ...
    if (!compatibleRoutes.some((route) => route.id === voice)) setVoice("")
  ...
  const run = async (confirmed = false) => {
    ...
    setBusy("run")
    try {
      const next = await studioApi.runBatch({ token: sheet.token, columns: Object.fromEntries(Object.entries(columns).map(([key, value]) => [key, value === none ? null : Number(value)])), binding_id: currentRoute?.bindingId || null, catalogue_voice_id: currentRoute?.catalogueVoiceId || null, voice_identity_id: selectedIdentity?.source === "mine" ? selectedIdentity.identityId : null, format: "mp3", language, instruction: "", rate: 1, pitch: 1, volume: 50, confirmed })
  ...
  return <main className="batch-page">
    ...
    </div><aside className="batch-setup"><h2>Voice setup</h2><label><span>Default voice</span><VoicePicker identities={identities} value={identityId} directory={voices.directory} playingKey={player.source?.key} playerPlaying={player.state === "playing"} onPlay={(source) => void player.toggleSource(source)} onChange={(identity) => { setIdentityId(identity.identityId); setVoice("") }} /></label><label><span>Output language</span><Select value={language} onValueChange={setLanguage}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{languageOptions.map((item) => <SelectItem value={item} key={item}>{item}</SelectItem>)}</SelectContent></Select></label><div className="batch-methods"><span>Exact recording route</span>{selectedIdentity && <VoiceMethodPicker compact routes={selectedIdentity.routes} availableRoutes={compatibleRoutes} selectedRouteId={voice} language={language} customVoice={selectedIdentity.source === "mine"} config={voices.config} onSelect={applyRoute} />}</div>{currentRoute && <VoiceLanguageSupport compact route={currentRoute} language={language} customVoice={selectedIdentity?.source === "mine"} />}<p>{selectedIdentity ? `${selectedIdentity.name} has ${selectedIdentity.routes.length} ready exact route(s). Changing voice clears the route; changing language never changes it.` : "Choose a voice, then choose its exact route."} A mapped voice column overrides this per row.</p></aside></div>
