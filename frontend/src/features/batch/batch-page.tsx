import { Download, Layers3, LoaderCircle, Play, Upload } from "lucide-react"
...
export function BatchPage() {
  ...
  return <main className="batch-page">
    ...
    <div className="batch-layout"><div className="batch-main">
      <section className="batch-card"><header><span>1</span><div><h2>Load a spreadsheet</h2><p>Headers are detected automatically. Previewing is free.</p></div></header><FileDropZone file={file} accept=".csv,.tsv,.xlsx,.xlsm" disabled={Boolean(busy)} onFile={(next) => { setFile(next); setSheet(null); setResult(null) }} hint="CSV, TSV, XLSX or XLSM" /><Button className="batch-preview-button" variant="outline" disabled={!file || Boolean(busy)} onClick={() => void preview()}>{busy === "preview" ? <LoaderCircle className="spin" /> : <Upload />}{busy === "preview" ? "Reading…" : "Preview rows"}</Button></section>
      {sheet && <section className="batch-card"><header><span>2</span><div><h2>Map columns</h2><p>{sheet.rows} rows · showing the first {sheet.preview.length}{sheet.truncated ? ` · maximum ${sheet.max_rows}` : ""}</p></div></header><div className="batch-mapping">{(["text", "name", "voice", "language"] as const).map((key) => <label key={key}><span>{key === "text" ? "Words to speak" : key === "name" ? "File name" : key === "voice" ? "Voice per row" : "Language per row"}</span><Select value={columns[key]} onValueChange={(value) => setColumns((current) => ({ ...current, [key]: value }))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{key !== "text" && <SelectItem value={none}>Same for every row</SelectItem>}{sheet.headers.map((header, index) => <SelectItem value={String(index)} key={`${header}-${index}`}>{header}</SelectItem>)}</SelectContent></Select></label>)}</div>{sheet.voices.unknown.length > 0 && <p className="batch-warning">{sheet.voices.unknown.length} unknown voice value(s) were found in the selected voice column. Those rows will fail unless you remap it.</p>}<div className="batch-table"><table><thead><tr><th>Output file</th><th>Words</th></tr></thead><tbody>{sheet.preview.map((row, index) => { const words = (row[textColumn] || "").trim(); const label = nameColumn == null ? `row-${index + 2}` : (row[nameColumn] || `row-${index + 2}`).trim(); return <tr key={index}><td>{label}.mp3</td><td className={!words ? "empty" : ""}>{words || "Empty — skipped"}</td></tr> })}</tbody></table></div></section>}
      {sheet && <section className="batch-card"><header><span>3</span><div><h2>Check and run</h2><p>Every usable row becomes one independent file.</p></div></header><Button disabled={!voice || Boolean(busy)} onClick={() => void run()}>{busy === "run" ? <LoaderCircle className="spin" /> : <Layers3 />}{busy === "run" ? "Generating rows…" : `Generate ${sheet.rows} rows`}</Button></section>}
      {result && <section className="batch-card batch-results"><header><div><h2>Results</h2><p>{result.made} made · {result.failed} failed · ${Number(result.cost).toFixed(4)}</p></div>{result.zip && <Button variant="outline" asChild><a href={result.zip} download><Download /> Download ZIP</a></Button>}</header>{result.results.map((item) => <article key={item.row}><span>{item.row}</span><div><b>{item.name || `Row ${item.row}`}</b><small>{item.error || item.text}</small></div>{item.url && <Button variant="ghost" size="icon" onClick={() => void player.toggleSource({ key: `batch:${sheet?.token}:${item.row}`, url: item.url!, title: item.name || `Row ${item.row}`, subtitle: item.text, kind: "part" })}><Play /></Button>}</article>)}</section>}
