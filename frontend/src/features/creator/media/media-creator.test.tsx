...
import { assignInputs, inputModeIssue } from "./media-creator-state"
import { ProjectLibraryGallery } from "@/features/projects/audiovisual/library/project-library-gallery"
...
describe("Media creator", () => {
  ...
  it("keeps the model-first creation form in semantic top-to-bottom order", async () => {
    ...
    expect(screen.getByRole("button", { name: "Generate" }).closest(".media-creator-actions")).toBeTruthy()
  ...
  it("uploads a file through the canonical File pipeline before using it", async () => {
    ...
    renderCreator({ onUploadReference })
    await screen.findByRole("textbox", { name: "Media prompt" })
