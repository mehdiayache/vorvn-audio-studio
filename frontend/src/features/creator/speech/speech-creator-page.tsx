...
function PendingSpeechExecution({ execution, directory, onTerminal }: {
  execution: SpeechExecution
  directory: ReturnType<typeof useVoiceDirectory>["directory"]
  onTerminal: (execution: SpeechExecution, job: DurableJob<GenerateResult>) => void
}) {
...
export function SpeechCreatorPage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  useEffect(() => {
    if (!history) return
    setExecutions((current) => recoverSpeechExecutions(current, history))
  }, [history])
  ...
  const settleExecution = useCallback(async (execution: SpeechExecution, job: DurableJob<GenerateResult>) => {
    try {
  ...
  return <Root className="speech-creator-page" data-embedded={embedded || undefined}>
    <h1 className="sr-only">Generate speech</h1>
    ...
    <div className="speak-workbench">
      ...
      <aside className="speak-session" aria-live="polite">
        ...
        <ScrollArea className="speak-session-scroll">
          {executions.length > 0 && <section className="speak-session-group"><h3>Generating now</h3>{executions.map((execution) => <PendingSpeechExecution key={execution.jobId} execution={execution} directory={voices.directory} onTerminal={(item, job) => void settleExecution(item, job)} />)}</section>}
