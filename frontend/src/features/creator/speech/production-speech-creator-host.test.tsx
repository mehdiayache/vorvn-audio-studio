...
import { ProductionSpeechCreatorSession as ProductionCreatorSession } from "./production-speech-creator-host"
...
describe("ProductionCreatorSession", () => {
  it("moves the same live Draft from its sequence seam into the Production Stage", async () => {
    ...
    const props = {
      context: { workspace_id: 4, production_id: 7, selection: { target: "script_part" } },
      config: null,
    ...
    const view = render(<ProductionCreatorSession {...props} target={inline} presentation="inline" />)
    ...
    view.rerender(<ProductionCreatorSession {...props} target={stage} presentation="stage" />)
