...
export function SpeechCreationRoute() {
  ...
  return <section className="creator-recording-bar" aria-label="Voice and recording context">
    <CreatorCapabilityRoute className="speech-creator-route">
      <CreatorCapabilityField label="Model" className="creator-model-field">
        <SpeechModelPicker routes={creator.visibleRoutes} selectedModelKey={creator.selectedModelKey} selectedCapabilityId={creator.route?.capabilityId || null} config={creator.config} onSelect={creator.selectModel} />
      ...
      <CreatorCapabilityGrid className="speech-context-grid">
        ...
        <CreatorCapabilityField label="Output language">
          <SpeechLanguagePicker value={creator.language} options={creator.languageOptions} route={creator.currentRoute || undefined} customVoice={creator.selectedIdentity?.source === "owned"} onChange={creator.setLanguage} />
        </CreatorCapabilityField>
        <CreatorCapabilityField label="Recording mode" className="creator-method-field">
          <SpeechMethodPicker route={creator.currentRoute || undefined} selectedCapabilityId={creator.route?.capabilityId || null} onSelect={creator.applyRoute} />
        </CreatorCapabilityField>
