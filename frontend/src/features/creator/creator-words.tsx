...
export function CreatorWords() {
  ...
  const states = ["raw", "shaped", ...(creator.capabilityControls.deliveryTags || text.states.tagged ? ["tagged" as const] : [])] as TextView[]
  ...
  const textToolDisabled = !creator.currentRoute || !text.text.trim() || Boolean(text.busy) || creator.recovery.status === "loading"
  ...
  return <section className={cn("creator-section script-section", focusMode && "is-focus")} aria-label="Script workspace">
    <div className="script-command-row">
      ...
      <div className="script-command-actions">
        {!text.review && <div className="text-tools" aria-label="Text tools">
          ...
          {creator.capabilityControls.deliveryTags && <>
            <div className="text-preparation-action">
    ...
    <div className="script-content-stack">
      {creator.currentRoute && creator.taggedIncompatible && <div className="creator-warning"><b>This recording method cannot record the current Tagged version.</b><span>Original and Spoken remain available. Nothing will be deleted.</span><div>{text.states.shaped && <Button size="sm" variant="outline" onClick={() => text.select("shaped")}>Use Spoken</Button>}<Button size="sm" variant="outline" onClick={() => { if (text.states.tagged) text.select("tagged"); setFocusMode(true) }}>Review tags</Button></div></div>}
      {creator.capabilityControls.ssml && !text.review && <div className="ssml-mode-row" aria-label="Script format">
        <div className="ssml-mode-actions">
          <Button variant={creator.enableSsml ? "ghost" : "secondary"} size="sm" aria-pressed={!creator.enableSsml} onClick={() => { if (creator.enableSsml) setPlainTextConfirmOpen(true) }}><FileText />Plain text</Button>
          <Button variant={creator.enableSsml ? "secondary" : "ghost"} size="sm" aria-pressed={creator.enableSsml} onClick={() => { if (!creator.enableSsml) creator.enableSsmlDocument() }}><Code2 />{creator.enableSsml ? "SSML document" : "Convert to SSML"}</Button>
        </div>
        {creator.enableSsml
          ? <span className={cn("ssml-status", creator.ssmlValidation.valid ? "is-valid" : "is-invalid")} role="status">{creator.ssmlValidation.valid ? <Check /> : <CircleAlert />}{creator.ssmlValidation.message}</span>
      ...
      <div className={cn("script-editor-shell", text.review && "is-reviewing")}>
        ...
        </> : text.view === "tagged" && !creator.enableSsml
          ? <TaggedScriptEditor value={text.text} onChange={text.updateText} autoFocus={Boolean(creator.currentRoute)} />
          : <Textarea dir="auto" aria-label={creator.enableSsml ? `${textLabel(text.view)} SSML document` : `${textLabel(text.view)} script`} value={text.text} onChange={(event) => text.updateText(event.target.value)} placeholder={creator.enableSsml ? "Write one <speak> document…" : "Type or paste what should be said…"} autoFocus={Boolean(creator.currentRoute)} />}
      </div>
    </div>
    <div className="script-meta-row">
      <span>{displayedText.length.toLocaleString()} characters</span>
      {!creator.currentRoute && <span>Choose a Voice and recording method to prepare text</span>}
      {creator.currentRoute && !creator.capabilityControls.deliveryTags && <span>{creator.methodLabel} uses words without inline tags</span>}
      <span>Text tools · {creator.config?.text_preparation?.model || "Qwen text"} · about {formatMicroMoney(creator.textPassEstimate)}</span>
    </div>
    {text.error && <p className="creator-warning">{text.error}</p>}
    <Dialog open={plainTextConfirmOpen} onOpenChange={setPlainTextConfirmOpen}><DialogContent><DialogHeader><DialogTitle>Return to plain text?</DialogTitle><DialogDescription>SSML markup and controls will be removed. Only the readable words remain in this script version.</DialogDescription></DialogHeader><DialogFooter><Button variant="outline" onClick={() => setPlainTextConfirmOpen(false)}>Keep SSML</Button><Button onClick={() => { creator.usePlainText(); setPlainTextConfirmOpen(false) }}>Use plain text</Button></DialogFooter></DialogContent></Dialog>
    <Dialog open={compareOpen} onOpenChange={setCompareOpen}><DialogContent className="creator-compare-dialog"><DialogHeader><DialogTitle>Compare script versions</DialogTitle><DialogDescription>Original editorial words beside the prepared version.</DialogDescription></DialogHeader><div className="creator-compare-grid"><section><b>Original</b><pre>{text.states.raw}</pre></section><section><b>{compareView ? textLabel(compareView) : "Prepared"}</b><pre>{compareText}</pre></section></div></DialogContent></Dialog>
  </section>
}
