...
export function resolveWorkstationSelection({ soundSelection, visualSelection, soundTracks, visualTracks, spans, assets }: {
  ...
  const visualPlacements = visualSelectionRefs(visualSelection).flatMap((ref) => {
    ...
    return track && clip ? [{ ref, track, clip, asset: assetsById.get(clip.asset_id) }] : []
  })
