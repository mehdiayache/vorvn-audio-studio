...
export function TimelineToolbar({ summary, canUndo, canRedo, saving, snapping, followPlayhead, hasVisualScene, onUndo, onRedo, onMoveView, onSnappingChange, onFollowPlayheadChange, onAddVisual, onAddAudio }: {
  summary: string
  canUndo: boolean
  canRedo: boolean
  saving: boolean
  ...
  return <div className="sound-scene-toolbar">
    ...
    <div className="sound-scene-history">
      <OperatorTooltip label="Undo the last Timeline edit" disabledTrigger={!canUndo || saving}><Button variant="ghost" size="sm" disabled={!canUndo || saving} onClick={onUndo} aria-label="Undo Timeline edit"><Undo2 /><span>Undo</span></Button></OperatorTooltip>
      <OperatorTooltip label="Redo the last undone Timeline edit" disabledTrigger={!canRedo || saving}><Button variant="ghost" size="sm" disabled={!canRedo || saving} onClick={onRedo} aria-label="Redo Timeline edit"><Redo2 /><span>Redo</span></Button></OperatorTooltip>
    </div>
