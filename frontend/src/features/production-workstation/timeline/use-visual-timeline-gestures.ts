...
export function useVisualTimelineGestures({ session, visualSession, assets, visualTracks, selectedRefs, saving, pixelsPerSecond, snap, clearSnapGuide, activeCancel }: {
  ...
  return useCallback((event: ReactPointerEvent, ref: VisualClipRef, mode: "move" | "start" | "end") => {
    ...
    const move = (next: PointerEvent) => {
      ...
      const asset = assets.find((item) => item.id === initial.asset_id)
      if (mode === "move") {
        visualSession.moveClip(ref, snap(originalStart / 1000 + deltaMs / 1000, next.altKey) * 1000)
      } else if (mode === "start") {
        visualSession.trimClip(ref, "start", snap(originalStart / 1000 + deltaMs / 1000, next.altKey) * 1000, asset)
      } else {
        visualSession.trimClip(ref, "end", snap(originalEnd / 1000 + deltaMs / 1000, next.altKey) * 1000, asset)
      }
    }
    const cleanup = () => {
  ...
  }, [activeCancel, assets, clearSnapGuide, pixelsPerSecond, saving, selectedRefs, session, snap, visualSession, visualTracks])
}
