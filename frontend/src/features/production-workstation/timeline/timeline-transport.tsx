...
export function TimelineTransport({ session }: { session: SoundSceneSession }) {
  ...
  return <div className="timeline-inline-transport" aria-label="Timeline transport">
    <OperatorIconButton label={playing ? "Pause Timeline" : "Play Timeline"} detail={!available ? "Add or record audible material before playback." : "Controls the complete Production mix."} disabled={!available} busy={preparing} busyLabel="Preparing Timeline" onClick={() => { player.pause(); void session.togglePlayback() }}>{playing ? <Pause /> : <Play />}</OperatorIconButton>
    <span>{formatDuration(current)}</span>
    <Slider value={[current]} max={Math.max(duration, 1)} step={.05} onValueChange={([value = 0]) => session.seek(value)} aria-label="Timeline playback position" />
