...
export function TimelineTransport({ session, onActivateTimeline }: { session: SoundSceneSession; onActivateTimeline?: () => void }) {
  ...
  return <div className="timeline-inline-transport" aria-label="Timeline transport">
    <OperatorIconButton label={playing ? "Pause Timeline" : "Play Timeline"} detail={!available ? "Add or record audible material before playback." : "Controls the complete Production mix."} disabled={!available} busy={preparing} busyLabel="Preparing Timeline" onClick={() => { onActivateTimeline?.(); player.close(); void session.togglePlayback() }}>{playing ? <Pause /> : <Play />}</OperatorIconButton>
