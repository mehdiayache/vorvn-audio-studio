...
import { SoundMediaIcon, audioAssetFamily, type SoundMediaKind } from "@/features/sound-scene/audio-presentation"
...
export function resolvePreviewTarget(target: PreviewTarget, assets: VentureAsset[]) {
...
function SourcePreviewHeader({ icon, onReturnTimeline }: { icon: ReactNode; onReturnTimeline: () => void }) {
  return <WorkstationPaneHeader
    icon={icon}
    title="Preview"
    actions={<><PreviewModeLabel mode="source" /><OperatorIconButton label="Return to Timeline Preview" detail="Show the complete Production at the Timeline playhead." onClick={onReturnTimeline}><MonitorPlay /></OperatorIconButton></>}
  />
}
...
function VisualSourcePreview({ asset, soundSession, onReturnTimeline }: {
  asset: VentureAsset
  ...
  const [playing, setPlaying] = useState(false)
  const [loading, setLoading] = useState(false)
  const [duration, setDuration] = useState(Math.max(0, Number(asset.duration_ms || 0)) / 1_000)
  const [current, setCurrent] = useState(0)
  const canvas = { width: Number(asset.width || 16), height: Number(asset.height || 9) }
  ...
  return <section className="media-preview visual-preview" aria-label="Source Preview">
    <SourcePreviewHeader icon={asset.media_type === "video" ? <Film /> : <ImageIcon />} onReturnTimeline={leave} />
    <div className="visual-preview-stage">
      <div className="visual-preview-frame" data-orientation={canvas.width < canvas.height ? "portrait" : "landscape"} style={frameStyle}>
        {asset.media_type === "video"
          ? <video ref={videoRef} src={visualAssetPlaybackUrl(asset)} poster={visualAssetPosterUrl(asset)} style={{ objectFit: "contain" }} playsInline preload="metadata" onLoadedMetadata={(event) => setDuration(Number.isFinite(event.currentTarget.duration) ? event.currentTarget.duration : duration)} onWaiting={() => setLoading(true)} onPlaying={() => { setLoading(false); setPlaying(true) }} onPause={() => { setLoading(false); setPlaying(false) }} onTimeUpdate={(event) => setCurrent(event.currentTarget.currentTime || 0)} />
          : <img src={visualAssetUrl(asset)} alt={name} style={{ objectFit: "contain" }} />}
      </div>
    </div>
    <div className="preview-identity-strip">
      <div><small>{String(asset.media_type).toUpperCase()} SOURCE</small><b title={name}>{name}</b></div>
      <div className="preview-timing"><SourceTiming durationMs={Number(asset.duration_ms || 0)} /></div>
    </div>
    {asset.media_type === "video" && <PreviewTransport label="Source Preview" current={current} duration={duration} playing={playing} loading={loading} disabled={!asset.filename} onToggle={() => void toggle()} onSeek={seek} />}
  </section>
}

export function PreviewPane({ target, selection, assets, document, hasVisualPlacements, playheadMs, playback, visualSession, soundSession, visualSaving, timelineTransport, onReturnTimeline }: {
  ...
  const resolved = resolvePreviewTarget(target, assets)
  if (resolved.kind === "source") {
    ...
    if (resolved.asset.media_type === "image" || resolved.asset.media_type === "video") return <VisualSourcePreview asset={resolved.asset} soundSession={soundSession} onReturnTimeline={onReturnTimeline} />
  }
  const visualSelection = selection?.kind === "visual-placement" ? selection.primary.ref : null
  if (visualSession && hasVisualPlacements) return <TimelinePreview document={document} assets={assets} playheadMs={playheadMs} playback={playback} selection={visualSelection} session={visualSession} saving={visualSaving} transport={timelineTransport} />
  return <section className="timeline-preview-empty" aria-label="Timeline Preview">
    ...
    <div><Waves /><b>Timeline Preview</b><small>Add visual media to see the Production composition. Audio remains available in Timeline playback.</small></div>
