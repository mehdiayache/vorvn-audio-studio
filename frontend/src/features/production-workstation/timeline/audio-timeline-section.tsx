...
function SoundTrackControl({ track, volume, collapsed, soloed, soloSuppressed, onMute, onSolo, onVolumeChange, onVolumeCommit, onAdd, onRemove }: {
  ...
  return <div className={cn("sound-track-control", collapsed && "is-compact", track.muted && "is-muted", soloed && "is-solo", soloSuppressed && "is-solo-suppressed")}>
    ...
    {collapsed ? <div className="sound-track-compact-actions">
      ...
      <OperatorTooltip label={`Add audio to ${name}`} detail="Choose an Audio Library source and place it in this exact track."><Button variant="ghost" size="sm" className="sound-track-add" aria-label={`Add audio to ${name}`} onClick={onAdd}><Plus data-icon="inline-start" /> Add</Button></OperatorTooltip>
...
export function AudioTrackHeaders({ tracks, engineTracks, collapsed, soloTrackIds, sequenceSummary, onMute, onSolo, onVolumeChange, onVolumeCommit, onAdd, onRemove }: {
  ...
  return <>
    <div className="sound-sequence-control" title={collapsed ? `Script · ${sequenceSummary}` : undefined}><span className="sound-track-icon is-sequence"><Volume2 /></span>{!collapsed && <span className="sound-track-copy"><b>Script</b><small>{sequenceSummary}</small></span>}</div>
