...
import { visualAssetPosterUrl, visualAssetUrl } from "../director-assets"
...
export function DirectorGenerationCard({ operations, generation, canCancel, outputAssets = [], working, compact = false, onCancel, onConfirm, onRetrySaving, onRegenerate, onUseSettings, onPreview, onAddToTimeline }: {
  ...
  return <article className={`director-generation-card${compact ? " is-creation-item" : ""}`} data-status={generation.status}>
    ...
    <div className="director-generation-copy">
      ...
      <footer>
        ...
        </> : loadingResult ? <Button variant="outline" size="sm" disabled>Loading result…</Button> : compact ? outputReady ? <>
          <Button variant="outline" size="sm" onClick={onPreview}><Eye />Preview</Button>
          <Button size="sm" onClick={onAddToTimeline}>Add to Timeline</Button>
        </> : <>
    ...
    <Dialog open={detailsOpen} onOpenChange={setDetailsOpen}><DialogContent className="director-generation-details"><DialogHeader><DialogTitle>Creation request</DialogTitle><DialogDescription>Durable request, model, inputs and output facts.</DialogDescription></DialogHeader><dl>
