...
export function SavedReferenceCreateDialog({ open, count, onOpenChange, onCreate }: {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="director-save-reference-dialog">
      <DialogHeader><DialogTitle>Save this reference</DialogTitle><DialogDescription>Reuse these {count} media item{count === 1 ? "" : "s"} in other Productions of this Venture. Media ownership does not change.</DialogDescription></DialogHeader>
      <label className="director-reference-field"><span>Name</span><Input autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder="Harbor guide" /></label>
      <label className="director-reference-field"><span>Type</span><Select value={type} onValueChange={(value) => setType(value as SavedVisualReference["type"])}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectGroup>{types.map((item) => <SelectItem key={item.value} value={item.value}>{item.label}</SelectItem>)}</SelectGroup></SelectContent></Select></label>
      {error && <p className="director-composer-error" role="alert">{error}</p>}
