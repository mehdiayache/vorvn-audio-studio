...
export function WorkstationSoundDesign({ parts, music, selection, onSelection, onAddSound }: {
  ...
  const total = Math.max(timing.total, Number(music.duration_ms || 0) / 1000, 30)
      ...
      <div className="ws-timeline" style={{ width }}>
        ...
        <div className="ws-lane is-voice" aria-label="Voice track">
          {timing.narration.map((span) => <button key={span.part.id} className={cn("ws-timeline-clip", `is-${roleColor(span.part.authored_role)}`, selection?.kind === "part" && selection.id === span.part.id && "is-selected", span.part.enabled === false && "is-disabled")} style={styleFor(span.start, span.duration)} onClick={() => onSelection({ kind: "part", id: span.part.id })} title={`${formatAuthoredRole(span.part.authored_role) || span.part.voice_name || "Voice"} · ${formatDuration(span.duration)}`}>
            <span>{String(span.number).padStart(2, "0")}</span><b>{formatAuthoredRole(span.part.authored_role) || span.part.voice_name || "Voice"}</b><i aria-hidden="true" />
          </button>)}
        ...
        <div className="ws-lane is-sfx" aria-label="Sound effects track">
          {timing.sfx.map((span) => <button key={span.part.id} className={cn("ws-timeline-clip is-sfx", selection?.kind === "part" && selection.id === span.part.id && "is-selected")} style={styleFor(span.start, span.duration)} onClick={() => onSelection({ kind: "part", id: span.part.id })}><Waves /><b>{span.part.title || "Linked audio"}</b></button>)}
        ...
        <div className="ws-lane is-music" aria-label="Music track">
          {music.filename ? <button className={cn("ws-music-clip", selection?.kind === "music" && "is-selected")} style={styleFor(0, Math.max(timing.total, 1))} onClick={() => onSelection({ kind: "music" })}><Music2 /><span><b>{music.name || "Music bed"}</b><small>{Math.round(Number(music.volume ?? .18) * 100)}% · {music.duck ? "ducking on" : "ducking off"}</small></span><SlidersHorizontal /></button> : <button className="ws-empty-lane-action" onClick={onAddSound}><Plus /> Choose music</button>}
        </div>
      </div>
    </div>
