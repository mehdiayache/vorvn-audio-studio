...
import { fileSourceLine } from "@/lib/file-provenance"
import { formatDuration } from "@/lib/format"
import type { PlayerSource, SoundSceneClip, SoundSceneTrack, WorkspaceFile } from "@/types/domain"
import { AudioVolumeControl, type AudioVolumeMix } from "../components/audio-volume-control"
import { AUDIO_FAMILY_LABELS, SoundMediaIcon, audioFileFamily, audioUsageTags } from "../audio-presentation"
...
function technicalSummary(file: WorkspaceFile | undefined, sourceDuration: number) {
  const facts = [formatDuration(sourceDuration)]
  if (file?.audio_format) facts.push(String(file.audio_format).toUpperCase())
  if (file?.sample_rate) facts.push(`${(Number(file.sample_rate) / 1000).toFixed(Number(file.sample_rate) % 1000 ? 1 : 0)} kHz`)
  if (file?.channels) facts.push(Number(file.channels) === 1 ? "Mono" : Number(file.channels) === 2 ? "Stereo" : `${file.channels} channels`)
  return facts.join(" · ")
}

export function AudioClipInspector({ track, clip, file, playingKey, playing, onPlay, onClipChange, onClipCommit, onTrackMixChange, onTrackMixCommit, onChoose }: {
  ...
  if (!clip?.filename) return <div className="music-workbench-empty"><Music2 /><span><b>No audio</b><p>Add one reusable audio File to this track.</p></span><Button onClick={onChoose}>Choose audio</Button></div>

  const key = `file-source:${clip.file_id}`
  const active = playing && playingKey === key
  const sourceDuration = Math.max(Number(file?.duration_ms || clip.source_duration_ms || 0) / 1000, 0.1)
  ...
  const category = audioFileFamily(file || { category: clip.file_kind })
  const usage = audioUsageTags(file).find((tag) => tag === "intro" || tag === "outro" || tag === "jingle" || tag === "stinger")
  ...
  return <div className="music-workbench-content" data-category={category}>
    <section className="music-workbench-source">
      ...
      <div><span className="eyebrow">{AUDIO_FAMILY_LABELS[category]}{usage ? ` · ${usage}` : ""}</span><h3>{file?.title || file?.name || clip.file_name || "Audio"}</h3><p>{file ? `${fileSourceLine(file)} · ${technicalSummary(file, sourceDuration)}` : technicalSummary(file, sourceDuration)}</p></div>
      <OperatorIconButton label={active ? "Pause audio audition" : "Play audio audition"} detail="Auditions the source without changing its timeline placement." variant="outline" size="icon" onClick={() => onPlay({ key, url: soundClipSourceUrl(clip), title: clip.file_name || "Audio", subtitle: "Source audition", kind: "file" })}>{active ? <Pause /> : <Play />}</OperatorIconButton>
    </section>

    <OperatorInspectorSection icon={Clock3} title="Source & timing" meta="Live preview" help="Drag the source window to choose which part of the reusable file this placement uses." className="music-workbench-section">
