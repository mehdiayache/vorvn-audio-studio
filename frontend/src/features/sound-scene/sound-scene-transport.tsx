...
export function SoundSceneTransport({ session }: { session: SoundSceneSession }) {
  ...
  return <section className="transport-strip is-production is-sound-scene" aria-label="Sound Scene player">
    ...
    <span className="transport-strip-time">{formatDuration(playhead)}</span>
    <Slider className="transport-strip-seek" value={[playhead]} max={Math.max(duration, 1)} step={0.05} onValueChange={([value = 0]) => session.seek(value)} aria-label="Sound Scene playback position" />
