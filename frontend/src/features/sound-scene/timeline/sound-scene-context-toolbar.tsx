...
export function SoundSceneContextToolbar({ context, saving, canSplit, onMute, onGainPreview, onGain, onEffectsPreview, onEffects, onLock, onSplit, onDuplicate, onCrossfade, onPlaySelection, onLoopSelection, onDelete, onOptions, onOpenSequence }: {
  ...
  return <div className="sound-scene-context" aria-label={`${context.label} actions`}>
    <div className="sound-context-group is-identity"><span className="sound-context-label"><b>{context.label}</b>{context.count && context.count > 1 ? <small>{context.count} clips</small> : <small>{context.kind === "audio" ? "Audio clip" : context.kind === "silence" ? "Script pause" : "Script Part"}</small>}</span></div>
    ...
    {(context.kind === "sequence" || context.kind === "silence") && onOpenSequence && <div className="sound-context-group is-object"><Button className="sound-context-command" variant="ghost" size="sm" disabled={saving} onClick={onOpenSequence}>Open Script</Button></div>}
  </div>
}
