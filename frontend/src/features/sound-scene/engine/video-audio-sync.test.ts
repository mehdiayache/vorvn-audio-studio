...
import type { SoundSceneDocument, WorkspaceFile, VisualSceneDocument } from "@/types/domain"
...
const file = (audio = true): WorkspaceFile => ({
  id: 7,
  version_id: 11,
  media_type: "video",
  filename: "camera.mov",
...
describe("video audio synchronization", () => {
  ...
  it("follows visual edits while preserving the operator mix decision", () => {
    ...
    const first = synchronizeVideoAudio(emptySound(), visuals([visualClip]), [file()]).document
  ...
  it("removes only the linked placement when visual media disappears", () => {
    ...
    first.tracks[0]!.clips[0]!.muted = true
    const second = synchronizeVideoAudio(first, visuals([]), [file()])
  ...
  it("does not invent audio for a silent video", () => {
    expect(videoHasEmbeddedAudio(file(false))).toBe(false)
    expect(synchronizeVideoAudio(emptySound(), visuals([{
      ...
      position_x: 0, position_y: 0, scale: 1, rotation_degrees: 0, flip_horizontal: false, flip_vertical: false, opacity: 1,
      locked: false,
    }]), [file(false)]).document.tracks).toEqual([])
  })
})
