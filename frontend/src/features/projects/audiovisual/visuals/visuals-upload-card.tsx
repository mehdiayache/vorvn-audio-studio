...
export function VisualsUploadCard({ item, onRetry, onDismiss }: {
  item: VisualsUploadItem
  onRetry: (item: VisualsUploadItem) => void
  onDismiss: (item: VisualsUploadItem) => void
}) {
  ...
  const label = item.status === "queued"
    ...
    : item.status === "attaching"
      ...
      : failed
        ...
        : "Uploading…"
  return <article className="visual-file-card visuals-upload-card" data-status={item.status}>
    <div className="visual-file-preview visuals-upload-preview">
      {item.previewUrl
        ? video
          ? <video src={item.previewUrl} muted preload="metadata" playsInline />
          : <img src={item.previewUrl} alt="" />
        : <Skeleton className="visuals-upload-skeleton" />}
