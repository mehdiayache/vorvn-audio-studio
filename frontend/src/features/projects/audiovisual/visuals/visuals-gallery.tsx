...
import { VisualsUploadCard, type VisualsUploadItem } from "./visuals-upload-card"
import { VisualFileCard } from "./visual-file-card"

export type VisualsCreationItem = {
  id: string
  node: ReactNode
...
export function VisualsGallery({ files, uploads, creationItems = [], usageCounts, pendingId, onPreview, onAddToTimeline, onRemove, onRetryUpload, onDismissUpload, onUpload, onOpenLibrary }: {
  files: WorkspaceFile[]
  uploads: VisualsUploadItem[]
  creationItems?: VisualsCreationItem[]
  ...
  onRetryUpload: (item: VisualsUploadItem) => void
  onDismissUpload: (item: VisualsUploadItem) => void
  onUpload: () => void
  onOpenLibrary: () => void
}) {
  ...
  if (!creationItems.length && !uploads.length && !files.length) return <section className="visuals-empty" aria-label="Visuals is empty">
    <span><Images aria-hidden="true" /></span>
    <h2>No visuals collected yet</h2>
    <p>Upload files above or choose from Library. Visuals keeps the material here; you decide what enters Timeline.</p>
  ...
  function renderEntry(entry: (typeof items)[number]) {
    if (entry.kind === "generation") return <div className="visuals-generation-gallery-entry" key={`generation-${entry.id}`}>{entry.node}</div>
    if (entry.kind === "upload") return <VisualsUploadCard key={entry.item.id} item={entry.item} onRetry={onRetryUpload} onDismiss={onDismissUpload} />
  ...
  return <section className="visuals-gallery" aria-label="Creation gallery">
    <header>
      <div className="visuals-gallery-filters">
    ...
    {!items.length && <div className="visuals-filter-empty"><Images /><p>No media matches these filters.</p><Button variant="outline" size="sm" onClick={() => { setMediaFilter("all"); setOriginFilter("all"); setShowFailed(false) }}>Clear filters</Button></div>}
    <div ref={galleryRef} className={`visuals-gallery-items${items.length <= columnCount ? " is-single-row" : ""}`} style={{ "--visuals-gallery-columns": columnCount } as CSSProperties}>
