...
export type WorkstationPartActions = {
  select: (part: ProjectPart) => void
  edit: (part: ProjectPart) => void
  replaceFile: (part: ProjectPart) => void
  play: (source: PlayerSource) => void
  captions: (part: ProjectPart) => void
  duplicate: (part: ProjectPart) => void
  remove: (part: ProjectPart) => void
  move: (part: ProjectPart, direction: -1 | 1) => void
  moveToPosition: (part: ProjectPart) => void
  retry: (part: ProjectPart, job: DurableJob<GenerateResult>) => void
  confirm: (part: ProjectPart, job: DurableJob<GenerateResult>) => void
  setEnabled: (part: ProjectPart, enabled: boolean) => void
  editSilence: (part: ProjectPart, seconds: number) => void
  addBefore: (part: ProjectPart, kind: SequenceInsertKind) => void
  reorderToPosition: (part: ProjectPart, position: number) => void
  isPending: (part: ProjectPart, action: "enabled" | "duplicate" | "move" | "silence" | "delete" | "replace") => boolean
}

export type SequenceInsertKind = "speech" | "silence" | "file"
type DragHandleProps = HTMLAttributes<HTMLElement> & { draggable?: boolean }

function operationJob(part: ProjectPart, liveJobs: Record<string, DurableJob<unknown>>) {
  if (!part.speech_job) return null
  return (liveJobs[part.speech_job.id] || part.speech_job) as DurableJob<GenerateResult>
}

function captionJob(part: ProjectPart, liveJobs: Record<string, DurableJob<unknown>>) {
...
function workstationPartReadiness(part: ProjectPart): Exclude<WorkstationPartState, "skipped"> {
...
export function workstationPartState(part: ProjectPart): WorkstationPartState {
...
function PartInclusionButton({ part, actions, noun = "Part" }: { part: ProjectPart; actions: WorkstationPartActions; noun?: string }) {
...
export function WorkstationOutline({ parts, selectedId, playingKey, playerPlaying = false, directory, onSelect, onCollapse }: {
  ...
  return <div className="ws-outline">
    ...
    <label className="ws-search"><Search /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Find a role or line" aria-label="Search Project outline" /></label>
...
function SilenceCard({ part, index, selected, actions, dragHandleProps }: { part: ProjectPart; index: number; selected: boolean; actions: WorkstationPartActions; dragHandleProps?: DragHandleProps }) {
...
function PartActionsMenu({ part, actions }: { part: ProjectPart; actions: WorkstationPartActions }) {
...
function compactFileDuration(part: ProjectPart) {
...
export function WorkstationSequence({ parts, selectedId, playingKey, playerPlaying, liveJobs, directory, actions, onAddEnd }: {
  ...
  function dragHandleProps(part: ProjectPart): DragHandleProps {
    return {
      draggable: true,
      onDragStart: (event) => {
  ...
  return <div className="ws-sequence-canvas" aria-label="Project sequence">
