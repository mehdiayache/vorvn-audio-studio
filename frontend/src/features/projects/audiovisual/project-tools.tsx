...
import type { AudioLibraryMode, FileUpdateInput, FileUploadInput, CatalogKeepInput, GeneratedKeepInput } from "@/features/workspace/library/audio-library"
...
import type { CatalogKeepResult, GeneratedKeepResult, LoadState, PlayerSource, Project, StudioConfig, WorkspaceFile } from "@/types/domain"
...
export function ProjectToolDialog({ open, project, config, nextPartNumber, beforePartId, replacingFileId, initialAudioFileId, files, fileState, usedFileIds = [], directory, playingKey, playerPlaying, onClose, onAddSilence, onInsertFile, onPlaceAudio, onUploadFile, onUpdateFile, onKeepFile, onKeepGenerated, onImported, onPlay, onRetryFiles }: {
  ...
  return <Dialog open={Boolean(open)} onOpenChange={(next) => { if (!next) onClose() }}>
    <DialogContent className={`tool-dialog ${open === "import" ? "import-dialog" : open === "silence" ? "silence-dialog" : "file-dialog"}`}>
      ...
      <Suspense fallback={open === "file" || open === "audio" ? <AudioLibraryToolLoading /> : <div className="tool-panel-body"><span className="eyebrow">Loading tool…</span></div>}>
        ...
        {(open === "file" || open === "audio") && <AudioLibrary files={files} loading={fileState.status === "loading" && !fileState.data} refreshing={fileState.status === "loading" && Boolean(fileState.data)} resourceError={fileState.status === "error" ? fileState.error : undefined} onRetryResource={onRetryFiles} usedFileIds={usedFileIds} mode={fileMode} projectId={projectId} chooseLabel={replacingFile ? "Replace linked audio" : undefined} initialSelectedId={fileMode === "sound" ? initialAudioFileId : replacingFileId} playingKey={playingKey} playerPlaying={playerPlaying} transport={libraryTransport} onChoose={fileMode === "sound" ? onPlaceAudio : onInsertFile} onUpload={onUploadFile} onUpdate={onUpdateFile} onKeep={onKeepFile} onKeepGenerated={onKeepGenerated} onPlay={onPlay} />}
