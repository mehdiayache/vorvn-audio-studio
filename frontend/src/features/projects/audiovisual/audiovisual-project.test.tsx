...
import type { DurableJob, GeneratePayload, GenerateResult, ProjectPart, SoundScene, VisualScene, VoiceDirectory } from "@/types/domain"
import { audioUrl } from "@/lib/api"
import { GlobalPlayerProvider } from "@/components/global-player-provider"
import { InlineProjectName } from "./workstation-header"
import { WorkstationFileCard, WorkstationOutline, WorkstationSequence, WorkstationSequenceCard, workstationPartState, type WorkstationPartActions } from "./workstation-sequence"
import { TimelineWorkspace } from "@/features/projects/audiovisual/timeline/timeline-workspace"
...
function partActions(values: Partial<WorkstationPartActions> = {}): WorkstationPartActions {
  return {
    select: vi.fn(), edit: vi.fn(), replaceFile: vi.fn(), play: vi.fn(), captions: vi.fn(), duplicate: vi.fn(), remove: vi.fn(),
...
function scene(parts: ProjectPart[], sourceDurationMs = 60_000): SoundScene {
  let cursor = 0
  ...
  const clip = { id: musicClipId, file_id: 9, duration_ms: null, source_offset_ms: 0, gain: .12, fade_in_ms: 2_000, fade_out_ms: 4_000, loop: true, ducking: true, muted: false, locked: false, effects: [], anchor: { kind: "absolute" as const, position_ms: 0 }, file_name: "Quiet room", filename: "bed.mp3", source_duration_ms: sourceDurationMs, resolved_start_ms: 0, resolved_duration_ms: cursor }
...
describe("Project Workstation", () => {
  it("renames a Project inline without introducing a settings flow", async () => {
    const rename = vi.fn().mockResolvedValue(undefined)
    render(<InlineProjectName name="Esther story" onRename={rename} />)

    fireEvent.click(screen.getByRole("button", { name: "Rename Project Esther story" }))
  ...
  it("offers the original Part recording without requiring a Project export", () => {
  ...
  it("projects actual Project timing into distinct sound tracks", () => {
    const parts = [
      ...
      part({ id: 3, position: 2, kind: "file", title: "Door closes", duration_ms: 2_000, clip_id: 30 }),
    ]
  ...
  it("keeps audio-only Projects neutral and reveals Timeline Preview after a visual placement exists", () => {
    ...
    const props = { session: sessionFor(scene([part({ duration_ms: 30_000 })])), files: [], onAddVisual: vi.fn(), onRemoveClip: vi.fn(), onRemoveTrack: vi.fn() }
    ...
    const placedVisual: VisualScene = { ...emptyVisual, document: { ...emptyVisual.document, tracks: [{ ...emptyVisual.document.tracks[0]!, clips: [{ id: "placement", file_id: 91, start_ms: 0, duration_ms: 5_000, source_offset_ms: 0, fit: "cover", position_x: 0, position_y: 0, scale: 1, rotation_degrees: 0, flip_horizontal: false, flip_vertical: false, opacity: 1, locked: false }] }] } }
  ...
  it("presents linked audio as a reusable Workspace File instead of empty speech", () => {
    const replaceFile = vi.fn()
    const source = part({ id: 4, position: 3, kind: "file", title: "Temple door stinger", text: "Temple door stinger", filename: "door.wav", duration_ms: 2_400, clip_id: 40, file_category: "SFX", cost: 0 })
    ...
    expect(replaceFile).toHaveBeenCalledWith(source)
  })

  it("sizes the timeline from Project time instead of the raw music source", () => {
    const story = part({ duration_ms: 120_000 })
    const soundScene = scene([story], 1_500_000)
    soundScene.document.tracks[0]!.clips[0]!.file_name = "Long source"
    soundScene.resolved.tracks[0]!.clips[0]!.file_name = "Long source"
    const { container } = render(<TimelineWorkspace session={sessionFor(soundScene)} onAddAudio={vi.fn()} onRemoveClip={vi.fn()} onRemoveTrack={vi.fn()} />)
  ...
  it("keeps a stable contextual toolbar while shift-selecting several clips", () => {
    ...
    const second = { ...soundScene.document.tracks[0]!.clips[0]!, id: "88af885c-aeb4-49bf-9edb-d3fc14496b2c", file_name: "Rain layer", anchor: { kind: "absolute" as const, position_ms: 20_000 }, resolved_start_ms: 20_000 }
  ...
  it("keeps exact positioning available in the contextual Part actions", () => {
