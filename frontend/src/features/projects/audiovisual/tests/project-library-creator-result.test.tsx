...
const api = vi.hoisted(() => ({
  attachProjectLibraryFile: vi.fn(),
  workspace: vi.fn(),
}))
...
vi.mock("@/features/creator/creator-capability-dispatcher", () => ({
  CreatorCapabilityDispatcher: ({ session, onResult, resultAction }: {
    session: { capability: string; context: { project_id?: number | null }; renderWorkspace: (value: { creator: React.ReactNode; library: React.ReactNode }) => React.ReactNode }
    ...
    creator: <div>
      ...
      <span data-testid="project-context">{session.context.project_id}</span>
...
describe("Audiovisual Library Creator result handling", () => {
  it("links Speech CreatorResult to the Project and keeps Timeline as a host action", async () => {
    ...
    expect(screen.getByTestId("project-context").textContent).toBe("7")
