...
const fixtures: Array<[string, WorkspaceFile]> = [
  ...
  ["existing File", { id: 5, media_type: "image", name: "Archive", filename: "archive.png", metadata: {} }],
...
describe("File provenance surface contract", () => {
  it.each(fixtures)("keeps Library, Visuals and Timeline aligned for %s", (_name, file) => {
    ...
    const visuals = render(<VisualFileCard file={file} onPreview={vi.fn()} />)
    expect(sourceContract(visuals.container)).toEqual(expectedContract)
    visuals.unmount()

    const timeline = render(<TimelineMediaBrowser
      files={[file]}
      projectFileIds={[file.id]}
      usedFileIds={[]}
      collapsed={false}
