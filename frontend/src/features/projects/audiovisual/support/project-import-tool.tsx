...
import {
  decodeProjectImportJson, PROJECT_IMPORT_EXAMPLE,
  type ProjectImportPlan, type ProjectImportResult,
  type ProjectImportRoute, type ProjectImportValidation,
} from "./project-import"
...
function WizardRail({ stage }: { stage: Stage }) {
  ...
  return <ol className="project-import-rail" aria-label="Import progress">
...
function JobProgressView({ jobId }: { jobId: string }) {
  const job = useSyncExternalStore(
    ...
    () => jobObserver.getSnapshot<ProjectImportResult>(jobId),
  )
  const percent = Math.round((job?.progress || 0) * 100)
  return <div className="project-import-running" role="status" aria-live="polite">
    <span className="import-running-icon"><LoaderCircle className="spin" /></span>
    <div><span className="eyebrow">Preparing Project</span><h3>{job?.detail || "Starting import…"}</h3><p>The Project is durable. You can safely leave this dialog after it opens.</p></div>
    <Progress value={percent} />
    <b>{percent}%</b>
  </div>
}

export function ProjectImportTool({ workspaceId, folderId, existing, config, directory, playingKey, playerPlaying, onPlay, onCancel, onCompleted }: {
  workspaceId: number
  ...
  onCancel: () => void
  onCompleted?: (result: ProjectImportResult) => void
}) {
  const [stage, setStage] = useState<Stage>("source")
  const [file, setFile] = useState<File | null>(null)
  const [validation, setValidation] = useState<ProjectImportValidation | null>(null)
  ...
  const estimatedTextToolCost = usesTextTools && validation
    ...
    ? validation.document.items.reduce((total, item) =>
      ...
      * config.text_preparation.estimated_price_per_million_characters
    : 0
  const exampleHref = `data:application/json;charset=utf-8,${encodeURIComponent(`${JSON.stringify(PROJECT_IMPORT_EXAMPLE, null, 2)}\n`)}`

  async function validate(source: string) {
    setChecking(true); setError(""); setValidation(null)
    try {
      const checked = await originsApi.validateProjectImport(decodeProjectImportJson(source))
      ...
      setError(reason instanceof Error ? reason.message : "This Project document could not be validated.")
    } finally { setChecking(false) }
  }

  async function choose(next: File) {
    setFile(next); setPasting(false); setError("")
    if (!next.name.toLocaleLowerCase().endsWith(".json")) { setError("Choose a .json Project document."); return }
  ...
  function roleRoutes(): Record<string, ProjectImportRoute> {
    if (!validation) return {}
  ...
  async function execute() {
    if (!validation || !title.trim() || !rolesReady) return
    setSubmitting(true); setError("")
    try {
      const plan: ProjectImportPlan = {
        document: validation.document,
        destination: destinationChoice === "current" && existing
          ? { kind: "existing", project_id: existing.id }
          : { kind: "new", workspace_id: workspaceId, folder_id: folderId || null },
      ...
      const job = await originsApi.enqueueProjectImport<ProjectImportResult>(plan)
      setJobId(job.id); setStage("running")
      const result = await originsApi.projectImportResult<ProjectImportResult>(job.id)
      toast.success(`${result.title} is ready to review`, { description: `${result.speech} Speech Drafts · ${result.silence} Pauses` })
      onCompleted?.(result)
      window.location.assign(`${originsBase}/projects/${result.project_public_id}`)
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "The Project import could not finish.")
      setStage("review")
  ...
  return <div className="project-import-tool">
    <WizardRail stage={stage} />
    <div className="project-import-body">
      {stage === "source" && <section className="import-stage import-source-stage">
        <header><span className="eyebrow">Project document</span><h2>Bring in the authored story</h2><p>Origins validates the structure. Voice, method and delivery are chosen here—not inherited silently from JSON.</p></header>
        <FileDropZone file={file} accept="application/json,.json" kind="file" emptyLabel="Drop a Project JSON here" chooseLabel={checking ? "Checking…" : "Choose JSON"} hint="V1 · Speech and Pause · maximum 5 MB" disabled={checking} onFile={(next) => void choose(next)} />
        <div className="project-import-source-actions">
          ...
          <Button variant="ghost" size="sm" asChild><a href={exampleHref} download="origins-audiovisual-project-v1-example.json"><Download /> Download example</a></Button>
        </div>
        {pasting && <div className="project-import-paste"><Textarea aria-label="Paste Project JSON" value={pastedJson} placeholder="Paste the complete Project JSON document…" disabled={checking} onChange={(event) => setPastedJson(event.target.value)} /><ActionButton busy={checking} busyLabel="Checking…" disabled={!pastedJson.trim()} onClick={() => void validate(pastedJson)}>Check document</ActionButton></div>}
        {!error && <p className="project-import-idle"><Braces /> The backend checks one canonical contract. Nothing is created yet.</p>}
      </section>}
      ...
      {stage === "destination" && validation && <section className="import-stage">
        ...
        {existing && <div className="import-choice-grid">
          <ChoiceCard selected={destinationChoice === "current"} title={`Add to ${existing.name}`} description={existing.partCount ? `Append after its ${existing.partCount} current Parts.` : "Use this empty Project."} icon={<Plus />} onClick={() => { setDestinationChoice("current"); chooseMetadata("current") }} />
          <ChoiceCard selected={destinationChoice === "new"} title="Create a new Project" description={`Keep ${existing.name} unchanged.`} icon={<FileJson2 />} onClick={() => { setDestinationChoice("new"); chooseMetadata("json") }} />
        </div>}
        ...
        <div className="import-metadata-fields">
          <label><span>Project title</span><Input value={title} maxLength={160} onChange={(event) => { setMetadataChoice("custom"); setTitle(event.target.value) }} /></label>
          <label><span>Description <small>optional</small></span><Textarea value={description} maxLength={2000} onChange={(event) => { setMetadataChoice("custom"); setDescription(event.target.value) }} placeholder="What is this Project for?" /></label>
        </div>
        {destinationChoice === "current" && existing?.partCount ? <div className="import-attention"><AlertCircle /><span><b>This Project already has {existing.partCount} Parts.</b>The imported Parts will be appended in document order. Nothing existing is replaced.</span></div> : null}
      </section>}
      ...
      {stage === "voices" && validation && <section className="import-stage import-voices-stage">
        ...
        <div className="project-import-roles">{validation.summary.roles.map((role) => {
          const identity = identities.find((item) => item.identityId === roleIdentities[role.name])
          ...
          return <article className="project-import-role" key={role.name}>
      ...
      {stage === "review" && validation && <section className="import-stage import-review-stage">
        <header><span className="eyebrow">Preflight</span><h2>Review the Project before preparing it</h2><p>This creates editable Draft truth. Audio generation remains a separate operator action.</p></header>
        <dl className="import-preflight">
          <div><dt>Project</dt><dd>{title}</dd></div><div><dt>Destination</dt><dd>{destinationChoice === "current" ? existing?.name : "Current Workspace"}</dd></div>
      ...
      {error && <div className="project-import-error" role="alert"><AlertCircle /><span><b>Import not ready</b>{error}</span></div>}
    </div>
    <DialogFooter className="project-import-footer">
      <span>{validation ? `${validation.summary.items} Parts · about ${formatEstimatedDuration(validation.summary.estimated_duration_ms)}${estimatedTextToolCost ? ` · text tools about ${formatMoney(estimatedTextToolCost)}` : ""}` : "No Project is changed until the final confirmation."}</span>
      ...
      {stage === "review" && <ActionButton busy={submitting} busyLabel="Starting…" disabled={!rolesReady || !title.trim()} onClick={() => void execute()}>{destinationChoice === "current" ? `Import and prepare ${validation?.summary.items || 0} Parts` : `Create Project and import ${validation?.summary.items || 0} Parts`}</ActionButton>}
    </DialogFooter>
  </div>
}
