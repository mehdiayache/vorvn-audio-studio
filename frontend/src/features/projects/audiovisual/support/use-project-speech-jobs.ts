 ...
 * This deliberately owns no task/card state: the Project document remains
 * the source of Part identity and the Job observer only projects execution.
 */
export function useProjectSpeechJobs(parts: ProjectPart[], refresh: () => Promise<void>) {
  ...
  ].filter(Boolean) as Array<{ job: DurableJob<unknown>; kind: "speech" | "caption"; part: ProjectPart }>), [parts])
  ...
  // The status key prevents a refreshed Project document from rebuilding
