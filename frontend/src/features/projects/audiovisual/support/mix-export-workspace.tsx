...
import type { DurableJob, Project, ProjectPart, SoundScene, VisualScene } from "@/types/domain"

import "@/features/projects/audiovisual/support/mix-export-workspace.css"

type ExportFormat = "mp3" | "mp4"
type ExportResult = { url?: string; name?: string; error?: string }
export type MixReadinessIssue = { part: ProjectPart; number: number; title: string; detail: string; blocking: boolean }

export function productionMixReadiness(project: Project) {
  const sequence = project.parts.filter((part) => part.kind !== "stitch" && part.enabled !== false)
...
export function MixExportWorkspace({ project, soundScene, visualScene, exportJob, onExport, onLocatePart, onOpenHealth, exporting, exportingFormat }: {
  project: Project
  ...
  const readiness = productionMixReadiness(project)
  ...
  return <section className="mix-export-workspace">
    ...
    {(blocking.length > 0 || review.length > 0) && <section className="mix-section mix-issues">
      <header><div><span className="eyebrow">Export checks</span><h3>Exact Parts requiring attention</h3></div><Button variant="ghost" size="sm" onClick={onOpenHealth}>Open Project health</Button></header>
