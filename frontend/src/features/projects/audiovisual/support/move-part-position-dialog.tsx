...
export function MovePartPositionDialog({ part, count, onClose, onMove }: {
  ...
  count: number
  onClose: () => void
  onMove: (part: ProjectPart, position: number) => Promise<void>
}) {
  ...
  return <Dialog open={Boolean(part)} onOpenChange={(open) => { if (!open && !busy) onClose() }}>
    <DialogContent>
      <DialogHeader><DialogTitle>Move Part to position</DialogTitle><DialogDescription>Choose its new place in this Project. Other Parts shift automatically.</DialogDescription></DialogHeader>
