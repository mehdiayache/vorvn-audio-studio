...
import { visualFileDetails, visualFileFacts, visualFileName, visualFilePosterUrl, visualFileUrl } from "@/features/projects/audiovisual/visuals/visuals-files"
...
export function VisualClipInspector({ clipRef, track, clip, file, session, saving, audioSaving = saving, hasEmbeddedAudio = false, audioMuted = false, audioGain = 1, onAudioMixChange, onAudioMixCommit }: {
  ...
  const details = file ? visualFileDetails(file) : { origin: [], technical: [], library: [] }
  const facts = file ? visualFileFacts(file) : null
  ...
  return <div className="visual-clip-inspector">
    <div className="visual-inspector-identity">
      <span className="visual-inspector-thumb">{file?.filename ? <img src={file.media_type === "video" ? visualFilePosterUrl(file) : visualFileUrl(file)} alt="" /> : file?.media_type === "video" ? <Film /> : <ImageIcon />}</span>
    ...
    <OperatorInspectorSection icon={Scan} title="Frame" help="Fit and Fill establish a starting frame. Position, scale, rotation and flips then create custom framing." actions={<OperatorTooltip label="Reset frame" detail="Restores Fill, centered placement, 100% scale, no rotation and no flip. Opacity and audio stay unchanged."><Button variant="ghost" size="sm" disabled={transformDisabled} onClick={() => void session.resetClipTransform(clipRef)}><RotateCcw /> Reset</Button></OperatorTooltip>} className="visual-inspector-transform">
      <div className="visual-frame-actions" role="group" aria-label="Quick framing actions">
        ...
        <OperatorTooltip label="Fill Project frame" detail="Recenters and removes rotation, then enlarges the source until the frame is covered. Outer edges may be cropped."><Button variant="outline" size="sm" disabled={transformDisabled} onClick={() => void session.frameClip(clipRef, "cover")}><Maximize /> Fill</Button></OperatorTooltip>
      </div>
