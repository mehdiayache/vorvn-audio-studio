...
export function TimelineContextBar({ audioSession, visualSession, selectedAudioRefs, selectedPart, context, selectedVisualRefs, selectedVisualTrack, selectedVisualFile, selectedVideoAudio, playhead, saving, visualSaving, canSplitAudio, canSplitVisual, canCrossfade, error, visualError, onFollowPlayhead, onRemoveAudio, onRemoveVisual }: {
  ...
  selectedVisualFile?: WorkspaceFile
  ...
  return <footer className="timeline-selection-bar" aria-label="Selection actions">
    {selectedVisualRef && selectedVisualTrack && selectedVisualClip && visualSession ? <VisualContextToolbar
      ...
      file={selectedVisualFile}
      saving={visualSaving || saving}
      canSplit={selectedVisualRefs.length === 1 && canSplitVisual}
      hasAudio={selectedVisualRefs.length === 1 && videoHasEmbeddedAudio(selectedVisualFile)}
      ...
      onSplit={() => void visualSession.splitVideo(selectedVisualRef, playhead * 1000, selectedVisualFile)}
