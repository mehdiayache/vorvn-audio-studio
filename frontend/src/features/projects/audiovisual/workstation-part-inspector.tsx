...
function recordedWording(part: ProjectPart) {
...
function availableTabs(part: ProjectPart): InspectorTab[] {
  if (part.kind === "silence") return ["timing", "details"]
  if (part.kind === "file") return ["recording", "details"]
...
export function WorkstationPartInspector({ projectId, part, directory, playingKey, playerPlaying, onChanged, onPlay, onEdit, onDuplicate, onDelete, onOpenCaptions, onReplaceFile }: {
  projectId: number
  part: ProjectPart
  ...
  onDuplicate: (part: ProjectPart) => void
  onDelete: (part: ProjectPart) => void
  onOpenCaptions: (part: ProjectPart) => void
  onReplaceFile: (part: ProjectPart) => void
}) {
  ...
  const playable = part.kind === "file" ? Boolean(part.filename && !part.missing) : facts.playable
  const source: PlayerSource = { key: `part:${part.id}`, url: audioUrl(part.filename || ""), title: part.kind === "file" ? part.title || "Linked audio" : role || facts.selectedVoiceName, subtitle: `Part ${formatPartNumber(part.position ?? 0)} · ${facts.durationLabel}`, kind: part.kind === "file" ? "file" : "clip" }
  ...
  return <div className="ws-part-inspector">
    <section className="ws-inspector-identity">
      <div className="ws-inspector-portrait">{part.kind === "silence" ? <Clock3 /> : part.kind === "file" ? <FileAudio /> : <VoiceIdentity voice={part.catalogue_voice_id || part.voice || part.voice_name} identityId={part.voice_identity_id} directory={directory} gender={facts.voice.gender} showCopy={false} />}</div>
      <div className="ws-inspector-title-copy">
        <span>Part {formatPartNumber(part.position ?? 0)}{role ? ` · ${role}` : ""}</span>
        <h3>{part.kind === "silence" ? "Intentional pause" : part.kind === "file" ? part.title || "Linked audio" : facts.selectedVoiceName}</h3>
        <p>{part.kind === "silence" ? "Editorial timing" : part.kind === "file" ? "Workspace audio file" : facts.methodLine}</p>
      </div>
      {playable && <div className="ws-inspector-media-actions">
        ...
        <AudioDownloadButton url={audioUrl(part.filename || "")} label={`Part ${formatPartNumber(part.position ?? 0)} audio`} detail="Downloads this Part's original audio file without exporting the whole Project." />
      </div>}
    </section>
    ...
    <Tabs value={tab} onValueChange={(value) => setTab(value as InspectorTab)} className="ws-inspector-tabs">
      ...
      <div className="ws-inspector-tab-scroll">
        ...
        <TabsContent value="timing" className="ws-inspector-tab">
          <div className="ws-inspector-empty"><Clock3 /><h3>{partDurationMs(part) / 1000} seconds</h3><p>This pause occupies real Project time and has no provider cost.</p></div>
        </TabsContent>
        ...
        <TabsContent value="details" className="ws-inspector-tab">
          {part.kind === "file" ? <dl className="ws-inspector-details">
            ...
            <Fact label="File ID" value={part.file_id || part.file_of} mono />
            <Fact label="File version" value={part.file_version_id} mono />
          ...
          <details className="ws-inspector-evidence"><summary>Technical evidence</summary><dl><Fact label="Part ID" value={part.public_id || part.id} mono />{part.kind === "file" ? <><Fact label="File ID" value={part.file_id || part.file_of} mono /><Fact label="File version" value={part.file_version_id} mono /></> : <><Fact label="Recording ID" value={part.clip_public_id} mono /><Fact label="Binding ID" value={part.binding_id} mono /><Fact label="Provider attempt" value={part.provider_attempt_id} mono /></>}</dl></details>
        </TabsContent>
      </div>
    </Tabs>
