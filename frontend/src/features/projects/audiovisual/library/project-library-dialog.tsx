import { Captions, FileText, Images, Plus, Search, SlidersHorizontal } from "lucide-react"
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="project-library-dialog">
      ...
      <div className="project-library-dialog-toolbar">
        ...
        <Select value={mediaType} onValueChange={(value) => setMediaType(value as CreatorLibraryKind)}><SelectTrigger aria-label="File type"><SelectValue /></SelectTrigger><SelectContent><SelectGroup><SelectItem value="all">All Files</SelectItem><SelectItem value="image">Images</SelectItem><SelectItem value="video">Videos</SelectItem><SelectItem value="speech">Speech</SelectItem><SelectItem value="music">Music</SelectItem><SelectItem value="sfx">SFX</SelectItem><SelectItem value="subtitle">Subtitles</SelectItem></SelectGroup></SelectContent></Select>
      ...
      {visible.length
        ? <div className="project-library-dialog-grid">{visible.map((file) => {
          ...
          if (kind === "speech" || kind === "music" || kind === "sfx") return <article className="project-library-dialog-file" key={file.id}><AudioFileCard file={file} used={Boolean(usageCounts.get(file.id))} /><Button disabled={pendingId === file.id} size="sm" onClick={() => onAdd(file)}><Plus />{pendingId === file.id ? "Adding…" : addLabel}</Button></article>
