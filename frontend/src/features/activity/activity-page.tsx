import { AlertTriangle, AudioLines, Clock3, RefreshCw, Video } from "lucide-react"
...
export function ActivityPage() {
  ...
  return <main className="activity-page">
    ...
    <section className="activity-totals" aria-label="Spend totals">
      <article><small>Recorded today</small><b>{formatMoney(data?.today || 0)}</b><div className="activity-media-spend"><span title="Speech, audio generation, transcription and audio processing"><AudioLines /> Audio {formatMoney(data?.today_media?.audio || 0)}</span><span title="Director image and video generation"><Video /> Visual {formatMoney(data?.today_media?.video || 0)}</span></div></article>
      <article><small>Recorded this month</small><b>{formatMoney(data?.month || 0)}</b><div className="activity-media-spend"><span title="Speech, audio generation, transcription and audio processing"><AudioLines /> Audio {formatMoney(data?.month_media?.audio || 0)}</span><span title="Director image and video generation"><Video /> Visual {formatMoney(data?.month_media?.video || 0)}</span></div></article>
      <article><small>Recorded all time</small><b>{formatMoney(data?.total || 0)}</b><div className="activity-media-spend"><span title="Speech, audio generation, transcription and audio processing"><AudioLines /> Audio {formatMoney(data?.total_media?.audio || 0)}</span><span title="Director image and video generation"><Video /> Visual {formatMoney(data?.total_media?.video || 0)}</span></div></article>
