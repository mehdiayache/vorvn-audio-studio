...
vi.mock("@/lib/api", () => ({ studioApi: { subtitleLayout: vi.fn() } }))
...
describe("caption presentation", () => {
  beforeEach(() => {
    vi.mocked(studioApi.subtitleLayout).mockImplementation(async (_id, profile) => ({
  ...
  it("builds every free player timeline from one saved transcript", async () => {
    ...
    expect(studioApi.subtitleLayout).toHaveBeenCalledTimes(3)
