...
import type { AudioFileCategory, CatalogKeepResult, CatalogSound, DurableJob, GeneratePayload, GenerateResult, GeneratedKeepResult, PartEditorialUpdate, PlayerSource, Production, ProductionPart, SoundScene, SoundSceneDocument, WorkspaceFile } from "@/types/domain"
...
export function useProductionActions({ production, soundScene, player, refresh, refreshFiles, preparePlayerSource, feedbackMode = "toast" }: {
  ...
  const preview = useCallback(async () => {
    setPreviewing(true)
    try {
      const result = await originsApi.preview(production.id)
  ...
  const exportProduction = useCallback(async (format: ExportFormat, allowIncomplete = false) => {
    ...
    try {
      const job = await originsApi.enqueueRender(production.id, "export", allowIncomplete, format)
  ...
  const updatePartEditorial = useCallback(async (part: ProductionPart, values: PartEditorialUpdate) => {
    try {
      await originsApi.savePartEditorial(production.id, part.id, values)
      await refresh()
    } catch (error) {
  ...
  const movePart = useCallback((part: ProductionPart, direction: -1 | 1) => {
    ...
    void mutate(`part:${part.id}:move`, () => originsApi.reorder(production.id, order), `Part moved ${direction < 0 ? "up" : "down"}`)
  ...
  const movePartToPosition = useCallback(async (part: ProductionPart, requestedPosition: number) => {
    ...
    await mutate(`part:${part.id}:move`, () => originsApi.reorder(production.id, order), `Part moved to position ${to + 1}`)
  }, [mutate, production.id, production.parts])
  ...
  const movePartsToPosition = useCallback(async (ids: number[], requestedPosition: number) => {
    ...
    await mutate("parts:move", () => originsApi.reorder(production.id, next), `${selectedCount} Part${selectedCount === 1 ? "" : "s"} moved to position ${position}`)
  }, [mutate, production.id, production.parts])
  ...
  const updateSoundScene = useCallback((document: SoundSceneDocument, expectedRevision: number, mutationKind: "operator" | "derived_visual_audio" = "operator") => {
    if (mutationKind === "derived_visual_audio") {
      return originsApi.updateSoundScene(production.id, expectedRevision, document, mutationKind).then(async (scene) => {
        invalidatePreview()
        await refresh()
        return scene
      })
    ...
    return mutate(
      `sound-scene:save:${expectedRevision}`, () => originsApi.updateSoundScene(production.id, expectedRevision, document, mutationKind),
      "Timeline saved",
    )
  }, [invalidatePreview, mutate, production.id, refresh])
  const undoSoundScene = useCallback(() => mutate(
    "sound-scene:undo", () => originsApi.undoSoundScene(production.id), "Timeline undone",
  ), [mutate, production.id])
  const redoSoundScene = useCallback(() => mutate(
    "sound-scene:redo", () => originsApi.redoSoundScene(production.id), "Timeline redone",
  ), [mutate, production.id])
  const duplicatePart = useCallback((part: ProductionPart) => mutate(`part:${part.id}:duplicate`, () => originsApi.duplicatePart(production.id, part.id), "Part duplicated"), [mutate, production.id])
  const deletePart = useCallback((part: ProductionPart) => mutate(`part:${part.id}:delete`, () => originsApi.deletePart(production.id, part.id), "Part permanently deleted", true), [mutate, production.id])
  const editSilence = useCallback((part: ProductionPart, seconds: number) => mutate(`part:${part.id}:silence`, () => originsApi.editSilence(production.id, part.id, seconds), "Silence updated"), [mutate, production.id])
  const setPartEnabled = useCallback((part: ProductionPart, enabled: boolean) => mutate(`part:${part.id}:enabled`, () => originsApi.setPartEnabled(production.id, part.id, enabled), enabled ? "Part included" : "Part excluded"), [mutate, production.id])
  const deleteParts = useCallback((ids: number[]) => mutate("parts:delete", () => originsApi.deleteParts(production.id, ids), "Parts permanently deleted", true), [mutate, production.id])
  const saveDraft = useCallback(async (payload: Omit<GeneratePayload, "confirmed">): Promise<void> => {
    await mutate("production:add-speech", () => originsApi.saveDraft(payload), "Draft added")
  }, [mutate])
  const addSilence = useCallback((seconds: number, beforePartId: string | null) => mutate("production:add-silence", () => originsApi.addSilence(production.id, seconds, beforePartId), "Silence added"), [mutate, production.id])
  const insertFile = useCallback((file: WorkspaceFile, beforePartId: string | null) => mutate("production:add-file", () => originsApi.insertFile(production.id, file.id, beforePartId), "Library audio inserted"), [mutate, production.id])
  const replaceFile = useCallback((part: ProductionPart, file: WorkspaceFile) => mutate(`part:${part.id}:replace`, () => originsApi.replaceFile(production.id, part.id, file.id), "Workspace audio replaced"), [mutate, production.id])
  const moveParts = useCallback((ids: number[], targetId: number, targetName: string) => mutate("parts:move-production", () => originsApi.moveParts(production.id, ids, targetId), `Moved to ${targetName}`, true), [mutate, production.id])
  const uploadFile = useCallback(async (_category: string, details: {
    ...
    return mutationActions.run("file:upload", async () => {
      const uploaded = await originsApi.uploadAudiovisualProductionFile(
        production.id, details.file, details)
      await refreshFiles()
      toast.success(`${details.name} added to the File Library`)
      return uploaded as WorkspaceFile
    })
  }, [mutationActions.run, production.workspace_id, refreshFiles])
  ...
  const keepFreesound = useCallback(async (details: {
    ...
    `file:keep:freesound:${details.result.external_id}`, async () => {
      ...
      const kept = await originsApi.keepFreesoundInProduction(
  ...
  const keepGeneratedAudio = useCallback(async (details: {
    ...
    `file:keep:generated:${details.candidateId}`, async () => {
      ...
      const kept = await originsApi.keepGeneratedAudioInProduction(
        details.candidateId, production.id, payload)
      await refreshFiles()
      toast.success(kept.duplicate ? `${details.name} is already in the File Library` : `${details.name} saved to the File Library`)
      return kept
    }), [mutationActions.run, production.workspace_id, refreshFiles])
