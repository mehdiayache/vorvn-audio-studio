...
describe("usePartDetailData", () => {
  ...
  it("recovers exact cost confirmation and confirms that same durable Job", async () => {
    ...
    const recoverWrapper = ({ children }: { children: React.ReactNode }) => <MemoryRouter initialEntries={["/origins/productions/audiovisual/production-7?part-caption-job=caption-confirm"]}>{children}</MemoryRouter>
    const activePart = part(1)
    const onChanged = vi.fn().mockResolvedValue(undefined)
