...
export function useCreatorText(
  ...
  const [view, setView] = useState<TextView>(creatorTextFromPart(part).active)
  ...
  useEffect(() => {
    const restored = creatorTextFromPart(part)
  ...
  function currentText(): CreatorText {
  ...
  function restore(next: CreatorText, nextDensity: TagDensity = "normal", nextSpokenProfile: SpokenProfile = "spoken_1") {
