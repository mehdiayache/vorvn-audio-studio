...
export function PendingPartCard({ task, index, directory, onRetry, onConfirm, onDismiss }: {
  task: RenderTask
  index: number
  directory: VoiceDirectory
  onRetry: (task: RenderTask) => void
  onConfirm: (task: RenderTask) => void
  ...
  const confirmation = review && task.needsConfirmation && !task.requiresReview
  const route = resolveRequestVoice(task.payload, directory)
  return <div className="sequence-row pending-row">
    ...
    <article className={`sequence-card pending-card ${failed || review ? "failed" : "generating"}`} aria-label={failed ? "Speech generation failed" : review ? "Speech generation needs review" : "Speech is generating"} role="status" aria-live="polite">
      ...
      <div className="pending-card-body">
        <div className="sequence-card-heading"><VoiceIdentity voice={task.voice} identityId={task.payload.voice_identity_id} directory={directory} compact /><span className="pending-card-status"><b>{failed ? "Generation failed" : confirmation ? "Cost confirmation needed" : review ? "Review required" : "Generating audio…"}</b><small>{failed ? task.error || "The provider did not finish this Part." : confirmation ? `No provider request was sent. Confirm the estimated $${Number(task.estimate || 0).toFixed(4)} to continue.` : review ? task.detail || "Open Activity before trying this paid operation again." : `${elapsed(task.startedAt)} · ${task.detail || "safe to continue working"}`}</small></span></div>
        <div className="pending-route"><SpeechRouteLabel route={{ engine: route.engine, model: route.model, language: task.payload.language }} includeLanguage config={directory.config} /></div>
      ...
      {(failed || review) && <div className="pending-card-actions">{failed && <Button variant="outline" size="sm" onClick={() => onRetry(task)}><RefreshCw /> Retry</Button>}{confirmation && <Button size="sm" onClick={() => onConfirm(task)}>Confirm ${Number(task.estimate || 0).toFixed(4)}</Button>}<Button variant="ghost" size="icon" aria-label="Dismiss generation" onClick={() => onDismiss(task.jobId)}><X /></Button></div>}
    </article>
  </div>
}
