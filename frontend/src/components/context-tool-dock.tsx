...
import type { HierarchyNode, ProductionCastRole, ProductionPart, VoiceDirectory } from "@/types/domain"
...
const toolMeta = [
  ...
  { key: "voices", label: "Cast", description: "See Production roles and their assigned Voice Identities.", icon: Mic2 },
...
export function ContextToolDock({ panel, onPanel, nodes, activeKey, parts, directory, cast, productionPlaying, onLocate, onOpenTool, onPreview, onRelease }: {
  panel: ContextPanel
  ...
  function panelBody(key: Exclude<ContextPanel, null>) {
    ...
    if (key === "voices") return <div className="context-list voice-context-list">{cast.length ? cast.map((role) => <div className="context-cast-role" key={role.id}><span className="context-cast-color" style={{ backgroundColor: role.color || undefined }} /><div><b>{role.name}</b><small>{role.persona_name ? `Character · ${role.persona_name}` : "Ad-hoc Production role"}</small>{role.voice_identity_id ? <VoiceIdentity voice="" identityId={role.voice_identity_id} directory={directory} compact /> : <small>{role.catalogue_voice_id ? "Alibaba catalogue voice" : "No voice assigned"}</small>}</div><small>Assignment {role.assignment_revision}</small></div>) : <div className="context-empty"><p>No Cast Role yet. Existing Takes remain historical; speech can still use a Voice Identity directly.</p><Button variant="outline" onClick={() => act(() => onOpenTool("speech"))}>Add speech</Button></div>}</div>
