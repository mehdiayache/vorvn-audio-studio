        ...
        {part && <>
          ...
          <Tabs defaultValue="summary" className="detail-tabs">
            ...
            <ScrollArea className="detail-scroll">
              ...
              {isRecorded && <TabsContent value="takes"><div className="detail-body"><section><div className="detail-section-head"><h3>Current take</h3><Button onClick={() => onNewTake(part)}><Plus /> Create another take</Button></div><RecordingTakeCard take={{ id: String(part.id), status: "current", voice: part.voice, voiceIdentityId: part.voice_identity_id, durationMs: Number(part.duration_ms || 0), cost: part.cost, language: part.language, method: speechEngineLabel(part.engine), audioUrl: part.filename ? audioUrl(part.filename) : null, message: part.fidelity && part.fidelity.status !== "pass" ? part.fidelity.message : undefined }} directory={directory} active={currentPlaying} onPlay={part.filename ? () => onPlay({ key: currentKey, url: audioUrl(part.filename!), title: "Current take", subtitle: resolveVoice(part.voice, directory, part.voice_identity_id).name, kind: "part" }) : undefined} /></section><section><h3>Earlier takes</h3>{loading ? <p className="detail-loading"><LoaderCircle className="spin" /> Loading takes…</p> : takes.length ? takes.map((take) => { const key = `take:${take.id}`; const active = playerPlaying && playingKey === key; return <RecordingTakeCard key={take.id} take={{ id: String(take.id), status: take.fidelity && take.fidelity.status !== "pass" ? "warning" : "ready", voice: take.voice, voiceIdentityId: take.voice_identity_id, createdAt: take.when, durationMs: Number(take.duration_ms || 0), cost: take.cost, language: take.language, method: speechModelLabel(take.model), audioUrl: audioUrl(take.filename), message: take.fidelity && take.fidelity.status !== "pass" ? take.fidelity.message : undefined }} directory={directory} active={active} onPlay={() => onPlay({ key, url: audioUrl(take.filename), title: "Earlier take", subtitle: resolveVoice(take.voice, directory, take.voice_identity_id).name, kind: "part" })} onSecondaryAction={() => void promote(take)} secondaryLabel="Use this take" /> }) : <p className="detail-empty">No earlier takes. Create another take to compare performances without losing this one.</p>}</section></div></TabsContent>}
