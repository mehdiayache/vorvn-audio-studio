...
export function AssetPartCard({ part, index, count, playing, onReplace, actions }: {
  ...
  return <article id={`part-${part.id}`} className={cn("sequence-card asset-part-card", playing && "playing", part.missing && "missing")}>
    <button className="sequence-card-open" onClick={() => actions.openPart(part)} aria-label={`Open details for ${title}`}>
      <div className="sequence-card-heading"><span className="sequence-asset-identity"><span className="sequence-asset-icon"><FileAudio /></span><span><b>{title}</b><small>{assetType}</small></span></span><span className="sequence-card-status"><b>{duration ? formatDuration(duration) : "Unknown duration"}</b></span></div>
