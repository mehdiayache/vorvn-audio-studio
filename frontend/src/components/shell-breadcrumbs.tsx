...
export function ShellBreadcrumbs({ trail = [], current, className = "" }: {
  ...
  return <nav className={`shell-breadcrumbs ${className}`} aria-label="Location">
    ...
    {trail.map((item) => <span className="shell-breadcrumb-item" key={`${item.type}:${item.id}`}><ChevronRight /><a href={resourceHref(item.type, item.id)}><CrumbIdentity type={item.type} name={item.name} icon={item.icon} /><span>{item.name}</span></a></span>)}
