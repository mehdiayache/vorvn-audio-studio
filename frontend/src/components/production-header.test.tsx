...
describe("ProductionHeader", () => {
  it("keeps the exact Health count in the Focus Bar", () => {
    ...
    render(<MemoryRouter><ProductionHeader production={production} duration={0} mixExportOpen={false} productionPlaying={false} issueCount={43} onExplorer={vi.fn()} onCommands={vi.fn()} onHealth={vi.fn()} onPreview={vi.fn()} onAdd={vi.fn()} onRelease={vi.fn()} /></MemoryRouter>)
