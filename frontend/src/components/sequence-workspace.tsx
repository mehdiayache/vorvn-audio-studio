...
export function SequenceWorkspace({ parts, liveJobs, visiblePartIds, filtersActive = false, activePartId, playingKey, playerPlaying, previewPlayingPartId, directory, onClearFilters, onInsert, onRetryJob, onConfirmJob, onReplaceAsset, onOpenCaptions, actions }: {
        ...
        <div className={cn("sequence-row", part.kind === "silence" && "silence", !["silence", "asset"].includes(part.kind) && "speech", part.enabled === false && "is-disabled", activePartId === part.id && "is-stage-active", playerPlaying && previewPlayingPartId === part.id && "is-preview-playing")} data-stage-active={activePartId === part.id || undefined} data-preview-playing={playerPlaying && previewPlayingPartId === part.id || undefined} role="listitem" aria-posinset={index + 1} aria-setsize={sourceParts.length}>
          ...
          {part.kind === "silence"
            ...
            : part.kind === "asset"
              ...
              : <SpeechPartCard part={part} job={part.speech_job ? (liveJobs[part.speech_job.id] as DurableJob<GenerateResult> | undefined) || part.speech_job : null} captionJob={part.caption_job ? (liveJobs[part.caption_job.id] as typeof part.caption_job | undefined) || part.caption_job : null} index={index} playing={playerPlaying && playingKey === `part:${part.id}`} playingPreview={playerPlaying && previewPlayingPartId === part.id} directory={directory} onRetryJob={() => { const job = part.speech_job ? (liveJobs[part.speech_job.id] as DurableJob<GenerateResult> | undefined) || part.speech_job : null; if (job) onRetryJob(part, job) }} onConfirmJob={() => { const job = part.speech_job ? (liveJobs[part.speech_job.id] as DurableJob<GenerateResult> | undefined) || part.speech_job : null; if (job) onConfirmJob(part, job) }} onOpenCaptions={() => onOpenCaptions(part)} actions={actions} />}
        </div>
