...
export function PartCaptionPanel({ captions, transcript, languages, sourceLanguage, loading, busy, confirmation, onSelect, onCreate, onTranslate, onConfirm, onCancel }: {
  ...
  return <div className="detail-body caption-detail">
    ...
    <Dialog open={Boolean(confirmation)} onOpenChange={(open) => { if (!open) onCancel() }}><DialogContent><DialogHeader><DialogTitle>{confirmation?.kind === "translate" ? `Translate into ${confirmation.target}?` : "Create subtitles?"}</DialogTitle><DialogDescription>This Alibaba request is estimated at ${Number(confirmation?.estimate || 0).toFixed(4)}. Actual provider usage is saved after completion.</DialogDescription></DialogHeader><DialogFooter><Button variant="outline" onClick={onCancel}>Cancel</Button><Button onClick={() => void onConfirm()}>Continue</Button></DialogFooter></DialogContent></Dialog>
  </div>
}
