import { Check, ChevronDown, FileAudio, Pause, Play, RotateCcw, Sparkles, Trash2 } from "lucide-react"
...
export function GenerationWorkspace({
  ...
  return <section className="asset-view asset-generation-view">
    <main className="asset-generation-compose">
      <div className="asset-generation-heading"><div className="asset-generation-kind" aria-label="Generation type"><button type="button" className={capability === "sfx" ? "active" : ""} onClick={() => setCapability("sfx")}><b>Sound Effect</b><small>1–30 seconds</small></button><button type="button" className={capability === "music" ? "active" : ""} onClick={() => setCapability("music")}><b>Music</b><small>5–120 seconds</small></button></div><ToggleGroup type="single" variant="outline" value={promptMode} onValueChange={(value) => value && setPromptMode(value as "simple" | "expert")} aria-label="Prompting mode"><ToggleGroupItem value="simple">Simple</ToggleGroupItem><ToggleGroupItem value="expert">Expert</ToggleGroupItem></ToggleGroup></div>
    ...
    <aside className="asset-inspector asset-generation-inspector" data-state={selectedState}>
      <header><div><small>Recent variations</small><h3>{history.length ? `${history.length} in this Production` : "Nothing generated yet"}</h3></div></header>
      <div className="asset-generation-history">{history.length ? history.map((item, index) => {
        const itemCandidate = item.candidate; const active = itemCandidate && playerPlaying && playingKey === `generated-candidate:${itemCandidate.candidate_id}`; const label = String.fromCharCode(65 + index)
