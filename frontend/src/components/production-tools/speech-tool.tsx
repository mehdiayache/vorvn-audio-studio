...
export function SpeechTool({ projectId, nextPartNumber = 1, insertAt = null, part = null, config, directory, playingKey, playerPlaying, onSave, onGenerate, onPlay }: {
  ...
  return <div className="speech-composer">
    ...
    <div className="composer-stage">
      ...
      {section === "voice" && <section className="composer-section voice-capability-section"><header><div><span className="eyebrow">Voice and recording style</span><h3>Choose who speaks and how</h3></div></header><div className="composer-route-grid voice-first-grid"><label className="wide"><span>Voice</span><VoicePicker identities={identities} value={identityId} directory={directory} playingKey={playingKey} playerPlaying={playerPlaying} onPlay={onPlay} onChange={selectIdentity} />{selectedIdentity?.sourceLanguage && <small className="voice-source-note">The flag describes the source recording only. Output language is decided by the selected model.</small>}</label><label className="wide"><span>Language to speak</span><Select value={language} onValueChange={setLanguage}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{languageOptions.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select>{language === "Auto" && outputLanguage !== "Auto" && <small className="detected-language">Detected from your script: <b>{outputLanguage}</b></small>}</label></div><div className="method-heading"><b>Recording style</b><span>Official coverage belongs to the model, never to the voice's source flag.</span></div>{selectedIdentity?.routes.length ? <VoiceMethodPicker routes={selectedIdentity.routes} availableRoutes={compatibleRoutes} selectedEngine={engine} language={outputLanguage} customVoice={selectedIdentity.source === "mine"} config={config} onSelect={(nextEngine) => applyRoute(chooseIdentityRoute(compatibleRoutes.filter((route) => route.engine === nextEngine), { engine: nextEngine, model }))} /> : <div className="composer-warning capability-empty"><b>This voice has no ready recording method.</b><span>Open Voices to finish or retry its capabilities.</span></div>}{routeConflict && <div className="composer-warning route-conflict"><b>{capabilityTitle(engine, config)} cannot speak {outputLanguage} with this Alibaba voice.</b><span>Choose one of the available styles above. Your voice and script stay unchanged.</span></div>}{currentRoute && <VoiceLanguageSupport route={currentRoute} language={outputLanguage} customVoice={selectedIdentity?.source === "mine"} />}{tierOptions.length > 1 && !routeConflict && <div className="composer-route-grid quality-grid"><label><span>Quality and cost</span><Select value={model} onValueChange={(nextModel) => applyRoute(tierOptions.find((route) => route.model === nextModel))}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{tierOptions.map((route) => <SelectItem key={route.id} value={route.model}>{route.model === "plus" ? "Best quality · Plus" : route.model === "flash" ? "Faster & economical · Flash" : "Voice Clone"}</SelectItem>)}</SelectContent></Select></label></div>}{currentRoute && <p className="composer-registry-note"><b>Selected:</b> {selectedIdentity?.name} · {outputLanguage} · {capabilityTitle(currentRoute.engine, config)} · {tierLabel(currentRoute.model)}</p>}</section>}
