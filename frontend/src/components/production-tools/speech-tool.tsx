...
export function SpeechTool({ projectId, nextPartNumber = 1, insertAt = null, part = null, config, directory, playingKey, playerPlaying, onSave, onGenerate, onPlay }: {
  ...
  useEffect(() => {
    setSection("voice")
    setEngine((part?.engine as SpeechEngine) || "audio"); setModel((part?.model as SpeechModel) || "plus"); setVoice(part?.voice || "")
  ...
  useEffect(() => {
    const routes = (directory.registry?.models || []).filter((route) => route.engine === engine)
    if (routes.length && !routes.some((route) => route.tier === model)) {
      setModel(routes[0]!.tier)
    }
  }, [directory.registry, engine, model])

  useEffect(() => {
    if (!config) return
    const currentIdentity = directory.registry?.bindings.find((item) => item.provider_voice_id === voice)?.identity_id
    const next = voices.compatible.find((item) => item.identityId === currentIdentity)?.id || voices.compatible[0]?.id || ""
  ...
  const nav: Array<{ key: ComposerSection; label: string; detail: string; icon: typeof AudioLines }> = [
    ...
    { key: "delivery", label: "Delivery", detail: engine === "omni" ? speechMode === "exact" ? "Script priority" : "Directed performance" : engine === "qwen_tts" ? "Exact narration" : "Exact TTS + tags", icon: WandSparkles },
    { key: "output", label: "Output", detail: engine === "audio" ? `${format.toUpperCase()} · ${rate.toFixed(2)}×` : `${format.toUpperCase()} · model controlled`, icon: Gauge },
  ]
  ...
  return <div className="speech-composer">
    ...
    <div className="composer-stage">
      ...
      {section === "voice" && <section className="composer-section"><header><div><span className="eyebrow">Voice and capability</span><h3>Who should speak?</h3></div></header><div className="composer-route-grid"><label><span>Speech engine</span><Select value={engine} onValueChange={(value) => setEngine(value as SpeechEngine)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{engineOptions.map((item) => { const routes = routeModels.filter((route) => route.engine === item); return <SelectItem key={item} value={item}>{routes[0]?.label || item}</SelectItem> })}</SelectContent></Select></label><label><span>Capability</span><Select value={model} onValueChange={(value) => setModel(value as SpeechModel)}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{tierOptions.map((route) => <SelectItem key={route.model_id} value={route.tier}>{tierLabel(route.tier)} · {route.total_count} voices</SelectItem>)}</SelectContent></Select></label><label className="wide"><span>Voice</span><VoicePicker choices={voices.choices} summary={voices.summary} value={voice} directory={directory} engineLabel={voices.summary?.label || engine} modelLabel={tierLabel(model)} playingKey={playingKey} playerPlaying={playerPlaying} onPlay={onPlay} onChange={(choice) => { if (choice.compatible) { setVoice(choice.id); applyVoiceLanguage(choice) } else setPendingVoiceRoute(choice) }} /></label><label><span>Output language</span><Select value={language} onValueChange={setLanguage}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{(config?.languages || []).filter((item) => item === "Auto" || voices.compatible.some((choice) => choice.languages.some((language) => language.toLocaleLowerCase() === item.toLocaleLowerCase()))).map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select></label></div>{voices.summary && <p className="composer-registry-note">{voices.summary.system_count} Alibaba voices · {voices.summary.custom_count} of your voices · catalogue verified {directory.registry?.source.verified_at}</p>}</section>}
      {section === "delivery" && <section className="composer-section"><header><div><span className="eyebrow">Performance</span><h3>How should it sound?</h3></div></header>{engine === "omni" && <div className="delivery-mode"><div><span>Mode</span><Button variant={speechMode === "exact" ? "secondary" : "outline"} onClick={() => setSpeechMode("exact")}>Script priority</Button><Button variant={speechMode === "directed" ? "secondary" : "outline"} onClick={() => setSpeechMode("directed")}>Directed performance</Button></div><p>{speechMode === "exact" ? "Requests a verbatim response and checks Alibaba's returned transcript. Omni can still deviate." : "Uses one overall direction and checks the returned transcript. Per-sentence tags are unavailable."}</p></div>}{engine === "qwen_tts" ? <p className="composer-engine-note">Qwen3 TTS Voice Clone is the faithful narration path. It reads the prepared script without inline tags or performance instructions.</p> : <label><span>{engine === "omni" ? "Overall performance direction" : "Voice direction"}</span><Input value={instruction} disabled={engine === "omni" && speechMode === "exact"} maxLength={config?.instruction_max || 100} onChange={(event) => setInstruction(event.target.value)} placeholder="Describe the performance in natural language" />{engine === "omni" && speechMode === "exact" && <small>Switch to Directed performance to apply a natural-language direction.</small>}</label>}{performancePresets.length > 0 && <div className="performance-presets"><span>Presets</span><div>{performancePresets.map((preset) => <Button key={preset.id} type="button" variant={instruction === preset.instruction ? "secondary" : "outline"} onClick={() => { setInstruction(preset.instruction); if (engine === "omni") setSpeechMode("directed") }}><b>{preset.name}</b><small>{preset.instruction}</small></Button>)}</div></div>}</section>}
    ...
    <Dialog open={Boolean(pendingVoiceRoute)} onOpenChange={(open) => { if (!open) setPendingVoiceRoute(null) }}><DialogContent><DialogHeader><DialogTitle>Use this voice capability?</DialogTitle><DialogDescription>{pendingVoiceRoute ? `${resolveVoice(pendingVoiceRoute.id, directory).name} is ready through ${engineLabel(pendingVoiceRoute.engine)} · ${tierLabel(pendingVoiceRoute.model)}. The Composer will select that compatible capability without changing the voice identity.` : ""}</DialogDescription></DialogHeader><DialogFooter><Button variant="outline" onClick={() => setPendingVoiceRoute(null)}>Keep current setup</Button><Button onClick={() => { if (!pendingVoiceRoute) return; setEngine(pendingVoiceRoute.engine); setModel(pendingVoiceRoute.model); setVoice(pendingVoiceRoute.id); applyVoiceLanguage(pendingVoiceRoute); setPendingVoiceRoute(null) }}>Use this capability</Button></DialogFooter></DialogContent></Dialog>
  </div>
}
