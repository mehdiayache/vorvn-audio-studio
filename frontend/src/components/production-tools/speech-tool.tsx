...
export function SpeechTool({ projectId, sessionId, nextPartNumber = 1, insertAt = null, insertBeforePartId = null, part = null, config, directory, cast = [], playingKey, playerPlaying, onSave, onUpdateEditorial, onGenerate, onPlay }: {
  ...
  const deliveryMode = resolvedDeliveryMode(capabilityControls, speechMode)
  ...
  const estimateRate = currentRoute?.estimateRatePerMillionCharacters || 0
  ...
  const nav: Array<{ key: ComposerSection; label: string; detail: string; icon: typeof AudioLines }> = [
    ...
    { key: "output", label: "Output", detail: currentRoute && model ? `${currentRoute.modelId} · ${tierLabel(model)}` : "No route selected", icon: Gauge },
  ]
  ...
  return <div className="speech-composer">
    ...
    <div className="composer-stage">
      {section === "script" && <section className="composer-section script-section"><header><div><span className="eyebrow">Words and text states</span><h3>What should be said?</h3></div><div className="speech-states">{(["raw", "shaped", ...(capabilityControls.deliveryTags ? ["tagged" as const] : [])] as TextView[]).map((state) => <Button key={state} variant="ghost" size="sm" className={textSession.view === state ? "active" : ""} disabled={state !== "raw" && !textSession.states[state]} onClick={() => textSession.select(state)}>{state === "raw" ? "Raw" : state === "shaped" ? "Spoken" : "Tagged"}</Button>)}</div></header><div className="script-language-setting"><label><span>Language to speak</span><Select value={language} onValueChange={setLanguage}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{languageOptions.map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select><small>Auto leaves language detection to the selected model. Changing this never changes the voice or capability.</small></label>{currentRoute && <VoiceLanguageSupport compact route={currentRoute} language={outputLanguage} customVoice={selectedIdentity?.source === "mine"} />}</div>{currentRoute && taggedIncompatible && <div className="composer-warning"><b>Inline tags are not available with {methodLabel}.</b><span>Your words stay untouched until you choose what to do.</span><div>{textSession.states.shaped && <Button size="sm" variant="outline" onClick={() => textSession.select("shaped")}>Use Spoken version</Button>}{hasInlineDeliveryTag && <Button size="sm" variant="outline" onClick={removeInlineTags}>Remove inline tags</Button>}</div></div>}<Textarea dir="auto" value={textSession.text} onChange={(event) => textSession.updateText(event.target.value)} placeholder="Type or paste what should be said…" autoFocus />
        <div className="text-pass-actions"><Button variant="outline" disabled={!currentRoute || !textSession.text.trim() || Boolean(textSession.busy) || recovery.status === "loading"} onClick={() => void textSession.run("shape")}><AudioLines />{textSession.busy === "shape" ? "Rewriting…" : "Make it spoken"}</Button>{capabilityControls.deliveryTags ? <><Select value={textSession.density} onValueChange={textSession.setDensity}><SelectTrigger aria-label="Tag density"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="light">Light tags</SelectItem><SelectItem value="normal">Normal tags</SelectItem><SelectItem value="heavy">Heavy tags</SelectItem></SelectContent></Select><Button variant="outline" disabled={!textSession.text.trim() || Boolean(textSession.busy) || recovery.status === "loading"} onClick={() => void textSession.run("tag")}><WandSparkles />{textSession.busy === "tag" ? "Tagging…" : "Add delivery tags"}</Button></> : <p className="composer-engine-note">{currentRoute ? `${methodLabel} does not use inline tags.` : "Choose an exact route to see its text tools."}</p>}<small className="text-pass-cost">Alibaba text pass · {config?.text_preparation?.model || "Qwen text"} · about {formatMicroMoney(textPassEstimate)} each</small></div>
      ...
      {section === "delivery" && <section className="composer-section"><header><div><span className="eyebrow">Performance</span><h3>How should it sound?</h3></div></header>{capabilityControls.directionModes.length > 1 && <div className="delivery-mode"><div><span>Delivery</span>{capabilityControls.directionModes.includes("exact") && <Button variant={deliveryMode === "exact" ? "secondary" : "outline"} onClick={() => setSpeechMode("exact")}>Keep the script</Button>}{capabilityControls.directionModes.includes("directed") && <Button variant={deliveryMode === "directed" ? "secondary" : "outline"} onClick={() => setSpeechMode("directed")}>Add direction</Button>}</div><p>{deliveryMode === "exact" ? capabilityControls.exactHelp : capabilityControls.directedHelp}</p></div>}{capabilityControls.naturalDirection ? <label><span>{capabilityControls.directionLabel}</span><Input value={instruction} disabled={deliveryMode === "exact"} maxLength={config?.instruction_max || 100} onChange={(event) => setInstruction(event.target.value)} placeholder="Describe the performance in natural language" />{deliveryMode === "exact" && <small>Choose Add direction to control the overall performance.</small>}</label> : <p className="composer-engine-note">{selectedCapability?.description || "This recording method uses the prepared script without a separate performance direction."}</p>}{performancePresets.length > 0 && <div className="performance-presets"><span>Presets</span><div>{performancePresets.map((preset) => <Button key={preset.id} type="button" variant={instruction === preset.instruction ? "secondary" : "outline"} onClick={() => { setInstruction(preset.instruction); if (capabilityControls.directionModes.includes("directed")) setSpeechMode("directed") }}><b>{preset.name}</b><small>{preset.instruction}</small></Button>)}</div></div>}</section>}
      {section === "output" && <section className="composer-section"><header><div><span className="eyebrow">Output</span><h3>{capabilityControls.rate || capabilityControls.pitch || capabilityControls.volume ? "Output and voice tuning" : "File settings"}</h3></div></header>{!currentRoute ? <p className="composer-engine-note">Choose an exact route before configuring model-specific output.</p> : capabilityControls.outputNote && <p className="composer-engine-note">{capabilityControls.outputNote}</p>}<div className="composer-fine-grid">{capabilityControls.rate && <label><span>Speed <b>{rate.toFixed(2)}×</b></span><Slider value={[rate]} min={0.5} max={2} step={0.05} onValueChange={([value = 1]) => setRate(value)} /></label>}{capabilityControls.pitch && <label><span>Pitch <b>{pitch.toFixed(2)}×</b></span><Slider value={[pitch]} min={0.5} max={2} step={0.05} onValueChange={([value = 1]) => setPitch(value)} /></label>}{capabilityControls.volume && <label><span>Volume <b>{volume}</b></span><Slider value={[volume]} min={0} max={100} step={1} onValueChange={([value = 50]) => setVolume(value)} /></label>}<label><span>File type</span><Select value={format} onValueChange={setFormat}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{(config?.formats || ["mp3"]).map((item) => <SelectItem key={item} value={item}>{item}</SelectItem>)}</SelectContent></Select></label></div></section>}
    </div>
    <footer className="composer-footer"><div className="composer-cost"><CircleDollarSign /><span>{taggedIncompatible ? `${methodLabel} does not use inline tags` : `${textSession.text.length.toLocaleString()} characters`}</span><b>{taggedIncompatible ? "Open Script to remove tags" : currentRoute ? `about $${estimate.toFixed(4)}` : "Choose an exact route"}</b>{recovery.status === "saving" && <small>Saving preparation…</small>}{recovery.status === "saved" && <small>Preparation saved</small>}{recovery.status === "conflict" && <small className="composer-save-error">Draft changed in another view</small>}{recovery.status === "error" && <small className="composer-save-error">Preparation could not be saved</small>}</div><div className="composer-actions">{!part && projectId && onSave && <Button variant="outline" disabled={!textSession.text.trim() || !currentRoute || Boolean(busy) || recovery.status === "loading"} onClick={async () => { setBusy("draft"); try { await onSave(payload()); await recovery.clear() } finally { setBusy(null) } }}><Plus />{busy === "draft" ? "Saving…" : "Save as draft"}</Button>}<Button disabled={!config?.has_key || !textSession.text.trim() || !currentRoute || Boolean(busy) || taggedIncompatible || recovery.status === "loading"} onClick={() => void generate()}><WandSparkles />{busy === "generate" ? "Generating…" : !projectId ? "Generate audio" : part?.kind === "draft" ? `Record Part ${(part.position ?? 0) + 1}` : part ? `Generate new take · Part ${(part.position ?? 0) + 1}` : `Generate and add Part ${insertAt === null ? nextPartNumber : insertAt + 1}`}</Button></div></footer>
