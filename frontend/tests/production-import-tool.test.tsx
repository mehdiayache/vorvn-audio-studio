...
import type { ProductionImportValidation } from "@/features/production/production-import"
...
function renderTool() {
  return render(<ProductionImportTool
    existing={{ id: 41, publicId: "production-current", name: "Current Production", description: "Keep this context.", partCount: 4, parent: { type: "project", id: 7, name: "Stories" } }}
    config={null}
    directory={directory}
    playerPlaying={false}
    onPlay={vi.fn()}
...
describe("ProductionImportTool", () => {
  ...
  it("uses backend validation and makes current versus JSON metadata explicit", async () => {
    const validate = vi.spyOn(studioApi, "validateProductionImport").mockResolvedValue(validation)
  ...
  it("surfaces the canonical backend validation error without changing stage", async () => {
    vi.spyOn(studioApi, "validateProductionImport").mockRejectedValue(new Error("Item 2 must be Speech or Pause."))
    const { container } = renderTool()
