...
class Settings:
    ...
    host: str = "127.0.0.1"
    port: int = 7860
    web_prefix: str = "/origins"
    database_url: str = "postgresql://origins:origins@127.0.0.1:5434/origins"
    root: Path = ROOT
    ...
    def from_env(cls) -> "Settings":
        return cls(
            host=os.getenv("ORIGINS_HOST", "127.0.0.1"),
            port=int(os.getenv("PORT", "7860")),
            database_url=os.getenv(
                "DATABASE_URL",
                "postgresql://origins:origins@127.0.0.1:5434/origins",
            ),
            output_dir=Path(os.getenv("ORIGINS_OUTPUT_DIR", str(ROOT / "out"))),
        )
...
class AlibabaEnvironment:
    ...
    def native_http_base(self) -> str:
        if self.provider_workspace_id:
            zone = "cn-beijing" if self.region == "beijing" else "ap-southeast-1"
            return f"https://{self.provider_workspace_id}.{zone}.maas.aliyuncs.com/api/v1"
...
def alibaba_environment() -> AlibabaEnvironment:
    ...
    return AlibabaEnvironment(
        ...
        provider_workspace_id=(os.getenv("DASHSCOPE_WORKSPACE_ID") or "").strip(),
...
def require_local_bind() -> None:
    ...
    raise RuntimeError(
        ...
        "ORIGINS_HOST must remain a loopback address."
    )
