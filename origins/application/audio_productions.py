...
from origins.domain.rendering import RenderError, silence_duration_seconds
from origins.domain.sound_scene import audible_sequence
...
def production_scene(editor: dict, sound_scene: dict) -> dict:
    """Production -> Tracks -> Clips view of one playable Production."""
    position = 0.0
    clips = []
    ...
    return {
        ...
        "name": editor.get("name") or "Untitled Production",
        "sample_rate": 48_000,
        "tracks": tracks,
    }
...
class AudioProductionService:
    def __init__(self, workspace: ProductionWorkspace):
        self.workspace = workspace

    def render(self, production: dict) -> dict:
        return self.workspace.render_production(production)
