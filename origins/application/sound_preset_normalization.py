"""Normalize only free Sound Preset language before deterministic compilation."""
...
from origins.domain.sound_presets import (
    LANGUAGE_NORMALIZATION_VERSION,
    PresetCapability,
    compile_sound_preset,
...
SYSTEM_PROMPT = """You normalize creative audio directions for a deterministic Sound Preset compiler.
...
class SoundPresetTextProvider(Protocol):
...
class SoundPresetNormalizationService:
    """Translate free language once; known preset controls remain deterministic."""

    def __init__(self, provider: SoundPresetTextProvider,
    ...
    def normalize(self, *, job_id: int, capability: PresetCapability,
        ...
        reservation_id = self.operations.authorize(
            job_id, "sound_preset_normalization", estimated,
            preferences, confirmed)
        attempt_id = self.operations.repository.begin_attempt(
            job_id, "sound_preset_normalization", {
...
class SoundPresetNormalizationJobHandler:
    def __init__(self, service: SoundPresetNormalizationService):
    ...
    def __call__(self, job: Job, repository: JobProgressRepository) -> dict[str, Any]:
        ...
        repository.progress(job.id, 1, 1, "Sound Preset is ready")
        return {
            **result,
