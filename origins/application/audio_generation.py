"""Generate temporary audio candidates, then Keep through canonical Files."""
...
from origins.application.jobs import JobProgress, JobService
...
from origins.providers.vorvn_audio import (
...
class AudioGenerationService:
    """Own the candidate lifecycle while Files remain owned by uploads."""
    ...
    def enqueue(self, *, capability: AudioGenerationCapability,
        ...
        if semantic_state is not None:
            ...
            if compiled.conflicts:
                raise ValueError(
                    "Resolve the conflicting Sound Preset directions before "
                    "generating.")
    ...
    def recent(
        ...
        for job in jobs:
            candidate_id = str(job.public_id)
            kept_file = (self.uploads.generated_workspace_file(
                workspace_id=job.workspace_id, candidate_id=candidate_id)
                if job.workspace_id is not None else None)
            candidate_available = (
    ...
    def keep(
        self, *, candidate_id: UUID, workspace_id: int, name: str,
        category: FileCategory, tags: tuple[str, ...],
        folder_id: int | None = None,
    ) -> dict:
        return self._keep(
            candidate_id=candidate_id, workspace_id=workspace_id,
            name=name, category=category, tags=tags, folder_id=folder_id)
    ...
    def _keep(
        ...
        existing = self._kept_file(candidate_id, workspace_id)
        if existing:
            self._attach_output_file(candidate_id, workspace_id, existing)
        ...
        details = self.uploads.prepare_file_upload(
            ...
            category=category, supplied_tags=tags,
            metadata=provenance)
        ...
        with TemporaryDirectory(
            ...
            try:
                ...
                raise LookupError(
                    "That temporary generated candidate is no longer available.")
            result = self.uploads.save_generated_workspace_file(
                workspace_id, copy, copy.stat().st_size,
                candidate_id=str(candidate_id), details=details,
                folder_id=folder_id)
        source.unlink(missing_ok=True)
        self._attach_output_file(candidate_id, workspace_id, result["file"])
        return result

    def _attach_output_file(
        self, candidate_id: UUID, workspace_id: int | None, file: dict,
    ) -> None:
        job = self.jobs.get(candidate_id)
        if not job or job.workspace_id != workspace_id:
            raise RuntimeError(
                "The generated File does not belong to this Workspace execution.")
        if not self.jobs.attach_output_file(candidate_id, int(file["id"])):
            raise RuntimeError(
                "The generated File could not be linked to its execution.")

    def _kept_file(
        self, candidate_id: UUID, workspace_id: int,
    ) -> dict | None:
        return self.uploads.generated_workspace_file(
            workspace_id=workspace_id, candidate_id=str(candidate_id))
