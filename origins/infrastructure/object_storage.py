"""Private S3-compatible object storage owned by Origins.
...
def settings() -> dict[str, str]:
    return {
        ...
        "organization_id": (os.getenv("ORIGINS_ORGANIZATION_ID")
...
def object_key(*, kind: str, object_id: str, extension: str,
    ...
    if not _SEGMENT.fullmatch(organization):
        raise ValueError("ORIGINS_ORGANIZATION_ID is invalid.")
...
def put(local_path: str | Path, content_type: str = "audio/wav",
    ...
    with target.open("rb") as source:
        _client().put_object(
            ...
            Metadata={"origins-object-id": identity,
                      "retention": retention,
                      "variant": variant},
