...
from origins.domain.sound_scene import effect_tail_ms
from origins.domain.rendering import (
    FinishedExport,
    RenderError,
...
def _mix_scene(sequence: Path, scene: dict, target: Path) -> bool:
    ...
    for _, clip in clips:
        ...
        if clip.get("source_media_type") == "video":
            ...
            # remains the one canonical File; this is only a render source.
            if video_audio_workspace is None:
                from origins.infrastructure.media_workspace import (
                    LocalMediaWorkspace,
                )
...
def _render_visual_scene(
    ...
    for track, clip in placements:
        source = sources.get(str(clip["file_id"]))
