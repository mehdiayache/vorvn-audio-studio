...
class ProductionDocumentRepository:
    ...
    def insert_file(self, production_id: int, file_id: int,
                     before_part_public_id: str | None = None) -> int | None:
        with read_only() as cursor:
            cursor.execute("""
                SELECT file.id, version.id, file.name, version.duration_ms
                 ...
                 WHERE file.id = %s
                   AND file.workspace_id = production.workspace_id
        ...
        if part_id is not None:
            with transaction() as cursor:
                ...
                """, (file_id, production_id))
        return part_id

    def replace_file(self, production_id: int, part_id: int,
                      file_id: int) -> bool:
        with transaction() as cursor:
            cursor.execute("""
            ...
            cursor.execute("""
                UPDATE production_parts
                 ...
                 WHERE id=%s AND production_id=%s AND kind='file'
                   AND archived_at IS NULL
            """, (file_id, file[0], file[1] or "", file[1] or "",
                  file[2], part_id, production_id))
