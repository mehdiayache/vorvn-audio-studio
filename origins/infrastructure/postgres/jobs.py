...
from origins.domain.jobs import (
    IdempotencyConflict,
    Job,
    JobCancelled,
    JobStatus,
...
from origins.domain import provider_catalog as alibaba_config
...
def _job(row) -> Job:
    return Job(row[0], row[1], row[2], JobStatus(row[3]), row[4] or {},
               ...
               row[17], row[18] or {},
               tuple(int(value) for value in (row[19] or [])))
...
class JobRepository:
    ...
    def enqueue_in_transaction(self, cursor, kind: str,
        ...
        requested_route = {
            "executor": _EXECUTOR,
            "source_tool": source_tool or "origins",
        ...
        cursor.execute("""
                ...
                VALUES (%s, 'queued', %s::jsonb, %s, %s, %s, %s, 0, 0,
        ...
        """, (kind, json.dumps(payload), idempotency_key, actor_id,
                  ...
                  workspace_id, creation_action_id, creation_preset_id,
    ...
    def confirm(self, public_id: UUID, *, idempotency_key: str) \
        ...
        with transaction() as cursor:
            ...
            confirmed, created = self.enqueue_in_transaction(
                ...
                workspace_id=source[12], creation_action_id=source[13],
                creation_preset_id=source[14],
                creation_context=source[15] or {})
            if created:
                cursor.execute("""
    ...
    def recent_for_workspace(self, workspace_id: int, *, kind: str,
                         limit: int = 8) -> list[Job]:
        """Return recent executions owned directly by one Workspace."""
        with read_only() as cursor:
            cursor.execute(
                _SELECT + """
                 ...
                 LIMIT %s
                """,
                (workspace_id, kind, max(1, min(limit, 20))),
            )
            return [_job(row) for row in cursor.fetchall()]
                 ...
                 WHERE public_id = %s
                   ...
                   AND EXISTS (
                       SELECT 1 FROM files file
                        WHERE file.id = %s
                          AND file.workspace_id = jobs.workspace_id
                   )
