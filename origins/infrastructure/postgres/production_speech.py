...
class ProductionSpeechCommandRepository:
    ...
    def enqueue(self, payload: dict[str, Any], *, idempotency_key: str,
        ...
        with transaction() as cursor:
            cursor.execute("""
                SELECT workspace_id FROM productions
                 WHERE id=%s AND production_type='audiovisual'
            """, (production_id,))
            production = cursor.fetchone()
            if not production:
                raise LookupError("That Production no longer exists.")
            workspace_id = int(production[0])
            context = dict(creation_context or {})
            if context.get("workspace_id") != workspace_id:
                raise ValueError("Creator context does not belong to this Production Workspace.")
            if context.get("production_id") != production_id:
                raise ValueError("Creator context does not target this Production.")
