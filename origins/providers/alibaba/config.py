...
from origins.domain.provider_catalog import (
...
def provider_workspace_id() -> str:
    return alibaba_environment().provider_workspace_id
...
def websocket_base() -> str:
    ...
    if environment.provider_workspace_id:
        zone = "cn-beijing" if environment.region == "beijing" else "ap-southeast-1"
        return f"wss://{environment.provider_workspace_id}.{zone}.maas.aliyuncs.com/api-ws/v1/inference"
...
def workspace_compatible_base_url() -> str:
    ...
    if environment.provider_workspace_id:
        zone = "cn-beijing" if environment.region == "beijing" else "ap-southeast-1"
        return f"https://{environment.provider_workspace_id}.{zone}.maas.aliyuncs.com/compatible-mode/v1"
