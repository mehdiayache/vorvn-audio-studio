...
class AlibabaSingaporeMediaGenerationProvider:
    ...
    def callback_configured() -> bool:
        # No Media generation callback is registered until a concrete Wan adapter has
    ...
    def estimate_cost(request: dict[str, Any]) -> float:
        ...
        raise MediaGenerationProviderError(
            "Alibaba pricing is not configured for this Media model.")
    ...
    def _key() -> str:
        ...
        if environment.region != "intl":
            raise MediaGenerationProviderSetupError(
                "Choose Singapore in Settings for Alibaba Media models.")
        if not key:
            raise MediaGenerationProviderSetupError(
    ...
    @staticmethod
    def _state(payload: dict[str, Any]) -> MediaGenerationProviderState:
        output = payload.get("output") or {}
