...
from origins.composition.settings import settings_service
from origins.http.errors import ApiProblem
from origins.http.settings_contracts import (
...
class MediaGenerationProviderUpdate(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
@router.patch("/providers/kie", operation_id="updateKieSettings",
              response_model=SettingsSnapshotEnvelope,
              response_model_exclude_none=True)
def update_kie(payload: MediaGenerationProviderUpdate) -> dict:
    try:
        return {"data": settings_service.update_media_generation_provider(
            payload.model_dump())}
    except (OSError, ValueError) as exc:
...
def test_kie_connection() -> dict:
    return {"data": {
        "provider": "KIE", **settings_service.test_media_generation_provider()}}
