"""Capabilities and durable provider generations for Composer."""
...
class ComposerInputSlot(BaseModel):
    role: str
    label: str
    required: bool
...
class ComposerPromptCapability(BaseModel):
    supported: bool
    required: bool
...
class ComposerParameterCapability(BaseModel):
...
class ComposerDurationRange(BaseModel):
    min: int
    max: int
    step: int
    default: int
...
class ComposerOutputCapability(BaseModel):
    mime_type: str
    extension: str


class ComposerControlRule(BaseModel):
    when: dict[str, Any] = Field(default_factory=dict)
    values: list[str]
    default: str


class ComposerOperationCapability(BaseModel):
    operation: str
    output_media_type: Literal["image", "video"]
    prompt: ComposerPromptCapability
    inputs: list[ComposerInputSlot]
    ...
    ratios: list[str]
    ratio_rules: list[ComposerControlRule] = Field(default_factory=list)
    resolutions: list[str]
    durations: list[int]
    duration_range: ComposerDurationRange | None = None
    fps: list[int]
    supports_seed: bool
    supports_cancel: bool
    parameters: list[ComposerParameterCapability] = Field(default_factory=list)
    output: ComposerOutputCapability
...
class MediaModelCapability(BaseModel):
    ...
    operations: list[ComposerOperationCapability]


class ComposerOperationInfo(BaseModel):
    id: str
    label: str
    detail: str
...
class ComposerCapabilities(BaseModel):
    ...
    operations: list[ComposerOperationInfo]
    models: list[MediaModelCapability]


class ComposerCapabilitiesEnvelope(BaseModel):
    data: ComposerCapabilities
...
class ComposerInputCompatibilityRequest(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class ComposerInputCompatibilityResponse(BaseModel):
    file_id: int
    state: Literal["compatible", "incompatible", "unknown"]
    reasons: list[str]


class ComposerInputCompatibilityEnvelope(BaseModel):
    data: list[ComposerInputCompatibilityResponse]
...
class ComposerGenerationRequest(BaseModel):
    model_config = ConfigDict(extra="forbid")
    context: ComposerContext
    preset: MediaGenerationPreset


@router.get("/composer/capabilities",
            operation_id="getMediaGenerationCapabilities",
            response_model=ComposerCapabilitiesEnvelope)
def composer_capabilities() -> dict:
    return {"data": media_generation_service.capabilities()}


@router.get("/composer/models", operation_id="getMediaModels",
            response_model=ComposerCapabilitiesEnvelope)
def media_models() -> dict:
    """Canonical model catalogue consumed by Composer and external clients."""
    return {"data": media_generation_service.capabilities()}


@router.post(
    "/composer/input-compatibility",
    operation_id="checkComposerInputCompatibility",
    response_model=ComposerInputCompatibilityEnvelope,
)
def check_composer_input_compatibility(
    payload: ComposerInputCompatibilityRequest,
) -> dict:
    try:
        ...
        raise ApiProblem(400, "invalid_composer_input", str(exc)) from exc


@router.get("/composer/generations",
...
@router.post("/composer/generations/{job_id}/cancel",
             operation_id="cancelMediaGeneration",
             response_model=MediaGenerationEnvelope)
def cancel_generation(job_id: UUID, context: ComposerContext) -> dict:
    try:
...
@router.post(
    "/composer/generations/{job_id}/retry-ingestion",
    operation_id="retryMediaGenerationIngestion",
    response_model=MediaGenerationEnvelope,
)
def retry_generation_ingestion(job_id: UUID, context: ComposerContext) -> dict:
    try:
