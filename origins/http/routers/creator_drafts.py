"""Recoverable provider-neutral Creator preparation state."""
...
from origins.application.creator_drafts import CreatorDraftConflict
from origins.domain.speech import DEFAULT_SPEECH_VOLUME
from origins.composition.creator_drafts import creator_draft_service
from origins.http.errors import ApiProblem


router = APIRouter(prefix="/api/v1/creator-drafts", tags=["creator"])
...
def _conflict(operation):
    try:
        return operation()
    except CreatorDraftConflict as exc:
        raise ApiProblem(409, "creator_draft_conflict", str(exc)) from exc
    except ValueError as exc:
        raise ApiProblem(400, "creator_draft_invalid", str(exc)) from exc


@router.post("/resolve", operation_id="resolveCreatorDraft",
             response_model=DraftEnvelope)
def resolve_draft(payload: DraftLookup) -> dict:
    return {"data": _conflict(
        lambda: creator_draft_service.get(_context(payload)))}


@router.put("", operation_id="saveCreatorDraft",
            response_model=DraftEnvelope)
def save_draft(payload: DraftWrite) -> dict:
    return {"data": _conflict(lambda: creator_draft_service.put(
        _context(payload), _state(payload), payload.expected_version))}


@router.api_route("", methods=["DELETE"], operation_id="deleteCreatorDraft",
                  response_model=DraftDeleteEnvelope)
def delete_draft(payload: DraftDelete) -> dict:
    return {"data": _conflict(lambda: creator_draft_service.delete(
