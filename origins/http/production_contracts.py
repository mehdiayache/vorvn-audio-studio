"""Public response contracts for audiovisual Productions."""
...
class ProductionFileLibraryResponse(BaseModel):
    ...
    production_file_ids: list[int] = Field(default_factory=list)
    library_file_ids: list[int] = Field(default_factory=list)


class ProductionFileLibraryEnvelope(BaseModel):
    data: ProductionFileLibraryResponse
...
class ProductionEditorResponse(BaseModel):
    model_config = ConfigDict(extra="allow")
