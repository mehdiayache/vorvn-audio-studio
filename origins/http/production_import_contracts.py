...
class ProductionImportDocument(BaseModel):
    ...
    schema_name: Literal["origins-production-import"] = Field(alias="schema")
    version: Literal[1]
...
class ProductionImportBody(BaseModel):
    """Append authored Parts to one existing audiovisual Production."""
...
class ProductionImportDestination(BaseModel):
    ...
    def coherent_target(self):
        if self.kind == "existing":
            if not self.production_id or self.workspace_id or self.folder_id:
                raise ValueError("An existing destination needs only a Production ID.")
        elif self.production_id or not self.workspace_id:
