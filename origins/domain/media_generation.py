...
from origins.domain.media_models import model_capability


MEDIA_GENERATION_KIND = "media_generate"
...
def file_list_compatibility_contract(
...
def _validate_file_list(
    field: dict[str, Any], value: Any, files: dict[int, dict[str, Any]],
) -> None:
    label = field["label"]
    ...
    for item in value:
        ...
        if not isinstance(file_ids, list):
            raise ValueError("Subject references must use canonical Files.")
        if not (int(variant.get("min_files") or 0) <= len(file_ids)
                <= int(variant.get("max_files") or 0)):
            raise ValueError(
                f"{variant['label']} needs between "
                f"{variant['min_files']} and {variant['max_files']} Files.")
        for file_id_value in file_ids:
            file_id = int(file_id_value or 0)
            file = files.get(file_id)
            if not file or file_id in used_files:
                raise ValueError(
                    "Every subject reference must be a unique canonical File.")
...
def input_file_compatibility(
    slot: dict[str, Any], file: dict[str, Any],
) -> dict[str, Any]:
    ...
    size_value = file.get("size_bytes")
    ...
    duration_value = file.get("duration_ms")
    ...
    width_value, height_value = file.get("width"), file.get("height")
...
def _validate_input_file(slot: dict[str, Any], file: dict[str, Any]) -> None:
    ...
    if result["state"] == "unknown":
        raise ValueError(
            f"{reason} Re-import this media so Origins can inspect it before generation.")
    raise ValueError(reason)


def validate_preset(preset: dict[str, Any], files: dict[int, dict[str, Any]]) -> None:
    _, selected = capability(
        str(preset.get("model_id") or ""), str(preset.get("operation") or ""))
    ...
    if preset.get("negative_prompt") and not prompt_contract["negative_prompt"]:
        raise ValueError(
    ...
    for position, item in enumerate(inputs):
        ...
        if item.get("media_type") != media_type:
            raise ValueError(
                "The reference media type does not match its canonical File.")
        if file_id in seen_files:
            raise ValueError("The same File cannot fill two input positions.")
    ...
    for key, field in fields.items():
        ...
        _validate_parameter(field, value, files)
    ...
    if input_mode:
        ...
        nested_images = sum(
            len(item.get("file_ids") or []) for item in elements
        ...
        maximum_images = int(
            element_policy.get("max_image_files_total") or 0)
        ...
        if video_subjects:
            mixed_maximum = element_policy.get(
                "max_image_files_with_video_subjects")
