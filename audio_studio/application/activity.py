"""Operator ledger read model built from durable Jobs, not editable outputs."""
...
def _run(row) -> dict[str, Any]:
    ...
    return {
        ...
        "kind": kind, "kind_label": KIND_LABELS.get(kind, kind.replace("_", " ").title()),
        "operation": operation_label or KIND_LABELS.get(kind, kind.replace("_", " ").title()),
        ...
        "provider_region": provider_region, "provider_endpoint": provider_endpoint,
        ...
        "cost_basis": _basis(cost_basis), "cost_basis_raw": cost_basis or "unknown",
        "children": 0,
    }


def snapshot(*, limit: int = 80, kind: str = "", failed_only: bool = False) -> dict:
    where, parameters = ["1=1"], []
    if kind:
        where.append("job.kind = %s")
        parameters.append(kind)
    if failed_only:
        where.append("job.status IN ('failed', 'warning', 'blocked', 'lost')")
    query = f"""
        SELECT job.id, job.public_id, job.created_at, job.kind, job.status,
               ...
               job.seconds, job.elapsed_ms, job.actor_id, job.organization_id,
               job.provider_request_id, job.provider_region, job.provider_endpoint,
          ...
          FROM jobs job
          LEFT JOIN productions production ON production.id = job.production_id
    ...
    with read_only() as cursor:
        ...
        cursor.execute("""
            SELECT coalesce(sum(cost) FILTER (WHERE created_at::date = current_date), 0),
                   coalesce(sum(cost) FILTER (WHERE date_trunc('month', created_at) = date_trunc('month', now())), 0),
                   coalesce(sum(cost), 0), count(*),
                   count(*) FILTER (WHERE status IN ('failed','warning','blocked','lost'))
              FROM jobs
        """)
        totals = cursor.fetchone()
        cursor.execute("""
            SELECT coalesce(cost_basis, 'unknown'), count(*), coalesce(sum(cost), 0)
              FROM jobs GROUP BY coalesce(cost_basis, 'unknown') ORDER BY 1
        """)
        breakdown = [{"basis": _basis(row[0]), "raw_basis": row[0],
                      "runs": row[1], "cost": float(row[2] or 0)}
                     for row in cursor.fetchall()]
        cursor.execute("SELECT kind, count(*), coalesce(sum(cost),0), count(*) FILTER (WHERE status IN ('failed','warning','blocked','lost')) FROM jobs GROUP BY kind ORDER BY kind")
        by_kind = [{"kind": row[0], "runs": row[1], "cost": float(row[2] or 0),
                    "problems": row[3]} for row in cursor.fetchall()]
    return {
        "today": float(totals[0]), "month": float(totals[1]),
        "total": float(totals[2]), "runs": totals[3], "problems": totals[4],
        "running": [item for item in runs if item["status"] in {"queued", "running", "retrying"}],
        "runs_list": runs, "kinds": KIND_LABELS, "cost_breakdown": breakdown,
        "by_kind": by_kind, "by_day": [],
    }
