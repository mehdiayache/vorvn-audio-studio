...
def init() -> bool:
    ...
    compact_positions()
    # A process may stop after provider creation but before its package-level
    # Activity row is closed. Reconcile those durable child states at startup.
    voice_package_reconcile_ledgers()
    return True
...
VOICE_PREFIX = re.compile(r"^qwen[\w.-]*?-tts-(?:plus|flash)-", re.I)


def voice_key(voice_id: str) -> str:
    """The voice itself, without the quality it happens to be speaking at.

    Alibaba gives the same voice two ids — one for Plus, one for Flash. Storing
    a picture or a favourite against the id meant setting it twice and seeing it
    once. Everything a person decides about a voice hangs off this key instead.
    """
    return VOICE_PREFIX.sub("", voice_id or "").strip()
...
def voice_meta() -> dict:
    """Pictures, favourites and notes, keyed by voice."""
    with cursor() as cur:
        if cur is None:
            return {}
        cur.execute("SELECT id, image, favourite, note, name, gender, age, "
                    "       trait, scene, languages, provider_voice_id, engine, "
                    "       target_model, provider_status FROM voices")
        return {voice_key(row[0]): {
                    ...
                    "target_model": row[12] or "", "provider_status": row[13] or ""}
                for row in cur.fetchall()}


def voice_save(voice_id: str, **fields) -> bool:
    """Set one or more of a voice's own values, leaving the rest alone."""
    allowed = {k: v for k, v in fields.items()
               if k in ("image", "favourite", "note", "name", "gender", "age",
                        "trait", "scene", "languages", "provider_voice_id",
                        "engine", "target_model", "provider_status")}
    voice_id = voice_key(voice_id)
    if not voice_id or not allowed:
        return False
    ...
    with cursor(write=True) as cur:
        if cur is None:
            return False
        cur.execute(
            f"INSERT INTO voices (id, {columns}) VALUES (%s, {placeholders}) "
            f"ON CONFLICT (id) DO UPDATE SET {updates}, updated_at = now()",
            [voice_id[:200], *allowed.values()],
        )
        return True


def voice_forget(voice_id: str) -> bool:
    """Drop everything decided about a voice, when its clone is deleted."""
    with cursor(write=True) as cur:
        if cur is None:
            return False
        cur.execute("DELETE FROM voices WHERE id = %s", (voice_key(voice_id),))
        return True


def voice_reference_create(original_name: str = "", original_path: str = "",
                           normalized_path: str = "", source_url: str = "") -> str | None:
    """Register source audio before enrollment so it can never become orphaned."""
...
def voice_reference_get(reference_id: str) -> dict | None:
    with cursor() as cur:
        if cur is None:
            return None
        cur.execute(
            "SELECT id, identity_id, original_name, original_path, normalized_path, created_at "
            "FROM voice_references WHERE id = %s", (reference_id,))
        row = cur.fetchone()
        if not row:
            return None
        return {"id": row[0], "identity_id": row[1], "original_name": row[2] or "",
                "original_path": row[3] or "", "normalized_path": row[4] or "",
                "created_at": row[5].isoformat()}


def voice_reference_attach(reference_id: str, identity_id: str) -> bool:
    """Attach a preserved source once; never steal it from another identity."""
    with cursor(write=True) as cur:
        if cur is None:
            return False
        cur.execute(
            """UPDATE voice_references SET identity_id = %s
                 WHERE id = %s AND (identity_id IS NULL OR identity_id = %s)""",
            (identity_id, reference_id, identity_id),
        )
        return cur.rowcount == 1


def voice_identity_create(name: str, metadata: dict, reference_id: str) -> str | None:
    with cursor(write=True) as cur:
        if cur is None:
            return None
        # Retrying the same request completes one identity instead of creating
        # a duplicate for the same biometric source.
        cur.execute("SELECT identity_id FROM voice_references WHERE id = %s FOR UPDATE",
                    (reference_id,))
        ...
        if reference[0]:
            return reference[0]
        identity_id = f"voice_{uuid.uuid4().hex}"
        cur.execute(
            """INSERT INTO voice_identities
                   (id, name, metadata, gender, age, accent, trait, scene, notes,
                    recording_language)
               VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)""",
            (identity_id, name.strip(), json.dumps(metadata or {}),
             metadata.get("gender") or None, metadata.get("age") or None,
             metadata.get("accent") or None, metadata.get("trait") or None,
             metadata.get("scene") or None, metadata.get("notes") or None,
             metadata.get("language") or None))
        cur.execute("UPDATE voice_references SET identity_id = %s WHERE id = %s",
                    (identity_id, reference_id))
    return identity_id


def voice_package_enqueue(identity_id: str, reference_id: str,
                          routes: list[dict]) -> list[str]:
    """Idempotently queue missing model bindings for one human voice."""
    queued = []
    with cursor(write=True) as cur:
        if cur is None:
            return queued
        for route in routes:
            cur.execute(
                "SELECT 1 FROM voice_bindings WHERE identity_id = %s AND model_id = %s ",
            ...
            cur.execute(
                """INSERT INTO voice_package_jobs
                    (id, identity_id, reference_id, model_id, engine, tier, status)
                   VALUES (%s, %s, %s, %s, %s, %s, 'queued')
                   ON CONFLICT (identity_id, model_id) DO UPDATE SET
                     reference_id = EXCLUDED.reference_id,
                     status = CASE WHEN voice_package_jobs.status = 'ready'
                                   THEN 'ready' ELSE 'queued' END,
                     error = NULL, updated_at = now()
                   RETURNING id, status""",
                (job_id, identity_id, reference_id, route["model_id"],
                 route["engine"], route["tier"]))
            returned_id, status = cur.fetchone()
            if status == "queued":
                queued.append(returned_id)
    return queued


def voice_package_job(job_id: str) -> dict | None:
    with cursor() as cur:
        if cur is None:
            return None
        cur.execute(
            """SELECT j.id, j.identity_id, j.reference_id, j.model_id, j.engine,
                      j.tier, j.status, j.provider_voice_id, j.error, j.attempts,
                      i.name, i.metadata
                 FROM voice_package_jobs j
                 JOIN voice_identities i ON i.id = j.identity_id
                WHERE j.id = %s""", (job_id,))
        row = cur.fetchone()
        if not row:
            return None
        return dict(zip(("id", "identity_id", "reference_id", "model_id", "engine",
                         "tier", "status", "provider_voice_id", "error", "attempts",
                         "name", "metadata"), row))


def voice_package_job_update(job_id: str, status: str, provider_voice_id: str = "",
                             error: str = "") -> bool:
    with cursor(write=True) as cur:
        if cur is None:
            return False
        cur.execute(
            """UPDATE voice_package_jobs
                  SET status = %s, provider_voice_id = NULLIF(%s, ''),
                      error = NULLIF(%s, ''),
                      attempts = attempts + CASE WHEN %s = 'creating' THEN 1 ELSE 0 END,
                      updated_at = now()
                WHERE id = %s""",
            (status, provider_voice_id, error[:600], status, job_id))
        changed = cur.rowcount == 1
    if changed and status in ("ready", "failed"):
        voice_package_reconcile_ledgers()
    return changed


def voice_package_reconcile_ledgers() -> int:
    """Close package-level Activity rows once every child capability settles."""
    with cursor(write=True) as cur:
        if cur is None:
            return 0
        cur.execute(
            """WITH package_state AS (
                   SELECT identity_id,
                          bool_or(status IN ('queued', 'creating', 'interrupted')) AS active,
                          count(*) FILTER (WHERE status = 'ready') AS ready,
                          count(*) FILTER (WHERE status = 'failed') AS failed
                     FROM voice_package_jobs GROUP BY identity_id
               )
               UPDATE jobs ledger
                  SET status = CASE WHEN state.failed > 0 THEN 'failed' ELSE 'ok' END,
                      detail = identity.name || ' · ' || state.ready ||
                               CASE WHEN state.ready = 1 THEN ' capability ready'
                                    ELSE ' capabilities ready' END ||
                               CASE WHEN state.failed > 0
                                    THEN ' · ' || state.failed || ' failed' ELSE '' END,
                      done = state.ready,
                      total = state.ready + state.failed,
                      finished_at = COALESCE(ledger.finished_at, now()),
                      elapsed_ms = GREATEST(0, EXTRACT(EPOCH FROM
                                   (now() - ledger.created_at)) * 1000)::int
                 FROM package_state state
                 JOIN voice_identities identity ON identity.id = state.identity_id
                WHERE ledger.kind = 'clone_package'
                  AND ledger.status IN ('queued', 'running')
                  AND COALESCE(ledger.voice_identity_id, ledger.voice) = state.identity_id
                  AND NOT state.active""")
        return cur.rowcount


def voice_package_retry(identity_id: str, model_id: str) -> str | None:
    with cursor(write=True) as cur:
        if cur is None:
            return None
        cur.execute(
            """UPDATE voice_package_jobs SET status = 'queued', error = NULL,
                      updated_at = now()
                WHERE identity_id = %s AND model_id = %s
                  AND status IN ('failed', 'interrupted') RETURNING id""",
            (identity_id, model_id))
        row = cur.fetchone()
        return row[0] if row else None


def voice_package_claim_next() -> str | None:
    """Atomically lease one voice creation to the dedicated worker process."""
    with cursor(write=True) as cur:
        if cur is None:
            return None
        cur.execute(
...
def voice_package_abandon_running() -> int:
    with cursor(write=True) as cur:
        if cur is None:
            return 0
        cur.execute(
            """UPDATE voice_package_jobs SET status = 'interrupted',
                      error = 'The app stopped before this variant finished',
                      updated_at = now() WHERE status = 'creating'""")
        return cur.rowcount


def voice_identities() -> list[dict]:
    """Human voices with ready bindings, source audio and package job states."""
    with cursor() as cur:
        if cur is None:
            return []
        cur.execute("""SELECT i.id, i.name, i.metadata, i.image, i.gender, i.age,
                               i.accent, i.trait, i.scene, i.notes,
                               i.recording_language, i.favourite, i.status,
                               i.created_at, i.updated_at
                         FROM voice_identities i
                        WHERE EXISTS (SELECT 1 FROM voice_bindings b WHERE b.identity_id = i.id)
                           OR EXISTS (SELECT 1 FROM voice_references r WHERE r.identity_id = i.id)
                           OR EXISTS (SELECT 1 FROM voice_package_jobs j WHERE j.identity_id = i.id)
                        ORDER BY i.created_at DESC""")
        identities = [{"id": row[0], "name": row[1], "metadata": {
                           **(row[2] or {}), "image": row[3] or "",
                           ...
                           "favourite": bool(row[11]), "status": row[12] or "active",
                       },
                       "created_at": row[13].isoformat(), "updated_at": row[14].isoformat(),
                       "references": [], "bindings": [], "jobs": []}
                      for row in cur.fetchall()]
        by_id = {item["id"]: item for item in identities}
        cur.execute("SELECT id, identity_id, original_name, normalized_path, created_at FROM voice_references WHERE identity_id IS NOT NULL ORDER BY created_at DESC")
        for ref_id, identity_id, original_name, normalized_path, created_at in cur.fetchall():
            if identity_id in by_id:
                by_id[identity_id]["references"].append({"id": ref_id,
                    "original_name": original_name or "", "normalized_path": normalized_path or "",
                    "created_at": created_at.isoformat()})
        cur.execute("SELECT provider_voice_id, model_id, identity_id, engine, tier, status, languages, created_at FROM voice_bindings ORDER BY created_at")
        for provider_id, model_id, identity_id, engine, tier, status, languages, created_at in cur.fetchall():
            if identity_id in by_id:
                by_id[identity_id]["bindings"].append({"provider_voice_id": provider_id,
                    "model_id": model_id, "engine": engine, "tier": tier, "status": status,
                    "languages": languages or [], "created_at": created_at.isoformat()})
        cur.execute("SELECT id, identity_id, reference_id, model_id, engine, tier, status, provider_voice_id, error, attempts, updated_at FROM voice_package_jobs ORDER BY created_at")
        for row in cur.fetchall():
            if row[1] in by_id:
...
VOICE_IDENTITY_FIELDS = {
    "name", "image", "gender", "age", "accent", "trait", "scene", "notes",
    "recording_language", "favourite", "status",
}


def voice_identity_update(identity_id: str, changes: dict) -> bool:
    """Edit our identity without mutating immutable provider voice IDs."""
    allowed = {key: changes[key] for key in VOICE_IDENTITY_FIELDS if key in changes}
    if not allowed:
        return False
    if "name" in allowed:
    ...
    with cursor(write=True) as cur:
        if cur is None:
            return False
        cur.execute(f"UPDATE voice_identities SET {sets}, updated_at = now() WHERE id = %s",
                    [*allowed.values(), identity_id])
        return cur.rowcount == 1


def voice_identity_for_provider(provider_voice_id: str, engine: str = "",
                                tier: str = "") -> str | None:
    with cursor() as cur:
        if cur is None:
            return None
        model_pattern = f"%{tier}%" if tier in ("plus", "flash") else "%"
        cur.execute(
            ...
            (provider_voice_id, engine, engine, model_pattern))
        row = cur.fetchone()
        return row[0] if row else None


def voice_identity_usage() -> dict[str, dict]:
    """Usage rolled up to our identity, regardless of provider binding."""
    with cursor() as cur:
        if cur is None:
            return {}
        cur.execute("""
            SELECT coalesce(g.voice_identity_id, b.identity_id) AS identity_id,
                   count(*) AS uses, count(DISTINCT g.project_id) AS productions,
                   coalesce(sum(g.cost), 0), max(g.created_at),
                   (array_agg(g.filename ORDER BY g.created_at DESC)
                       FILTER (WHERE g.filename <> ''))[1]
              FROM generations g
              LEFT JOIN voice_bindings b ON b.provider_voice_id = g.voice
             WHERE g.voice NOT IN ('', '-') AND coalesce(g.kind, '') <> 'asset'
             GROUP BY coalesce(g.voice_identity_id, b.identity_id)
        """)
        return {row[0]: {"uses": row[1], "productions": row[2],
                         "spend": float(row[3]),
...
def voice_historical_unlinked() -> list[dict]:
    """Old custom-clone references that predate canonical voice identities.

    System catalogue voices deliberately stay outside this result. Relinking
    history is metadata-only: the immutable provider id and existing audio are
    never rewritten.
    """
    with cursor() as cur:
        if cur is None:
            return []
        cur.execute("""
            SELECT g.voice, coalesce(max(g.engine), ''), coalesce(max(g.model), ''),
                   count(*), count(DISTINCT g.project_id), max(g.created_at),
                   (array_agg(g.filename ORDER BY g.created_at DESC)
                       FILTER (WHERE coalesce(g.filename, '') <> ''))[1]
              FROM generations g
             WHERE g.voice_identity_id IS NULL
               AND coalesce(g.kind, '') NOT IN ('asset', 'silence')
               AND g.voice ~* '^qwen.*-[0-9a-f]{32}$'
             GROUP BY g.voice
             ORDER BY max(g.created_at) DESC
        """)
        return [{"provider_voice_id": row[0], "engine": row[1], "model": row[2],
...
def voice_link_history(provider_voice_id: str, identity_id: str) -> int:
    """Attach legacy generation rows to one identity without changing audio."""
    ...
    if not provider_voice_id or not identity_id:
        raise ValueError("A historical provider voice and target identity are required.")
    with cursor(write=True) as cur:
        if cur is None:
            return 0
        cur.execute("SELECT status FROM voice_identities WHERE id = %s", (identity_id,))
        identity = cur.fetchone()
        if not identity:
            raise ValueError("That voice identity does not exist.")
        if identity[0] == "archived":
            raise ValueError("Restore the archived voice before linking history to it.")
        cur.execute(
...
def voice_identity_bind(provider_voice_id: str, model_id: str, name: str,
                        engine: str, tier: str, languages=None,
                        status: str = "active", reference_id: str | None = None,
                        identity_id: str | None = None) -> str | None:
    """Create/update one identity and its exact provider-model binding."""
    identity_id = identity_id or f"voice_{uuid.uuid4().hex}"
    clean_languages = [str(item).strip() for item in (languages or []) if str(item).strip()]
    with cursor(write=True) as cur:
        if cur is None:
            return None
        cur.execute(
            ...
            (identity_id, name or provider_voice_id),
        )
        cur.execute(
            "INSERT INTO voice_bindings "
            "(provider_voice_id, model_id, identity_id, engine, tier, status, languages) "
            ...
            "identity_id = EXCLUDED.identity_id, engine = EXCLUDED.engine, "
            "tier = EXCLUDED.tier, status = EXCLUDED.status, "
            "languages = EXCLUDED.languages, updated_at = now()",
            (provider_voice_id, model_id, identity_id, engine, tier, status,
             json.dumps(clean_languages)),
        )
        if reference_id:
            cur.execute("UPDATE voice_references SET identity_id = %s WHERE id = %s",
                        (identity_id, reference_id))
...
def voice_reference_link(reference_id: str, provider_voice_id: str) -> bool:
    """Attach newly uploaded source audio to an existing voice identity."""
    with cursor(write=True) as cur:
        if cur is None:
            return False
        cur.execute(
            """UPDATE voice_references r
                  SET identity_id = b.identity_id
                 FROM voice_bindings b
                WHERE r.id = %s AND b.provider_voice_id = %s""",
            (reference_id, provider_voice_id),
        )
        return cur.rowcount > 0


def voice_binding_references() -> dict:
    """Source-audio details keyed by exact provider voice ID for the registry."""
    with cursor() as cur:
        if cur is None:
            return {}
        cur.execute("""
            SELECT b.provider_voice_id, b.identity_id, r.id, r.original_name,
                   r.original_path, r.normalized_path
              FROM voice_bindings b
              LEFT JOIN voice_references r ON r.identity_id = b.identity_id
             ORDER BY r.created_at DESC NULLS LAST
        """)
        result = {}
        for provider_id, identity_id, ref_id, original_name, original_path, normalized_path in cur.fetchall():
            item = result.setdefault(provider_id, {"identity_id": identity_id})
            if ref_id and "id" not in item:
                item.update({"id": ref_id, "original_name": original_name or "",
                             "original_path": original_path or "",
                             "normalized_path": normalized_path or ""})
        return result


def voice_custom_bindings() -> list[dict]:
    """Durable provider bindings used by the fast Composer registry."""
    with cursor() as cur:
        if cur is None:
            return []
        cur.execute("""
            SELECT b.provider_voice_id, b.model_id, b.engine, b.tier, b.status,
                   b.languages, i.id, i.name, i.image, i.gender, i.age,
                   i.accent, i.trait, i.scene, i.notes
              FROM voice_bindings b
              JOIN voice_identities i ON i.id = b.identity_id
             WHERE b.source = 'custom' AND i.status = 'active'
             ORDER BY i.name, b.model_id
        """)
        return [{"voice_id": provider_id, "target_model": model_id,
                 "source": "custom",
                 "engine": engine, "tier": tier, "status": status,
                 "languages": languages or [], "identity_id": identity_id,
                 "name": name, "image": image or "", "gender": gender or "",
                 "age": age, "accent": accent or "", "trait": trait or "",
...
def voice_binding_forget(provider_voice_id: str) -> bool:
    """Remove a provider binding; identity/source survive if another model uses them."""
    with cursor(write=True) as cur:
        if cur is None:
            return False
        cur.execute("DELETE FROM voice_bindings WHERE provider_voice_id = %s",
                    (provider_voice_id,))
        return True


def adopt_voice_prefs(images: dict, favourites: list) -> int:
    """Move what used to live in the settings file into the database, once."""
    moved = 0
    for voice, url in (images or {}).items():
        if voice_save(voice, image=url):
            moved += 1
    for voice in (favourites or []):
        if voice_save(voice, favourite=True):
            moved += 1
    return moved


def voice_usage() -> dict:
    """Per voice: how many recordings, where, how much, and when last used."""
    with cursor() as cur:
        if cur is None:
            return {}
        cur.execute("""
            SELECT g.voice,
                   count(*) AS uses,
                   count(DISTINCT g.project_id) AS folders,
                   coalesce(sum(g.cost), 0) AS spend,
                   max(g.created_at) AS last_used,
                   -- Something you actually made with this voice is a better
                   -- preview than a stock clip, and it costs nothing.
                   (SELECT h.filename FROM generations h
                     WHERE h.voice = g.voice AND h.filename <> ''
                     ORDER BY h.created_at DESC LIMIT 1) AS latest
              FROM generations g
             WHERE g.voice <> '-' AND g.voice <> ''
               AND coalesce(g.kind, '') <> 'asset'
             GROUP BY g.voice
        """)
        rolled = {}
        for voice, uses, folders, spend, last, latest in cur.fetchall():
            key = voice_key(voice)
            seen = rolled.setdefault(key, {"uses": 0, "folders": 0, "spend": 0.0,
...
def production_accounting_many(production_ids: list[int]) -> dict[int, dict]:
    """Historical spend and current editable content are different facts.

    Jobs survive Part deletion. Extant generations that predate the ledger are
    added only as the positive gap above recorded speech Jobs, so old takes are
    not double counted.
    """
...
def production_accounting(production_id: int) -> dict:
    return production_accounting_many([production_id]).get(int(production_id), {
        "historical_spend": 0.0, "current_sequence_cost": 0.0,
        "retained_generation_cost": 0.0, "tracked_spend": 0.0,
