...
test("opens an audiovisual Project and renders Timeline and Library without browser warnings", async ({ page, request }) => {
  ...
  await page.getByRole("button", { name: "Library · Find and collect reusable Files" }).click()
  await expect(page.getByRole("heading", { name: "No media collected yet" })).toBeVisible()
  await page.getByRole("button", { name: "Show Create panel" }).click()
