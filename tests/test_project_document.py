"""Real PostgreSQL lifecycle checks for an editable Project document."""
...
from origins.http.project_contracts import ProjectEditorEnvelope
...
class ProjectDocumentTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.workspace_record = workspaces.create_workspace(
            f"Document {self.marker}", "Project document integration tests")
        self.workspace_id = int(self.workspace_record["id"])
        self.first = workspaces.create_audiovisual_project(
            self.workspace_id, f"First {self.marker}", "", None)
        self.second = workspaces.create_audiovisual_project(
            self.workspace_id, f"Second {self.marker}", "", None)
        self.repository = ProjectDocumentRepository()
        self.file_repository = FileRepository()
        self.transcripts = _TranscriptState()
        self.workspace = _Workspace()
        self.timeline = TimelineService(
            PostgresTimelineRecords(
                documents=self.repository, files=self.file_repository),
    ...
    def test_part_recording_music_move_and_delete_flow(self):
        ...
        intro = self.file_repository.create_workspace_file(
            self.workspace_id, name=f"Intro {self.marker}",
        ...
        music_asset = self.file_repository.create_workspace_file(
            self.workspace_id, name=f"Music {self.marker}",
        ...
        linked = self.timeline.insert_file(first_id, intro["id"])
        linked_part = next(item for item in self.repository.parts(first_id)
                           if item["id"] == linked["id"])
        self.assertEqual(linked_part["kind"], "file")
        self.assertEqual(linked_part["file_kind"], "audio")
        self.assertEqual(linked_part["file_category"], "music")
        ...
        self.assertEqual(
            self.repository.part_snapshot(duplicate["id"])["text"],
            "A revised opening",
        )
        ...
        with psycopg.connect(settings.database_url) as database:
            with database.cursor() as cursor:
                ...
                cursor.execute("""
                    INSERT INTO jobs
                        (kind,status,cost,cost_basis,project_id,part_id,clip_id,
                         payload,result,detail)
                    VALUES ('speech','ok',0.123,'actual_usage',%s,%s,%s,
        ...
        self.assertEqual(self.repository.part_snapshot(draft["id"])["text"],
                         "A quiet opening")
        ...
        saved_scene = sound_scenes.commit(first_id, current_scene["revision"], {
            "version": 1,
            "tracks": [{"id": "music", "kind": "audio", "role": "music", "name": "Music",
                        "muted": False, "clips": [{
                            "id": sound_clip_id, "file_id": music_asset["id"],
        ...
        self.assertEqual(
            (music_clip["file_id"], music_clip["filename"],
             music_clip["file_kind"],
        ...
        self.assertIsNotNone(persisted_music_clip["file_version_id"])
        ...
        unavailable_version["tracks"][0]["clips"][0]["file_version_id"] = (
            2_147_483_647
        )
        ...
        self.assertIsNone(self.repository.part_snapshot(duplicate["id"]))
        editor = work.project_editor(first_id)
        self.assertEqual(len(editor["parts"]), 3)

        self.assertEqual(next(item for item in editor["parts"]
                              if item["id"] == draft["id"])["clip_id"],
                         clip_id)
        ProjectEditorEnvelope.model_validate({"data": editor})

        accounting = ProjectAccountingRepository()
        ...
        with psycopg.connect(settings.database_url) as database:
            with database.cursor() as cursor:
                cursor.execute("""
                    SELECT 1 FROM project_parts WHERE id=%s
                """, (draft["id"],))
                ...
                self.assertIsNone(cursor.fetchone())
                cursor.execute("""
                    SELECT part_id, clip_id, payload, result, chars, detail,
                           error, cost, cost_basis
                ...
                self.assertEqual(float(sanitized_job[7]), 0.123)
                self.assertEqual(sanitized_job[8], "actual_usage")
                ...
                cursor.execute("""
                    SELECT scene.document, history.document
                      ...
                      JOIN sound_scene_history history
                        ON history.project_id=scene.project_id
                       AND history.revision=scene.revision
                     WHERE scene.project_id=%s
                """, (project_id,))
    ...
    def test_derived_video_audio_never_becomes_an_audio_undo_step(self):
        project_id = int(self.first["id"])
        file = self.file_repository.create_workspace_file(
            self.workspace_id, name=f"History {self.marker}",
        ...
        initial = records.get(project_id)
        ...
        first_audio_edit = {
            "version": 1, "sequence_overrides": {}, "tracks": [{
                ...
                "volume": 1, "muted": False, "clips": [{
                    "id": operator_clip_id, "file_id": file["id"],
        ...
        first = records.commit(
            project_id, initial["revision"], first_audio_edit)
        ...
        second = records.commit(
            project_id, derived["revision"], second_audio_edit)

        undone_second_audio = records.step(project_id, -1)
        self.assertEqual(
        ...
        synchronized = records.commit(
            project_id, undone_second_audio["revision"],
            visual_undo_projection, "derived_visual_audio")
        self.assertTrue(synchronized["can_redo"])
        redone_second_audio = records.step(project_id, 1)
    ...
    def test_visual_delete_undo_restores_exact_linked_audio_mix_history(self):
        project_id = int(self.first["id"])
        file = self.file_repository.create_workspace_file(
            self.workspace_id, name=f"Linked history {self.marker}",
        ...
        added = records.commit(
            project_id, initial["revision"], projection,
            "derived_visual_audio")
        ...
        mixed = records.commit(project_id, added["revision"], authored)

        # The visual projection contains only present placements. The Sound
        # Scene remains responsible for retaining authored audio truth.
        ...
        deleted = records.commit(
            project_id, mixed["revision"], deleted_projection,
            "derived_visual_audio")
        ...
        before_mix = records.step(project_id, -1)
        self.assertEqual(
            before_mix["document"]["tracks"][0]["clips"][0]["gain"], 1)
        after_mix = records.step(project_id, 1)
        self.assertEqual(
    ...
    def test_sequence_accepts_any_accessible_audio_classification(self):
        project_id = int(self.first["id"])
        file = self.file_repository.create_workspace_file(
            self.workspace_id, name=f"Sequence music {self.marker}",
        ...
        inserted = self.timeline.insert_file(project_id, file["id"])
        part = self.repository.part(project_id, inserted["id"])
        self.assertEqual((part["kind"], part["file_id"]),
                         ("file", file["id"]))

    def test_reorder_locks_the_active_sequence_and_persists_exact_order(self):
        project_id = int(self.first["id"])
        first = self.timeline.add_silence(project_id, 1)
        second = self.timeline.add_silence(project_id, 2)
        third = self.timeline.add_silence(project_id, 3)
        self.assertTrue(self.timeline.reorder(
            project_id, [third["id"], first["id"], second["id"]]))
        self.assertEqual(
            [item["id"] for item in self.repository.parts(project_id)],
            [third["id"], first["id"], second["id"]],
        )

    def test_editorial_revision_marks_the_active_recording_outdated(self):
        project_id = int(self.first["id"])
        draft = self.timeline.add_draft(project_id, {
            "text": "Original words",
        })
        part = self.repository.part(project_id, draft["id"])
        with psycopg.connect(settings.database_url) as database:
            with database.cursor() as cursor:
                cursor.execute("""
                    INSERT INTO clips
    ...
    def test_active_recording_projection_is_immutable_and_caption_relevant(self):
        ...
        selected_caption, _ = jobs.enqueue(
            "transcribe", {
                "part_id": draft["id"], "project_id": project_id,
                "file": filename,
            }, idempotency_key=f"selected-caption-{self.marker}",
            project_id=project_id, part_id=draft["id"])
        ...
        projected = self.repository.parts(project_id)[0]
        ...
        with psycopg.connect(settings.database_url) as database:
            with database.cursor() as cursor:
                ...
                """, (recording_id,))
            database.commit()
        historical = self.repository.parts(project_id)[0]
