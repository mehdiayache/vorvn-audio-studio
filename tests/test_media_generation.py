...
from origins.domain.media_models import KIE_CONTRACT_EVIDENCE
...
class FakeFiles:
    def __init__(self):
        ...
        self.files = [
            ...
            *[
                {"id": file_id, "media_type": "image",
                 "mime_type": "image/png", "size_bytes": 12_000,
                 "width": 1280, "height": 720}
                for file_id in range(15, 23)
            ],
...
class MediaGenerationTest(unittest.TestCase):
    def test_every_enabled_kie_manifest_has_exact_contract_evidence(self):
        catalog = MediaGenerationService(FakeJobs(), FakeFiles()).capabilities()
        enabled_kie = {
    ...
    def test_capabilities_are_scoped_to_model_and_operation(self):
        service = MediaGenerationService(FakeJobs(), FakeFiles())
        ...
        self.assertEqual(fields["elements"]["type"], "file_list")
    ...
    def test_one_image_is_a_direct_image_to_video_input_not_an_element(self):
        jobs = FakeJobs()
        service = MediaGenerationService(jobs, FakeFiles())
        value = preset(
            operation="image_to_video",
            model_id="kling-3.0-omni/image-to-video",
            inputs=[{
                "file_id": 11, "role": "source-image",
                "media_type": "image", "position": 0,
            }],
        )
        value["controls"]["ratio"] = "auto"
        projected, _ = service.enqueue(context(), value, idempotency_key="one-image")
        self.assertEqual(projected["preset"]["inputs"], value["inputs"])
        self.assertEqual(
            projected["preset"]["controls"]["provider_parameters"]["elements"],
            [],
        )
    ...
    def test_image_to_video_ratio_follows_custom_shot_mode(self):
        ...
        with self.assertRaisesRegex(ValueError, "Unsupported ratio"):
            service.enqueue(context(), value, idempotency_key="invalid-fixed-ratio")

        value["controls"]["ratio"] = "auto"
        service.enqueue(context(), value, idempotency_key="automatic-source-ratio")
        ...
        service.enqueue(context(), value, idempotency_key="directed-fixed-ratio")
    ...
    def test_start_and_end_frames_are_ordered_and_force_automatic_ratio(self):
        ...
        value = preset(
            ...
            model_id="kling-3.0-omni/image-to-video",
            inputs=[
                {"file_id": 11, "role": "start-frame",
                 "media_type": "image", "position": 0},
                {"file_id": 14, "role": "end-frame",
        ...
        projected, _ = service.enqueue(
            context(), value, idempotency_key="start-end")
        self.assertEqual(
            [item["role"] for item in projected["preset"]["inputs"]],
            ["start-frame", "end-frame"])
    ...
    def test_image_constraints_fail_before_a_provider_job_is_created(self):
        ...
        with self.assertRaisesRegex(ValueError, "too narrow"):
            service.enqueue(context(), value, idempotency_key="too-small")
    ...
    def test_multiple_ordinary_images_use_reference_input_slots(self):
        ...
        service = MediaGenerationService(jobs, FakeFiles())
        inputs = [
            {"file_id": file_id, "role": "reference-image",
             "media_type": "image", "position": position}
            for position, file_id in enumerate((11, 14))
        ]
        value = preset(
        ...
        projected, _ = service.enqueue(
            context(), value, idempotency_key="ordinary-references")
        self.assertEqual(projected["preset"]["inputs"], inputs)
    ...
    def test_reference_to_video_conditional_contracts_are_explicit(self):
        ...
        value = preset(
            operation="reference_to_video",
            model_id="kling-3.0-omni/reference-to-video",
            inputs=[{"file_id": 12, "role": "reference-video",
                     "media_type": "video", "position": 0}],
        )
        value["controls"]["ratio"] = "auto"
        service.enqueue(context(), value, idempotency_key="video-only")

        value["controls"]["ratio"] = "16:9"
        with self.assertRaisesRegex(ValueError, "Unsupported ratio"):
            service.enqueue(context(), value, idempotency_key="video-fixed-ratio")

        value["controls"]["ratio"] = "auto"
        value["controls"]["provider_parameters"]["audio"] = True
        with self.assertRaisesRegex(ValueError, "Generate audio"):
            service.enqueue(context(), value, idempotency_key="video-audio")

        value["controls"]["provider_parameters"]["audio"] = False
        value["inputs"].append({
            "file_id": 11, "role": "reference-image",
            "media_type": "image", "position": 1,
        })
        value["controls"]["ratio"] = "16:9"
        service.enqueue(context(), value, idempotency_key="video-and-image")

    def test_reference_video_technical_limits_fail_before_paid_job(self):
        files = FakeFiles()
        files.files[1]["frame_rate"] = 12
        service = MediaGenerationService(FakeJobs(), files)
        value = preset(
            operation="reference_to_video",
            model_id="kling-3.0-omni/reference-to-video",
            inputs=[{"file_id": 12, "role": "reference-video",
        ...
        with self.assertRaisesRegex(ValueError, "frame rate"):
            service.enqueue(context(), value, idempotency_key="slow-video")
    ...
    def test_reference_video_subject_accepts_exact_four_image_boundary(self):
        ...
        value["controls"]["provider_parameters"]["elements"] = [
            {
                "name": "hero", "description": "The main character",
                "variant": "images", "file_ids": [15, 16],
                "audio_file_ids": [],
            },
            {
                "name": "guide", "description": "The guide character",
                "variant": "video", "file_ids": [23],
                "audio_file_ids": [], "start_time_ms": 0,
                "end_time_ms": 8000,
            },
        ]
        projected, _ = service.enqueue(
            context(), value, idempotency_key="mixed-reference-boundary")
        self.assertEqual(projected["preset"], value)
    ...
    def test_video_input_rejects_image_plus_video_subject(self):
        ...
        value = preset(
            ...
            model_id="kling-3.0-omni/reference-to-video",
            inputs=[
                {"file_id": 12, "role": "reference-video",
                 "media_type": "video", "position": 0},
                {"file_id": 11, "role": "reference-image",
                 "media_type": "image", "position": 1},
            ],
        ...
        with self.assertRaisesRegex(
            ...
            service.enqueue(context(), value, idempotency_key="forbidden-mix")
    ...
    def test_video_only_elements_require_directed_multi_shot_mode(self):
        ...
        with self.assertRaisesRegex(ValueError, "directed multi-shot"):
            service.enqueue(context(), value, idempotency_key="video-elements-mode")

    def test_enqueue_keeps_one_ordered_canonical_input_contract(self):
        jobs = FakeJobs()
        service = MediaGenerationService(jobs, FakeFiles())
        projected, created = service.enqueue(context(), preset(), idempotency_key="one")
        self.assertTrue(created)
        kind, payload, values = jobs.enqueued[0]
        self.assertEqual(kind, "media_generate")
    ...
    def test_preset_rejects_noncanonical_or_incoherent_inputs(self):
        service = MediaGenerationService(FakeJobs(), FakeFiles())
        for file_ids, message in [
            ([11, 999], "canonical File"),
            ([11, 13], "does not accept audio"),
            ([11, 11], "unique canonical File"),
        ]:
            with self.subTest(file_ids=file_ids):
                invalid = preset()
                invalid["controls"]["provider_parameters"]["elements"] = [{
                    "name": "hero", "description": "The main character",
                    "variant": "images", "file_ids": file_ids,
                    "audio_file_ids": [],
                }]
                with self.assertRaisesRegex(ValueError, message):
                    service.enqueue(context(), invalid, idempotency_key="invalid")

        missing_description = preset()
        missing_description["controls"]["provider_parameters"]["elements"] = [{
            "name": "hero", "description": "", "variant": "images",
            "file_ids": [11, 14], "audio_file_ids": [],
        }]
        with self.assertRaisesRegex(ValueError, "description"):
            service.enqueue(context(), missing_description,
    ...
    def test_recent_restores_exact_preset_and_cancel_uses_same_job(self):
        jobs = FakeJobs()
        service = MediaGenerationService(jobs, FakeFiles())
        created, _ = service.enqueue(context(), preset(), idempotency_key="history")
        self.assertEqual(service.recent(context())[0]["preset"], preset())
        with self.assertRaisesRegex(ValueError, "cannot be canceled"):
            service.cancel(context(), UUID(created["job_id"]))
    ...
    def test_multi_shot_contract_requires_exact_total_duration(self):
        ...
        with self.assertRaisesRegex(ValueError, "add up"):
            service.enqueue(context(), value, idempotency_key="shots")
