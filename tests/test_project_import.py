"""Lean V1 Project import contracts; no provider calls."""
...
from origins.infrastructure.postgres.speech import (
...
FIXTURE = Path(__file__).parent / "fixtures" / "esther-project-import-v1.json"
...
class ProjectImportTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.workspace_record = records.create_workspace(
            f"Import {marker[:8]}", "Project import integration tests")
        self.project = records.create_audiovisual_project(
            int(self.workspace_record["id"]), f"Project {marker[:8]}", "", None)
    ...
    def test_mixed_import_appends_in_order_and_restores_draft_truth(self):
        result = self.timeline.import_document(
            self.project["id"], self.document, self.identity_ids)
        self.assertEqual(result, {"items": 10, "speech": 7, "silence": 3})
        parts = self.repository.parts(self.project["id"])
        self.assertEqual(
        ...
        with psycopg.connect(settings.database_url) as database:
            with database.cursor() as cursor:
                ...
                self.assertEqual(cursor.fetchone()[0], 0)
                cursor.execute(
                    "SELECT authored_role FROM project_parts "
                    "WHERE project_id=%s AND kind='draft' ORDER BY position",
                    (self.project["id"],),
                )
                self.assertEqual(
    ...
    def test_resolved_route_identity_is_valid_canonical_voice_truth(self):
        ...
        result = self.timeline.import_document(
            self.project["id"], document, {"narrator": route},
            {"language": "English", "output_format": "mp3"},
        )

        self.assertEqual(result["speech"], 1)
        draft = self.repository.parts(self.project["id"])[0]
    ...
    def test_invalid_document_and_missing_mapping_mutate_nothing(self):
        ...
        with self.assertRaisesRegex(TimelineError, "Item 4: text"):
            self.timeline.import_document(
                self.project["id"], invalid, self.identity_ids)
        ...
        with self.assertRaisesRegex(TimelineError, "Missing: esther"):
            self.timeline.import_document(
                self.project["id"], self.document, incomplete)
        self.assertEqual(self._active_part_count(), 0)
    ...
    def test_invalid_owned_voice_rolls_back_the_whole_import(self):
        ...
        with self.assertRaisesRegex(TimelineError, "active owned Voice"):
            self.timeline.import_document(
                self.project["id"], self.document, mappings)
        self.assertEqual(self._active_part_count(), 0)
    ...
    def test_recording_again_atomically_keeps_one_active_clip(self):
        ...
        recorded = self.repository.parts(self.project["id"])[0]
...
class ProjectImportHttpTests(unittest.TestCase):
    ...
    def test_canonical_validation_preserves_authoring_truth_only(self):
        ...
        response = client.post(
            "/api/v1/project-imports/validate", json={"document": document})
        ...
        self.assertNotIn("legacy_generation_hints", data["summary"])

        document["items"][0]["rate"] = 1.4
