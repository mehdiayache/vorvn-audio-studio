"""Lean Production -> Tracks -> Clips rendering contracts."""
...
class AudioProductionTests(unittest.TestCase):
    ...
    def test_production_render_is_normalized_and_cached(self):
        with TemporaryDirectory() as folder:
            ...
            with patch("origins.infrastructure.render_workspace._output",
                       return_value=root):
                first = FFmpegRenderWorkspace().render_production(production)
                second = FFmpegRenderWorkspace().render_production(production)
    ...
    def test_openapi_exposes_headless_render_and_peaks(self):
        ...
        self.assertIn("post", paths["/api/v1/productions/render"])
        self.assertIn("get", paths["/api/v1/productions/{production_id}/production-scene"])
