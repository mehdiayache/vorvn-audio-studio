...
from origins.http.production_contracts import ProductionEditorEnvelope
...
class ProductionDocumentTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.workspace_record = workspaces.create_workspace(
            f"Document {self.marker}", "Production document integration tests")
        self.workspace_id = int(self.workspace_record["id"])
        self.first = workspaces.create_audiovisual_production(
            self.workspace_id, f"First {self.marker}", "", None)
        self.second = workspaces.create_audiovisual_production(
    ...
    def test_part_recording_music_move_and_delete_flow(self):
        ...
        with psycopg.connect(settings.database_url) as database:
            with database.cursor() as cursor:
                ...
                cursor.execute("""
                    INSERT INTO jobs
                        (kind,status,cost,cost_basis,production_id,part_id,clip_id,
    ...
    def test_sequence_accepts_any_accessible_audio_classification(self):
        ...
        inserted = self.timeline.insert_file(production_id, file["id"])
