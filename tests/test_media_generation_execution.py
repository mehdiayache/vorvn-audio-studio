...
from origins.domain.jobs import JobFailed
...
class FakeProvider:
    ...
    def submit(self, request, *, callback_reference=None):
        ...
        return MediaGenerationSubmission("provider-job")
...
class FakeMaterializer:
    def materialize(self, file, *, job_id, role):
        return {
            "file_id": file["id"], "role": role,
            "media_type": file["media_type"],
            "url": f"https://temporary.test/{file['id']}",
        }
...
class MediaGenerationExecutionTest(unittest.TestCase):
    @staticmethod
    def _job():
        preset = {
    ...
    def test_subject_assets_materialize_temporarily_and_output_becomes_canonical(self):
        ...
        operation = {
            ...
            "parameters": [{"key": "elements", "type": "file_list"}],
            "output": {"extension": "mp4"},
        }
        job = Job(
            ...
            payload={
                ...
                "preset": preset,
                "capability_snapshot": {
                    "label": "Kling", "provider_model_id": preset["model_id"],
        ...
        with TemporaryDirectory() as directory:
            ...
            handler = MediaGenerationHandler(
                ...
                model_adapters={preset["model_id"]: adapter},
        ...
        self.assertEqual(
            [file["file_id"] for file in groups[0]["files"]], [11, 14])
    ...
    def test_resumes_persisted_provider_task_without_submitting_again(self):
        ...
        with TemporaryDirectory() as directory:
            result = MediaGenerationHandler(
                providers={"kie": provider},
                model_adapters={job.payload["model"]: FakeAdapter()},
                files=FakeFiles(), uploads=FakeUploads(),
                materializer=FakeMaterializer(),
    ...
    def test_provider_success_can_retry_only_local_ingestion(self):
        ...
        with TemporaryDirectory() as directory:
            handler = MediaGenerationHandler(
                providers={"kie": provider},
                model_adapters={job.payload["model"]: FakeAdapter()},
                files=FakeFiles(), uploads=uploads,
                materializer=FakeMaterializer(),
                operations=ProviderOperationService(operations),
                scratch_root=Path(directory), poll_interval=0,
                sleeper=lambda _: None,
