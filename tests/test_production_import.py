...
class ProductionImportTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.workspace_record = records.create_workspace(
            f"Import {marker[:8]}", "Production import integration tests")
        self.production = records.create_audiovisual_production(
            int(self.workspace_record["id"]), f"Production {marker[:8]}", "", None)
    ...
    def test_recording_again_atomically_keeps_one_active_clip(self):
        ...
        with psycopg.connect(settings.database_url) as database:
            ...
            current_file = database.execute(
                "SELECT file_id,file_version_id FROM production_parts WHERE id=%s",
                (part["id"],),
            ).fetchone()
            ...
            production_files = database.execute(
                "SELECT file_id FROM production_file_usages "
                "WHERE production_id=%s AND purpose='script' ORDER BY file_id",
                (self.production["id"],),
            ).fetchall()
        ...
        self.assertEqual(
            [row[0] for row in production_files],
            sorted([first_file["id"], second_file["id"]]),
        )
        self.assertEqual(
