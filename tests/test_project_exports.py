"""Real PostgreSQL lifecycle checks for immutable Project Exports."""
...
class ProjectExportTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.project = records.create_audiovisual_project(
            int(self.workspace_record["id"]), f"Project {self.marker}", "", None)
        self.repository = ProjectExportRepository()
    ...
    def test_export_is_atomic_immutable_history_not_a_timeline_part(self):
        ...
        created = self.repository.create(
            project_id, filename=f"final-{self.marker}.mp3",
        ...
        self.assertEqual(self.repository.list(project_id), [item])

        editor = work.project_editor(project_id)
        self.assertEqual(editor["exports"], [item])
        self.assertEqual(editor["parts"], [])
        before = len(self.repository.list(project_id))
        self.assertIsNone(self.repository.create(
            project_id + 10_000_000, filename="orphan.mp3",
            path="/tmp/orphan.mp3", manifest={}, renderer="test",
            duration_ms=1, size_bytes=1, part_count=1,
        ))
        self.assertEqual(len(self.repository.list(project_id)), before)
...
class RenderApplicationTests(unittest.TestCase):
    def test_export_finishes_files_before_recording_canonical_history(self):
        ...
        class Records:
            @staticmethod
            def project(_project_id):
                return {"id": 6, "name": "Evening Reset"}

            @staticmethod
            def parts(_project_id):
                return [part]
            ...
            @staticmethod
            def create_export(project_id, *, artifact):
                recorded.update({"project_id": project_id,
                                 "artifact": artifact})
        ...
        class Workspace:
            ...
            @staticmethod
            def finish_export(
                    project_id, project_name, parts, scene, subtitles):
        ...
        self.assertEqual(recorded["project_id"], 6)
